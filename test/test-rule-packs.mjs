...
/* ---------- 槽位与默认顺序 ---------- */
ok(RULE_SLOTS.length === 6 && RULE_SLOTS.includes('template') && RULE_SLOTS.includes('private') && RULE_SLOTS.includes('dsh'), '六个规则槽位存在（含 template 模板 + dsh 插件审计 + private 强制槽位）');
ok(DEFAULT_RULE_ORDER.join(',') === 'nodejs,frontend,comment,dsh', `默认顺序 nodejs→frontend→comment→dsh（template 不加载）实际: ${DEFAULT_RULE_ORDER.join(',')}`);

/* ---------- 私密拦截强制槽位（v1.48.0）---------- */
{
  const c = getCompiledRulePack('');
  ok(c.meta.order.join(',') === 'nodejs,frontend,comment,dsh,private', `强制槽位末尾合入（默认+dsh）实际: ${c.meta.order.join(',')}`);
...
ok(loaded.ok, '默认装载成功');
ok(loaded.order.join(',') === 'nodejs,frontend,comment,dsh', `实际加载顺序 ${loaded.order.join(',')}`);
ok(loaded.files.length === 4 && loaded.files.every((f) => !f.includes('template')), 'template.yml 不加载（默认加载 nodejs/frontend/comment/dsh 四份）');
...
ok(c.meta.order.join(',') === 'nodejs,frontend,comment,dsh', `meta.order 随装载`);
...
ok(g.secretPatterns.length === 3 && g.meta.order.join(',') === 'nodejs,frontend,comment,dsh,private', 'getCompiledRulePack 缺省装载正确（private 强制末尾）');
...
ok(resolveRulesetChoice('').source === 'yaml' && resolveRulesetChoice('').order.length === 5, '空配置 → yaml 默认顺序（含 dsh+private 强制）');
...
ok(lu.ok && lu.source === 'yaml' && lu.order.length === 5, 'loadRulePack 默认装载（含 dsh+private 强制）');
