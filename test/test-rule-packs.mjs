...
ok(loaded.ok, '默认装载成功');
ok(loaded.order.join(',') === 'nodejs,frontend,comment', `实际加载顺序 ${loaded.order.join(',')}`);
ok(loaded.files.length === 3 && loaded.files.every((f) => !f.includes('template')), 'template.yml 不加载');
...
ok(c.meta.order.join(',') === 'nodejs,frontend,comment', `meta.order 随装载`);
