...
> 同时是 dsh-git-push 插件的开发者要求：调用 `git_commit_push` 前必须按本格式收尾（或即将收尾），✅ 即提交推送授权（要求清单随插件内置 `lib/user-requirements.json`，原同级仓 requirements.md 废除）。
...
- 走插件工具 `git_commit_push`，不手敲 git；开发者要求清单（内置 `lib/user-requirements.json`，可外挂插件配置目录 `requirements.json`）逐条核对后带 `requirementsConfirmed:true`
...
- 插件配置目录 `DSH_HOME/git-push/`（原同级仓废除）：开发者要求清单（内置）+ 本机凭据（私有，不随本公开仓分发）
