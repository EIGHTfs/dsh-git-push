...
**提交 vs 推送**：commit 可自主执行（bugfix-auto-authority：提交不推送）；**push 必须用户同意**（commit-checkpoint-before-push-reorg：推送前会撤销重写整理）。
