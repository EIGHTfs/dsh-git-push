...
**io-risk 循环内 I/O 改多维分级（2026-09-20）**：不再「在循环里就判 high」——风险取决于「循环边界可不可控 + I/O 同不同步 + 在不在请求路径上」三维度：①循环内**同步** I/O：非请求路径 medium（阻塞事件循环）、请求路径 high（三条件齐）；②循环内**异步串行 await**：非请求路径 low（只慢不阻塞）、请求路径 high（响应时间线性增长）；③循环内**异步并行**（`Promise.all` 包裹，新增 `findPromiseAllRanges` AST 识别）：low（可接受）；④固定小数组循环 low（保留）。写/删类在重复上下文仍加权一档。test-io-risk 更新 2 个用例 + 新增 3 个用例（请求路径循环串行 high / 同步循环三条件 high / Promise.all 并行 low）。\
...
**测试**：`test/test-self.mjs` 新增 6 项（HELP 列出 5 命令、main 分发已接线、值参数解析与缺值报错、布尔开关、缺必填参数退出码为 1、非法 visibility 拒绝），全部离线不联网；40/40 通过。`self-check` 的 cli-help-sync 覆盖 30 个 flag 双向一致。\
| **1.4.6** | **clone 并发互斥 + 可中止 + 失败不再删目录（2026-09-18）** \
...
| **1.4.6** | **推送成功后回写仓库索引的远端状态与可见性（2026-09-19）** \
**现象**：推完一个仓库，`dsh-repo-index.json` 里该条目的 `visibility` 仍是「未知」、`remoteHead`/`remoteHeadAt`/`remoteStateAt` 停在上一轮扫描的旧值——列表看着像「刚推送完却没更新」。\
**根因**：`/api/git-push/repo-push` 推送成功后只调 `maintainRepoIndex`（**全量重建索引**）并 fire-and-forget。而全量重建**不写** `remoteHead`/`remoteHeadAt`/`remoteStateAt`（这三个是 `updateRepoRemoteStateInIndex` 的职责），`visibility` 又依赖 token 有效性；于是推送这一动作事实上一辈子不更新这 4 个字段，只有等到下次「本地扫描补查」才会刷新——中间窗口里索引就是过期的。\
**修复**：推送成功后改为**精确回写该条目**——远端 HEAD 取本次推上去的 `commitSha`，`ahead`/`behind` 归零（刚推完必然同步）、`synced:true`、刷新 `remoteStateAt`，并调 `detectRepoVisibility({repoPath})` 补查可见性真值（`private`→「私有」/`public`→「公开」）。全量重建仍保留，但降级为「登记同步」（新仓库/改名的补登记），两者职责分开。回归上仍为 fire-and-forget，索引写失败不阻断推送响应。\
...
**列表远端探测调优（同日，解决「多仓时除首个外全显示未探测」）**：原实现把 `liveRemoteHead` 放在同步 `.map()` 里**逐仓串行**——单仓 SSH 握手实测 4.2~4.8s，4 仓即 16~24s，必然撞穿 6s 预算并触发「一次失败就熔断全部剩余」的全局熔断，于是列表里**第二个仓库起全部标 liveSkipped**（前端显示「未探测远端」），而点 push 时后端又能实时探到真值——这正是「显示未探测、实际能推」的来源。\
现做三处改动：① **SSH 连接复用**（ControlMaster/ControlPersist）——`liveRemoteHead` 每次调用都重建连接，握手占掉绝大部分耗时；共享 `ControlPath` 后第二次起实测 6.3s → **1.6s**（省约 74%），跨仓库同样有效。ControlPath **不含 pid**（含 pid 时每个进程/每次插件重载各建一个 socket，既无法复用又残留 /tmp），改用 uid 隔离的稳定路径，实测可跨进程复用（A 3.8s 建立 → B/C 1.45s/1.97s）；也**不再注册 exit 清理**——共享连接池下 `ssh -O exit` 会掐断其他会话正在用的连接，交给 `ControlPersist=60` 自然回收。② **真并发探测**——新增 `runGitAsync`（`execFile` 回调版）与 `liveRemoteHeadAsync`：原 `runGit` 走 `execFileSync` **同步阻塞事件循环**，放在 Promise worker 里并不会并行（实测 6 仓仍要 13.0s ≈ 串行）；换异步后 6 仓 **6.0s**，提速 4.8x。③ **预算与熔断**——预算 6s → **15s**（它是上界而非预期耗时，正常探完即返回；6s 会在并发刚铺开时就把后续仓库判超时，恰好复现「除首个外全未探测」），并移除「全局熔断」改为**逐仓独立判定**（并发下所有仓同时开跑，熔断已无意义；单仓超时由 `liveRemoteHeadAsync` 的 8s 内建超时兜底）。\
**端到端实测**（11 条索引、真实 GitHub）：**9/11 已探测成功**（其余 2 条是目录已不存在的残留条目，标未探测是正确行为），总耗时 12.5s 未超预算；调优前同场景可探测数接近 0。\
...
| **1.4.6** | **clone 下载改双通道：先 api 后 raw（2026-09-19）** \
...
| **1.4.6** | **`.auditignore` 非 git 目录兜底 + cookie-secure-flag 误报/漏报各修一（同一版本内修订）** \
...
**测试实现两次返工，值得记下**：判据最初手写正则剥字符串/注释，两次都栽在同一类问题上——① 行注释正则 `(^|[^:])\/\/…` 的「非冒号非反斜杠」会匹配换行符，从行首吞掉代码行；② 双引号串正则遇到正则字面量里的未闭合引号会**跨行失控**，实测把 2204 行砍成 478 行，含用法的代码整段消失 → 未定义检测恒为空、**测试恒过形同虚设**。反向验证（故意删掉定义看测试是否失败）才暴露了这点。最终改用项目自带 `lib/ast/tokenizer.js`（按类型标明 ident/str/comment，取 ident 即天然排除字符串与注释），并加「token 数不得低于阈值」的自证断言防止判据再次静默失效。\
