...
| `lib/audit-rules/audit-rules-*.yml` | 审计规则 YAML 文件（v1.47.0 替换 JSON）：`nodejs` 20 规则 + 7 凭据翻译 / `frontend` 19 规则 / `comment` 关键词权重（黑 22 / 白 22，权重 ≥40 进门禁）/ `template` 空模板默认不加载；顺序装载后覆盖前 |
