...
| 版本 | 说明 |
|---|---|
| **1.3.8**（当前） | **噪音治理与实质修复**：低风险 I/O 降为 info 不计分（warning 430→269）+ `.auditignore` 豁免 client.js + 修 3 处实质缺陷（写文件 ENOENT / 探针无超时永久挂起 / 空 catch 未交代原因）+ mkdir-before-write 自举误报修复 \
