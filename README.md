...
| **1.60.1** | **移除提交历史查看器（viewer 组件）**：删除 lib/viewer.js / lib/viewer-locales.js / test/test-viewer.mjs + /git-push/viewer 页面 + /api/git-push/repos|commits|diff 三个只读端点 + 侧边栏查看器入口按钮/文案/CSS；plugin-http.js/client.js/index.js/package.json 引用面同步清零；全量 10 套件回归通过（npm run check 语法 10/10） |
