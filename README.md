...
| **1.49.0** | **侧边栏独立 UI + 插件配置精简**：①**插件配置卡（settings.plugin.item）只保留 3 块**——token 填写 / SSH key（生成+公钥）/ 登录信息（检测结果）+ 保存·检测·放弃按钮，注入开关/忽略规则/规则引擎/查看入口四块移出；②**侧边栏设置独立页（settings.section）重新设计平铺 UI**，不复用插件配置折叠卡——顶部「登录信息（只读）」块（复用 AccountTop 只读展示，不提供填写）+ 引导条「去插件配置填写 →」（含路径提示），下方「高级设置（即时保存）」承载注入三开关 / 自定义忽略 / 规则引擎（槽位排序+权重滑块） / 提交历史查看器入口；③两处仍共享同一 GitPushCardController / store（状态一致），折叠卡形态仅保留在插件配置位；被移出的高级组件函数体原样保留（注释声明，防改主意）；④新增 .dshgp_section* 平铺样式 + section* i18n 文案（zh/en） |
