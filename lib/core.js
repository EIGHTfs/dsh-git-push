...
export async function commitAndPush({ repoPath, message, push = true, dryRun = false, requirementsConfirmed = false, workspaceRoot = '', customIgnorePatterns = '' } = {}) {
  ...
  if (push) {
    const originUrl = runGit(['remote', 'get-url', 'origin'], repoPath).stdout;
    tokenInfo = resolveGitToken({ repoPath, workspaceRoot });
...
export function resolveGitToken({ tokenPath = '', repoPath = '', workspaceRoot = '' } = {}) {
  ...
  // 自动切换（v1.18.0 / v1.33.0）：设置页写入同级仓，必须先于项目内残留 .git-push-token
  const { dir: userDir, user: ownerUser } = resolveUserDir({ workspaceRoot });
  if (userDir) {
    candidates.push(join(userDir, 'github-token'));
    candidates.push(join(userDir, 'token'));
    if (ownerUser) {
      candidates.push(join(userDir, ownerUser, 'github-token'));
      candidates.push(join(userDir, ownerUser, 'token'));
    }
  }
...
export function resolveUserDir({ workspaceRoot = '', extraRepos = [], extraReposFile = '' } = {}) {
  ...
  if (workspaceRoot) {
    ...
    // 树的语义：workspaceRoot 树内有「官方 remote 且与远程一致」的仓 → 只在树内比（正常情况）。
    // 树内没有同步的官方仓（如 workspaceRoot 配置指向 NAS 旧副本，NAS 仓未 fetch 远程 refs）
    // → 全池按 isSynced 比对，默认选更接近远程的仓（排除 NAS 旧副本，不硬编码路径）。
    if (inWs.some((info) => info.matchesTarget && info.isSynced)) pool = inWs;
  }
