...
export function runGit(args, cwd, env = {}) {
  // 自动切换：从插件 User/<username>/ 探测 SSH 私钥（有则设 GIT_SSH_COMMAND，无则走默认）
...
export async function detectRepoVisibility({ repoPath = '', token = '' } = {}) {
  try {
    ...
    return { visibility: 'unknown', reason: `GitHub API ${res.status}` };
  } catch (e) {
...
export async function setRepoVisibility({ repoPath = '', visibility = '', token = '' } = {}) {
  try {
    ...
    const res = await fetch(`https://api.github.com/repos/${pr.owner}/${pr.repo}`, {
      method: 'PATCH',
      headers: H,
      body: JSON.stringify({ private: target === 'private' }),
      signal: AbortSignal.timeout(30_000),
    });
    if (res.status === 200) {
      const json = await res.json();
    ...
    let detail = '';
    try { const j = await res.json(); detail = j.message || j.errors?.[0]?.message || ''; } catch { /* 忽略 */ }
...
export async function autoTagDSHProject({ repoPath = '', version = '', commitSha = '', owner = '', repo = '', token = '' } = {}) {
  try {
    ...
    // 已存在 → 跳过（幂等）
    const exists = await fetch(`https://api.github.com/repos/${o}/${r}/git/ref/tags/${tag}`, { headers: H, signal: AbortSignal.timeout(30_000) });
    if (exists.status === 200) return { ok: false, skipped: 'tag-exists', tag };
    // 建 tag ref
    const res = await fetch(`https://api.github.com/repos/${o}/${r}/git/refs`, {
      method: 'POST',
      headers: H,
    ...
    if (res.status === 422) {
      ...
      if (/already exists/i.test(j?.message || '')) return { ok: false, skipped: 'tag-exists', tag };
      return { ok: false, skipped: '422', tag, error: j?.message || res.status };
    }
    return { ok: false, skipped: String(res.status), tag, error: 'GitHub API ' + res.status };
  } catch (e) {
    ...
    if (apiPushed?.ok && apiPushed.pushed) {
      ...
      if (behind > 0) {
        ...
        if (p.status !== 0) {
          ...
          // 自动打 tag（v1.16.0）：dsh- 前缀项目推送成功后自动打 v<version> tag
          try {
            const headSha = runGit(['rev-parse', 'HEAD'], repoPath).stdout;
            result.autoTag = await autoTagDSHProject({ repoPath, version: readPkgVersion(repoPath), commitSha: headSha, token: tokenInfo.token });
          } catch (e) {
            result.autoTag = { ok: false, skipped: 'auto-tag-error', error: String(e?.message || e) };
          }
        }
...
export function resolveGitToken({ tokenPath = '', repoPath = '', workspaceRoot = '' } = {}) {
  ...
  // 自动切换（v1.16.0）：优先从插件 User/<username>/ 读取 token（本机专用，git 忽略）
...
/**
 * 探测插件 User/ 目录下第一个子目录作为当前用户目录（不硬编码用户名，自动切换）。
 * 插件 User/ 按用户 git 忽略（.gitignore User/**），本机专用，适合存放 token 与 SSH 私钥。
 * @returns {{ dir: string, user: string }} 无子目录返回 { dir: '', user: '' }
 */
export function resolveUserDir() {
  const userRoot = join(PLUGIN_ROOT, 'User');
  try {
    if (!existsSync(userRoot)) return { dir: '', user: '' };
    for (const ent of readdirSync(userRoot, { withFileTypes: true })) {
      if (!ent.isDirectory()) continue;
      const dir = join(userRoot, ent.name);
      try { if (existsSync(dir)) return { dir, user: ent.name }; } catch { /* 跳过 */ }
    }
  } catch { /* User/ 不可读 */ }
  return { dir: '', user: '' };
}
...
/**
 * 从插件 User/<username>/ 探测 SSH 私钥（自动切换，不硬编码路径）。
 * 支持 id_ed25519 / id_rsa / id_ecdsa。返回 { keyPath, user }；未找到返回 { keyPath: '', user: '' }。
 */
export function resolveUserSshKey() {
  const { dir: userDir, user } = resolveUserDir();
