...
export function pushViaSsh({ repoPath, branch, owner, repo, force = false }) {
  const { keyPath } = resolveUserSshKey();
  if (!keyPath) {
    return { ok: false, pushed: false, reason: '无 SSH 私钥（同级仓 dsh-git-push-User/id_rsa，其次 id_ed25519）' };
  }
...
export function findGitDirs(root, depth = 3) {  let out = '';
  try {
...
export function persistGithubToken(token, { workspaceRoot = '' } = {}) {
  ...
  let { dir } = resolveUserDir({ workspaceRoot });
  if (!dir) {
    const first = userRepoCandidates({ workspaceRoot })[0];
    if (!first) return { ok: false, error: '找不到 dsh-git-push-User 目录' };
 ...
 * 生成到同级仓 dsh-git-push-User 目录（与 token/公钥同位置）：id_rsa（私钥 600）+ id_rsa.pub（公钥 644）。
...
export function collectRepoSkillDocs({ workspaceRoot = '', pluginRoot = PLUGIN_ROOT, maxFileBytes = 80_000, maxTotalBytes = 400_000 } = {}) {
  ...
  walk(join(pluginRoot, 'skills'), 'dsh-git-push/skills');
  const { dir } = resolveUserDir({ workspaceRoot });
  if (dir) walk(dir, USER_REPO_NAME);
  return files;
}

/**
 * 只收集 skill 目录与 .md 文件名（不读内容）——「只注入 skill 目录」模式。
 * 返回 [{ base, dir, files: [name, ...] }]；base 是展示名（如 dsh-git-push/skills）。
 */
/** 只收集 skill 目录与 .md 文件名（不读内容）——「只注入 skill 目录」模式。 */
/** 只收集 skill 目录与 .md 相对路径（不读内容）——「只注入 skill 目录」模式。递归全仓，跳过 .git/node_modules。 */
export function collectRepoSkillDirs({ workspaceRoot = '', pluginRoot = PLUGIN_ROOT } = {}) {
  ...
  collect(join(pluginRoot, 'skills'), 'dsh-git-push/skills');
  const { dir } = resolveUserDir({ workspaceRoot });
  if (dir) collect(dir, USER_REPO_NAME);
  return dirs;
}
...
/**
 * 设备/用户 json 脱敏注入（v1.27.0 新功能）。
 * 「获取本机信息在 gitpushuser 中创建设备 json 文件夹，保存用户 json 数据、设备 json 数据（账号密码）并注入」
 * 安全设计：device-map.json + user.json 全量注入（非敏感）；
 * ssh-credentials.json 只注入「设备账号 + 密码存放位置」清单，**不注入密码明文**
 * （明文注入上下文会发给模型服务商 + 落会话日志，四个泄露口，用户已确认脱敏方案）。
 */
export function buildDeviceUserInjection({ workspaceRoot = '' } = {}) {
  const { dir, user } = resolveUserDir({ workspaceRoot });
  if (!dir) return '';
  // json 可能在仓根，也可能在 <owner>/ 子目录（新布局：dsh-git-push-User/EIGHTfs/…）
  const candidates = [dir, user ? join(dir, user) : ''].filter(Boolean);
  const findJson = (rel) => {
    for (const base of candidates) {
      const f = join(base, rel);
      try { if (existsSync(f)) return f; } catch { /* 跳过 */ }
    }
    return '';
  };
  const parts = ['【dsh-git-push 本机设备/用户信息（json，来自同级仓 dsh-git-push-User）】'];
  // 1) 用户 json（非敏感身份）
  const userFile = findJson('user.json');
  try {
    if (userFile) {
      const u = JSON.parse(readFileSync(userFile, 'utf8'));
      parts.push('', '## 用户信息 user.json');
      parts.push(`- GitHub: ${u.user || u.github?.username || '未知'}（${u.github?.email || ''}）`);
      if (u.github?.sshKey) parts.push(`- GitHub SSH: ${u.github.sshKey}`);
    }
  } catch { /* 读取失败静默 */ }
  // 2) 设备地图（非敏感，全量注入）
  const mapFile = findJson(join('devices', 'device-map.json'));
  try {
    if (mapFile) {
      const m = JSON.parse(readFileSync(mapFile, 'utf8'));
      parts.push('', '## 设备地图 devices/device-map.json');
      for (const d of m.devices || []) {
        const ports = d.ports ? ' | 端口: ' + Object.entries(d.ports).map(([k, v]) => `${k}=${v}`).join(', ') : '';
        parts.push(`- ${d.name}（${d.host}）: ${d.role || ''}${ports} | 访问: ${(d.access || []).join('/') || '未知'}`);
      }
    }
  } catch { /* 读取失败静默 */ }
  // 3) SSH 凭据：只给账号 + 位置，不给密码（脱敏）
  const credFile = findJson(join('devices', 'ssh-credentials.json'));
  try {
    if (credFile) {
      const c = JSON.parse(readFileSync(credFile, 'utf8'));
      parts.push('', '## 设备凭据（账号清单）devices/ssh-credentials.json');
      parts.push('> ⚠️ 密码明文不注入上下文。需要真实密码时，先向用户说明要访问哪台设备、什么目的，经同意后读取文件 `dsh-git-push-User/EIGHTfs/devices/ssh-credentials.json` 对应条目（设备条目内含 rootPassword 等提权密码）。');
      for (const s of c.ssh || []) {
        parts.push(`- ${s.target}: 账号 ${s.user}`);
      }
    }
  } catch { /* 读取失败静默 */ }
  // 4) 网站凭据：只列站点清单 + 位置，不给账号密码（脱敏）
  const websitesDir = candidates.map((b) => join(b, 'websites')).find((d) => {
    try { return existsSync(d); } catch { return false; }
  });
  try {
    if (websitesDir) {
      const files = readdirSync(websitesDir).filter((f) => f.endsWith('.json'));
      parts.push('', '## 网站凭据（站点清单）websites/');
      parts.push('> ⚠️ 密码/cookie 明文不注入上下文。需要某站点凭据时，先向用户说明用途，经同意后读取 `dsh-git-push-User/EIGHTfs/websites/<站点>.json`。');
      for (const f of files) {
        parts.push(`- ${f.replace(/\.json$/, '')}`);
      }
    }
  } catch { /* 读取失败静默 */ }
  return parts.join('\n');
...
function readSshPub({ workspaceRoot = '' } = {}) {
  const { dir } = resolveUserDir({ workspaceRoot });
  if (!dir) return { configured: false, pub: '', file: '', fingerprint: '' };
...
export function probeSshGithubAuth({ workspaceRoot = '' } = {}) {
  const { keyPath } = resolveUserSshKey({ workspaceRoot });
...
/**
 * README 模板优先同级仓 dsh-git-push-User（每人习惯不同）。
 * 查找顺序：User 根 readme-template.md → User/<username>/readme-template.md → 插件默认。
 */
export function resolveReadmeTemplate({ workspaceRoot = '' } = {}) {
  const { dir } = resolveUserDir({ workspaceRoot });
  const candidates = [];
  if (dir) {
    candidates.push(join(dir, 'readme-template.md'));
    try {
      for (const ent of readdirSync(dir, { withFileTypes: true })) {
        if (ent.isDirectory() && ent.name !== '.git') {
          candidates.push(join(dir, ent.name, 'readme-template.md'));
        }
      }
    } catch { /* 列目录失败就只看根 */ }
  }
...
/**
 * 规范化生成 README 骨架。
 *  - 模板：同级仓 dsh-git-push-User/readme-template.md（「模板应该放在我 dsh-git-push-User」）
...
/**
 * 探测 GitHub token（多源，返回首个可读）：显式 tokenPath → 同级仓 dsh-git-push-User →
 * repo 内 .git-push-token → workspaceRoot data/sensitive → 常见凭据位置。
 * 设置页写入 User 仓；仓库里残留的过期 .git-push-token 不得盖过手填的新 token。
...
export function resolveGitToken({ tokenPath = '', repoPath = '', workspaceRoot = '' } = {}) {
  ...
  // v1.36.2：候选扩展到 userRepoCandidates 全列表——双副本场景（本地失效 + NAS 有效）下
  // 即使 resolveUserDir 暂时选中失效副本，也能扫到树外官方副本的有效 token。
  // 【原代码】只从 resolveUserDir 选中仓读 token
  const allUserDirs = userRepoCandidates({ workspaceRoot });
  const seenDirs = new Set();
  for (const userDir of [...allUserDirs, resolveUserDir({ workspaceRoot }).dir].filter(Boolean)) {
    const d = resolve(userDir);
    if (!d || seenDirs.has(d)) continue;
    seenDirs.add(d);
    const owner = ownerFromRemote(runGit(['remote', 'get-url', 'origin'], d).stdout) || 'EIGHTfs';
    const authorDir = join(d, owner);
    candidates.push(join(d, 'github-token'));
    candidates.push(join(d, 'token'));
    if (owner && existsSync(authorDir)) {
      candidates.push(join(authorDir, 'github-token'));
      candidates.push(join(authorDir, 'token'));
    }
  }
 ...
 * 双副本场景核心修复：本地副本 token 截断（len 24/401）不再被选中，继续找下一个有效 token。
...
export async function resolveValidGitToken(opts = {}) {
  ...
  // 逐个候选校验：resolveGitToken 返回第一个格式匹配项；失效则从源列表里继续找。
  // 复刻候选顺序，跳过已校验失效的文件，找下一个格式匹配且 probeTokenValid=true 的。
  const candidates = [];
  if (opts.tokenPath) candidates.push(opts.tokenPath);
  const allUserDirs = userRepoCandidates({ workspaceRoot: opts.workspaceRoot || '' });
  const seenDirs = new Set();
  for (const userDir of [...allUserDirs, resolveUserDir({ workspaceRoot: opts.workspaceRoot || '' }).dir].filter(Boolean)) {
    const d = resolve(userDir);
    if (!d || seenDirs.has(d)) continue;
    seenDirs.add(d);
    const owner = ownerFromRemote(runGit(['remote', 'get-url', 'origin'], d).stdout) || 'EIGHTfs';
 ...
 */
function looksLikeUserRepo(userRoot) {
  try {
    if (!existsSync(userRoot) || !statSync(userRoot).isDirectory()) return false;
    return existsSync(join(userRoot, 'requirements.md'))
      || existsSync(join(userRoot, 'github-token'))
      || existsSync(join(userRoot, '.git'));
  } catch {
    return false;
  }
}

/** 不走 runGit：runGit → resolveUserSshKey → resolveUserDir 会递归。 */
...
/**
 * 读一个候选 User 仓的 git 特征（origin / 提交数 / 与远程接近度 / 作者挂钩文件）。
 * 与远程的接近度纯探测：读本地 origin refs（不 fetch、不联网），HEAD 与 origin/master|main
 * 一致 = 更接近远程，默认优先选它（排除 NAS 等旧副本不硬编码路径）。
 * 作者挂钩文件（token / ssh key / skill md / repo-index）按作者文件夹 <owner>/ 布局加分。
 */
export function inspectUserRepo(userRoot) {
  if (!userRoot || !looksLikeUserRepo(userRoot)) return null;
  const origin = spawnGit(['remote', 'get-url', 'origin'], userRoot).stdout;
  const pr = parseGithubOwnerRepo(origin);
  const countOut = spawnGit(['rev-list', '--all', '--count'], userRoot).stdout;
  const commitCount = Number(countOut || 0) || 0;
  const headSha = spawnGit(['rev-parse', 'HEAD'], userRoot).stdout;
  const owner = pr?.owner || basename(userRoot).replace(/-User$/i, '') || 'EIGHTfs';
  const repo = pr?.repo || '';
  const authorDir = owner && owner !== 'EIGHTfs' ? join(userRoot, owner) : join(userRoot, owner || '');
  // 远程 refs（origin/master|main）纯本地读，不联网
  let remoteHeadSha = '';
  try {
    const refs = spawnGit(['for-each-ref', '--format=%(refname:short)', 'refs/remotes/origin'], userRoot).stdout;
    for (const ref of String(refs || '').split('\n')) {
      const m = /^origin\/(master|main)$/.exec(String(ref).trim());
      if (m) { remoteHeadSha = spawnGit(['rev-parse', String(ref).trim()], userRoot).stdout; break; }
    }
  } catch { /* 无远程 refs */ }
  const isSynced = !!(headSha && remoteHeadSha && headSha === remoteHeadSha);
  let hasToken = false;
  let hasRequirements = false;
  let hasSshKey = false;
  let hasIndex = false;
  let hasAuthorLayout = false;
  // v1.36.2：同步启发式 token 可用性（格式 + 长度 ≥40，ghp_ classic 为 40 位）。
  // 双副本场景：失效副本 token 常为截断/占位（如 24 位），有效 classic 恒为 40 位。
  // 选仓用同步启发式（resolveUserDir 是同步的）；push 路径另有异步真校验 resolveValidGitToken。
  let tokenValid = false;
  const rootFiles = [join(userRoot, 'github-token'), join(userRoot, 'token')];
  const authorFiles = [join(authorDir, 'github-token'), join(authorDir, 'token')];
  const rootMd = join(userRoot, 'requirements.md');
  const authorMd = join(authorDir, 'requirements.md');
  try { hasToken = rootFiles.some((p) => existsSync(p)) || authorFiles.some((p) => existsSync(p)); } catch { /* 跳过 */ }
  try {
    const tokenFile = [...rootFiles, ...authorFiles].find((p) => existsSync(p));
    if (tokenFile) {
      const t = readFileSync(tokenFile, 'utf8').trim();
      tokenValid = !!(t && /^(gh[pous]_|github_pat_)/.test(t) && t.length >= 40);
    }
  } catch { /* 跳过 */ }
  try { hasRequirements = existsSync(rootMd) || existsSync(authorMd); } catch { /* 跳过 */ }
  try { hasSshKey = ['id_rsa', 'id_ed25519', 'id_ecdsa'].some((n) => existsSync(join(userRoot, n)) || existsSync(join(authorDir, n))); } catch { /* 跳过 */ }
  try { hasIndex = existsSync(join(authorDir, 'dsh-repo-index.json')) || existsSync(join(userRoot, 'dsh-repo-index.json')); } catch { /* 跳过 */ }
  try { hasAuthorLayout = !!authorDir && existsSync(authorDir) && (existsSync(join(authorDir, 'github-token')) || existsSync(join(authorDir, 'requirements.md')) || existsSync(join(authorDir, 'dsh-repo-index.json'))); } catch { /* 跳过 */ }
  const matchesTarget = !!(pr && `${pr.owner}/${pr.repo}` === USER_REPO_TARGET);
  return {
    dir: resolve(userRoot),
    user: owner,
...
/** git 比对打分：官方 remote 优先 → 更接近远程(isSynced) → 显式配置(extraRepos) → 作者挂钩文件 → 提交数。 */
export function scoreUserRepo(info) {
  if (!info) return -1;
  let s = 0;
  if (info.matchesTarget) s += 1000;
  else if (info.repo === USER_REPO_NAME) s += 400;
  else if (info.origin) s += 50;
  if (info.explicit) s += 500; // v1.36.0：extraRepos 显式指定的 User 仓优先于自动发现的同级仓
  if (info.isSynced) s += 300; // v1.36.0：与远程 origin 一致 = 更接近远程（默认选它，排除 NAS 旧副本）
  else if (info.remoteHeadSha) s += 80; // 有远程 ref 但落后，仍比没有远程 ref 的副本接近
  // v1.36.2：token 实际可用 = 官方仓强信号（+800，压过 isSynced+300 假象）。
  // 双副本场景：失效副本 token 截断/占位（len<40）不得冒充官方仓。
  if (info.tokenValid) s += 800;
  if (info.hasAuthorLayout) s += 60; // 作者挂钩文件在作者文件夹（新布局）
  s += Math.min(Number(info.commitCount) || 0, 200);
  if (info.hasToken) s += 20;
  if (info.hasSshKey) s += 15;
  if (info.hasIndex) s += 15;
  if (info.hasRequirements) s += 10;
  return s;
}

/** 多个候选按 git 比对选出真正的 User 仓。 */
export function pickBestUserRepo(infos) {
  const list = (Array.isArray(infos) ? infos : []).filter(Boolean);
  if (!list.length) return null;
  list.sort((a, b) => {
    const ds = scoreUserRepo(b) - scoreUserRepo(a);
    if (ds) return ds;
...
export function userRepoCandidates({ workspaceRoot = '', extraRepos = [], extraReposFile = '' } = {}) {
  const out = [];
  const seen = new Set();
  const add = (p) => {
    if (!p) return;
  ...
  // workspaceRoot 优先列入（单测/指定工作区）；真正选用见 pickBestUserRepo
  ...
  // v1.36.0：目录搬家后不一定还在同级。扫 workspace / extraRepos 里叫这个名字的 git 仓。
  const scanRoots = [];
  if (workspaceRoot) scanRoots.push(resolve(workspaceRoot));
  for (const p of [...(Array.isArray(extraRepos) ? extraRepos : []), ...readExtraReposFile(extraReposFile)]) {
    try {
      const abs = resolve(p);
      if (basename(abs) === USER_REPO_NAME) add(abs);
      else if (existsSync(abs) && statSync(abs).isDirectory()) scanRoots.push(abs);
    } catch { /* 跳过 */ }
  }
  for (const root of scanRoots) {
    try {
      for (const repoPath of findGitDirs(root, 4)) {
        if (basename(repoPath) === USER_REPO_NAME) add(repoPath);
      }
    } catch { /* 跳过 */ }
  }
  return out;
...
export function loadUserRequirements({ repoPath = '', owner = '', workspaceRoot = '' } = {}) {
  ...
  const { dir, user: resolvedUser } = resolveUserDir({ workspaceRoot });
  const bases = [];
  if (dir && existsSync(dir)) {
    bases.push(dir);
    const authorDir = join(dir, resolvedUser || user);
    if (resolvedUser && existsSync(authorDir)) bases.push(authorDir);
  }
  const files = [];
 ...
 * 网络异常按「无效」处理（false），但调用方在单候选无竞争时不受影响（见 scoreUserRepo 注释）。
 ...
 * 「不要存插件目录了，就从github获取仓库到同一层级」
 * 【原代码】扫插件 User/ 下第一个子目录当用户目录
 * @returns {{ dir: string, user: string, root: string }} 找不到返回空串
 */
export function resolveUserDir({ workspaceRoot = '', extraRepos = [], extraReposFile = '' } = {}) {
  const roots = userRepoCandidates({ workspaceRoot, extraRepos, extraReposFile });
  // v1.36.0：extraRepos/extraReposFile 里显式给出 dsh-git-push-User 路径 → 打 explicit 标记，评分加权
  const explicit = new Set();
  for (const p of [...(Array.isArray(extraRepos) ? extraRepos : []), ...readExtraReposFile(extraReposFile)]) {
    try {
      const abs = resolve(p);
      if (basename(abs) === USER_REPO_NAME) explicit.add(abs);
    } catch { /* 跳过 */ }
  }
  const infos = [];
  for (const userRoot of roots) {
    try {
      const flat = inspectUserRepo(userRoot);
      if (flat) infos.push({ ...flat, explicit: explicit.has(flat.dir) });
      else if (existsSync(userRoot) && statSync(userRoot).isDirectory()) {
        // 兼容旧嵌套 User/<username>/
        for (const ent of readdirSync(userRoot, { withFileTypes: true })) {
          if (!ent.isDirectory() || ent.name.startsWith('.')) continue;
          const nested = inspectUserRepo(join(userRoot, ent.name));
          if (nested) infos.push({ ...nested, user: nested.user || ent.name, root: userRoot, explicit: explicit.has(join(userRoot, ent.name)) });
        }
      }
    } catch { /* 不可读 */ }
  }
  // 指定 workspace 时：先在该树内 git 比对，避免本机真实仓抢走单测/指定工作区。
  // 树内没有（User 仓已搬出）再对全部候选 git 比对。
  let pool = infos;
  if (workspaceRoot) {
    const ws = resolve(workspaceRoot);
    const prefix = ws.endsWith('/') ? ws : `${ws}/`;
    const inWs = infos.filter((info) => info.dir === ws || String(info.dir).startsWith(prefix));
    // 树的语义：workspaceRoot 树内有「官方 remote 且与远程一致且 token 可用」的仓 → 只在树内比（正常情况）。
    // v1.36.2：树内优先条件加 tokenValid——失效 token 的副本（isSynced 假象）不再锁死树内，
    // 让树外「token 可用」的官方副本（如 NAS）在全池比对中胜出。
    // 树内没有同步且可用的官方仓（如 workspaceRoot 配置指向 NAS 旧副本，NAS 仓未 fetch 远程 refs）
    // → 全池按 isSynced 比对，默认选更接近远程的仓（排除 NAS 旧副本，不硬编码路径）。
    if (inWs.some((info) => info.matchesTarget && info.isSynced && info.tokenValid)) pool = inWs;
  }
  const best = pickBestUserRepo(pool);
  if (!best) return { dir: '', user: '', root: '' };
  return { dir: best.dir, user: best.user, root: best.root, origin: best.origin || '', commitCount: best.commitCount, matchesTarget: !!best.matchesTarget };
}
...
export function resolveUserSshKey({ workspaceRoot = '' } = {}) {
  ...
  // v1.33.0：id_ed25519 常未绑定 GitHub，id_rsa 才是设置页生成并实测通过的钥匙。
  // 先扫 rsa，避免探测/回退推送卡在未绑定的 ed25519 上显示「SSH 未认证」。
  ...
  if (user) {
    for (const name of ['id_rsa', 'id_ed25519', 'id_ecdsa']) {
      const p = join(userDir, user, name);
      try {
        if (existsSync(p) && statSync(p).isFile()) return { keyPath: p, user };
      } catch { /* 跳过 */ }
    }
 ...
 * v1.36.3：clone 失败（无 token / 远端不存在 / 网络错误）时落「内置模板」自动建仓——
 *   README + .gitignore + <owner>/ 作者文件夹（owner 变量，三级解析：origin → 目录名 → 兜底），
 *   保证每个 GitHub 用户首次使用插件即有可用的 dsh-git-push-User。
 * @returns {{ ok: boolean, dest?: string, cloned?: boolean, created?: 'template', template?: boolean, skipped?: string, error?: string }}
 */
export async function ensureUserRepoSibling({ workspaceRoot = '', token = '' } = {}) {
  ...
  if (tok) {
    ...
    if (r.ok) return { ok: true, dest: r.dest, cloned: true, method: r.method, files: r.files };
    // clone 失败（远端不存在 / 网络错）→ 落模板，不中断
  }
  // v1.36.3：内置模板落地（无 token 或 clone 失败均走此分支）
  return createUserRepoTemplate({ dest, workspaceRoot });
}

/**
 * v1.36.3：用插件内置模板创建 User 仓（README + .gitignore + <owner>/ 作者文件夹 + git init + 初始 commit）。
 * owner 三级解析：origin remote → 目录名去 -User 后缀 → 'EIGHTfs' 兜底（全部变量，不写死路径）。
 * @param {{ dest: string, workspaceRoot?: string }} opts
 * @returns {{ ok: boolean, dest?: string, created?: 'template', template?: boolean, owner?: string, error?: string }}
 */
export function createUserRepoTemplate({ dest = '', workspaceRoot = '' } = {}) {
  try {
    if (!dest) return { ok: false, error: '缺少目标目录' };
    // owner 变量解析：先看 workspaceRoot 树内/同级是否已有带 origin 的仓；没有则目录名回退
    let owner = '';
    try {
      const { dir: resolved } = resolveUserDir({ workspaceRoot });
      if (resolved) {
        const o = ownerFromRemote(runGit(['remote', 'get-url', 'origin'], resolved).stdout);
        if (o) owner = o;
      }
    } catch { /* 探测失败走回退 */ }
    if (!owner) owner = basename(dest).replace(/-User$/i, '') || '';
    if (!owner) owner = 'EIGHTfs'; // 兜底默认（与全库 owner 解析一致）
    // 目标目录已非空且不是我们刚建的 → 不覆盖
    if (existsSync(dest) && readdirSync(dest).length > 0) {
      return { ok: true, dest, created: 'template', skipped: 'dest-nonempty' };
    }
    mkdirSync(join(dest, owner), { recursive: true });
    // 复制内置模板（template/user-repo/README.md + .gitignore）
    const tplDir = join(PLUGIN_ROOT, 'template', 'user-repo');
    for (const f of ['README.md', '.gitignore']) {
      const src = join(tplDir, f);
      if (existsSync(src)) copyFileSync(src, join(dest, f));
    }
    // git init + 初始 commit（/tmp 中转规避 CIFS chmod 问题）
    const tmpRoot = mkdtempSync(join('/tmp', 'dsh-tpl-'));
    try {
      const tmpRepo = join(tmpRoot, 'repo');
      const ini = spawnSync('git', [...gitCFlags(tmpRepo), 'init', tmpRepo], { encoding: 'utf8', timeout: 60_000, stdio: ['pipe', 'pipe', 'ignore'] });
      if (ini.status !== 0) return { ok: false, dest, error: `git init 失败: ${ini.stderr || ini.status}` };
      // 模板内容拷入中转仓
      mkdirSync(join(tmpRepo, owner), { recursive: true });
      copyFileSync(join(dest, 'README.md'), join(tmpRepo, 'README.md'));
      copyFileSync(join(dest, '.gitignore'), join(tmpRepo, '.gitignore'));
      const add = spawnSync('git', [...gitCFlags(tmpRepo), 'add', '-A'], { cwd: tmpRepo, encoding: 'utf8', timeout: 60_000, stdio: ['pipe', 'pipe', 'ignore'] });
      if (add.status !== 0) return { ok: false, dest, error: `git add 失败: ${add.stderr || add.status}` };
      const cm = spawnSync('git', [...gitCFlags(tmpRepo), '-c', 'user.name=DSH Agent', '-c', 'user.email=agent@dsh.local', 'commit', '-m', 'chore: dsh-git-push-User 内置模板初始化'], { cwd: tmpRepo, encoding: 'utf8', timeout: 60_000, stdio: ['pipe', 'pipe', 'ignore'] });
      if (cm.status !== 0) return { ok: false, dest, error: `初始 commit 失败: ${cm.stderr || cm.status}` };
      // 中转仓整拷回目标（兼容 CIFS 卷）
      rmSync(dest, { recursive: true, force: true });
      mkdirSync(dirname(dest), { recursive: true });
      const cp = spawnSync('cp', ['-a', `${tmpRepo}/.`, dest], { encoding: 'utf8', timeout: 60_000, stdio: ['pipe', 'pipe', 'ignore'] });
      if (cp.status !== 0) return { ok: false, dest, error: `拷贝失败: ${cp.stderr || cp.status}` };
      return { ok: true, dest, created: 'template', template: true, owner };
    } finally {
      rmSync(tmpRoot, { recursive: true, force: true });
    }
  } catch (e) {
    return { ok: false, dest, error: String(e?.message || e) };
  }
}
