  ...
  if (push) {
    ...
    if (!originUrl || !pr) {
      ...
      if (apiPushed?.ok && apiPushed.pushed) {
        ...
        // 这里用 update-ref 手动写入，让 git log origin/<branch> 等能看到远端最新。
        try {
          const branchRef = apiPushed.branch || branch;
          const upd = runGit(['update-ref', `refs/remotes/origin/${branchRef}`, apiPushed.commitSha], repoPath);
          result.push.remoteRef = upd.status === 0
            ? `refs/remotes/origin/${branchRef} = ${apiPushed.commitSha.slice(0, 7)}`
...
/**
 * 插件功能说明书全文（v1.28.0）：读 skills/dsh-git-push-functions.md。
 * 「把插件每个功能写份说明书强制注入全部内容（不受设置影响）」——本函数无条件返回全文，
 * 由 pre-step 钩子每个会话强制注入，不受 injectFullSkill 开关影响。
 */
export function collectFunctionManual({ pluginRoot = PLUGIN_ROOT } = {}) {
...
export function resolveGitToken({ tokenPath = '', repoPath = '', workspaceRoot = '' } = {}) {
  ...
  // 【改为】「干脆不要存插件目录了，就从github获取仓库到同一层级吧」
  ...
  if (userDir) {
    ...
    // v1.27.0 布局：凭据 json/token 在 <owner>/ 子目录（dsh-git-push-User/EIGHTfs/github-token）
    if (ownerUser) {
  ...
  for (const userRoot of roots) {
    try {
      ...
      if (existsSync(join(userRoot, 'requirements.md')) || existsSync(join(userRoot, 'github-token')) || existsSync(join(userRoot, '.git'))) {
        let user = 'EIGHTfs';
        // 不能调 runGit：runGit 会 resolveUserSshKey → 再进本函数，形成递归。「到同一层级」
        try {
