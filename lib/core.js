...
export async function pushViaApi({ repoPath, branch, token }) {
  ...
  if (remoteHead === headSha) return { ok: true, pushed: false, reason: '无新提交可推送', owner, repo };
  ...
  if (remoteHead) {
    ...
    if (localTree && remoteTree && localTree === remoteTree) {
      return { ok: true, pushed: false, reason: '无新提交可推送', owner, repo };
    }
  }
  ...
  if (remoteHead) {
    const r = await api(`/git/refs/heads/${branch}`, 'PATCH', { sha: c.json.sha, force: false });
    if (r.status !== 200) return { ok: false, pushed: false, reason: `更新 ref 失败: ${r.json?.message || r.status}` };
  } else {
    const r = await api('/git/refs', 'POST', { ref: `refs/heads/${branch}`, sha: c.json.sha });
    if (r.status === 201) {
      return { ok: true, pushed: true, commitSha: c.json.sha, owner, repo };
    }
    if (r.status === 422 && /already exists/i.test(r.json?.message || '')) {
      const r2 = await api(`/git/refs/heads/${branch}`, 'PATCH', { sha: c.json.sha, force: false });
