...
/**
 * 读一个候选 User 仓的 git 特征（origin / 提交数 / token）。找不到返回 null。
 */
export function inspectUserRepo(userRoot) {
  ...
  try { hasToken = existsSync(join(userRoot, 'github-token')) || existsSync(join(userRoot, pr?.owner || '', 'github-token')); } catch { /* 跳过 */ }
  try { hasRequirements = existsSync(join(userRoot, 'requirements.md')) || existsSync(join(userRoot, pr?.owner || '', 'requirements.md')); } catch { /* 跳过 */ }
...
/** git 比对打分：匹配官方 User 仓 remote 优先，其次显式配置(extraRepos)，再提交数/token/要求清单。 */
