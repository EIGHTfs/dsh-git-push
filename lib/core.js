...
export function userRepoCandidates({ workspaceRoot = '' } = {}) {
  ...
  for (const root of [process.env.DSH_WORKSPACE, process.env.HOME && join(process.env.HOME, '工作区')]) {
    if (root) add(join(root, USER_REPO_NAME));
  }
  // 【原代码】插件内 User/ 仍探测一次，只为旧副本过渡，新安装不再往这里写
  ...
  for (const userRoot of roots) {
    try {
      ...
      if (existsSync(join(userRoot, 'requirements.md')) || existsSync(join(userRoot, 'github-token')) || existsSync(join(userRoot, '.git'))) {
        ...
        try {
          const r = spawnSync('git', ['-c', `safe.directory=${userRoot}`, 'remote', 'get-url', 'origin'], {
