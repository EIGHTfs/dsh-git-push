 ...
 *
 * 2026-09-02：所有 GitHub 网络操作默认且仅走 api.github.com（Git Data API /
 * REST）。禁止 git clone/push/fetch 直连 github.com、ssh.github.com、
 * raw.githubusercontent.com、codeload.github.com。
 * 用户原话：「修复此插件，使所有功能都默认api.github.com」
 * AI 思路：本机实测 tarball 会 302 到 codeload.github.com；git push origin
 * 回退会打 github.com——这两条都必须拿掉，clone 改走 git/trees + git/blobs。
 *
...
export async function commitAndPush({ repoPath, message, push = true, dryRun = false, requirementsConfirmed = false } = {}) {
  ...
  // push（v1.17.0：只走 api.github.com Git Data API，不再回退 git push origin）
  // 用户原话：「修复此插件，使所有功能都默认api.github.com」
  // 【原代码】API 失败后回退 git push origin（SSH origin + token 时经 HTTPS+token，GIT_ASKPASS 注入）
  // 【改为】失败即返回错误，绝不 git fetch/push github.com / ssh.github.com
  // 【思路】本机实测 git ls-remote https://github.com 会被包装器拦截或超时；回退等于打被阻断域名
  result.push = { pushed: false, reason: 'push=false' };
  if (push) {
    const originUrl = runGit(['remote', 'get-url', 'origin'], repoPath).stdout;
    const tokenInfo = resolveGitToken({ repoPath });
    if (!originUrl) {
      result.push = { pushed: false, reason: '无 origin，无法走 api.github.com 推送' };
    } else if (!tokenInfo.token) {
      result.push = { pushed: false, reason: '无 GitHub token，无法走 api.github.com 推送（不再回退 git push github.com）' };
    } else {
      let apiPushed = null;
      try {
      ...
      if (apiPushed?.ok && apiPushed.pushed) {
        ...
        result.push = { pushed: false, reason: apiPushed?.reason || 'api.github.com 推送失败（已禁用 github.com 回退）', method: 'api' };
      }
    }
  }
