...
export function registerPreStepInjection(ctx, { envInjectText, log } = {}) {
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    try {
      // 2026-09-20：注入 cwd 取**会话工作区**（agent.session.header.cwd，与本机宿主
      //   sandbox-policy 的 resolve 同源：`session?.header.cwd ?? this.workspaceRoot`），
      //   而不是宿主全局 workspaceRoot（= DSH 安装根）——否则 AI 拿到的 cwd/项目根/
      //   工作区子目录全是框架根，任务落位全错。无 session（非会话场景）回退默认。
      const sessionCwd = (agent.session && agent.session.header && agent.session.header.cwd) || '';
      const text = String(envInjectText(sessionCwd) ?? '');
