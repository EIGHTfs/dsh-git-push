...
/** 默认检查参数（可被调用方 opts 覆盖）。 */
const DEFAULT_CONCURRENCY = 10;
const DEFAULT_TIMEOUT_MS = 5000;
const DEFAULT_MAX_LINKS = 200;
export const DEFAULT_OPTS = { concurrency: DEFAULT_CONCURRENCY, timeoutMs: DEFAULT_TIMEOUT_MS, maxLinks: DEFAULT_MAX_LINKS };

/** flaky 主机（网络抖动）的严重度折扣系数：临时故障在 flaky 域名上进一步打折。 */
const FLAKY_SCORE_DISCOUNT = 0.2;
...
export function isFlakyHost(host = '') {
  const hostNorm = String(host).toLowerCase().split(':')[0].replace(/^www\./, '');
  return FLAKY_HOSTS.some((f) => hostNorm === f || hostNorm.endsWith('.' + f));
}
...
export function extractLinks(text = '') {
  ...
  for (let i = 0; i < lines.length; i++) {
    for (const match of lines[i].matchAll(/https?:\/\/[^\s)\]}"'`<>，。；]+/g)) {
...
export function gradeResult(url = '', r = {}) {
  ...
  if (TRANSIENT_STATUSES.includes(status)) {
    ...
    const score = flaky ? Number((base * FLAKY_SCORE_DISCOUNT).toFixed(2)) : base;
...
export async function probeLink(url, { timeoutMs = DEFAULT_OPTS.timeoutMs, fetcher } = {}) {
  ...
  try {
    const headResp = await fetchImpl(url, { method: 'HEAD', redirect: 'follow', signal: AbortSignal.timeout(timeoutMs) });
    return { url, status: headResp.status, ...gradeResult(url, { status: headResp.status }) };
  } catch (e) {
...
export async function probeLinks(links = [], { concurrency = DEFAULT_OPTS.concurrency, timeoutMs = DEFAULT_OPTS.timeoutMs, fetcher } = {}) {
  ...
  const workers = new Array(Math.max(1, Math.min(concurrency, links.length || 1))).fill(0).map(async () => {
    for (;;) {
      ...
      const r = await probeLink(linkEntry.url, { timeoutMs, fetcher });
      results[i] = { ...r, line: linkEntry.line };
    }
  });
