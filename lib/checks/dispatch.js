...
import { checkFuncLines, checkEmptyCatch, checkMinLength, checkComplexity, checkDepth, checkMaxLines, checkRepeated } from './structural.js';
...
export function runChecks({ file, relPath, text, grouped }, opts = {}) {
  ...
  // 2026-09-17：I/O 风险分级 —— 经「正则初筛 → AST 精筛」产出（robustness/io-risk
  //   规则 + astConfirmKind: io-risk），不在此独立调用检查器：两条路径会为同一处 I/O
  //   各报一次（实测 6 处重叠），且既有架构里 astConfirmKind 类规则一律只走正则路径、
  //   AST 只做过滤（与 shell-cd-dynamic / write-into-gitignored 等一致）。
  return findings;
}
