...
export function runChecks({ file, relPath, text, grouped }, opts = {}) {
  ...
  // 2026-09-17：I/O 风险分级（取代原 quality/sync-fs —— 分级版是其超集，
  //   覆盖异步路径/循环内/请求路径/启动路径四上下文，并区分读/写/删/改名）
  if (!quick) findings = findings.concat(checkIoRisk({ file, text }));
  return findings;
}
