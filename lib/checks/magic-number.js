...
import { isModuleConstAssignment } from '../ast/scope.js';
...
export function checkMagicNumberSmart({ file, text, rules }) {
  ...
  for (const hit of checkMagicNumberSmartAst(text, opts)) {
    // 2026-09-23 作用域豁免（方案最小实验）：模块顶层 `const MAX_RETRY = 3` 类命名常量不报魔数
    if (isModuleConstAssignment(text, hit.line)) continue;
