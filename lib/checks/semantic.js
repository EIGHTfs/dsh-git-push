...
/** 顶层 `- id:` 行（缩进 0 的覆盖），或缩进 1 的 insert 块内列表项。 */
const ID_TOP_RE = /^- id:\s*["']?([^"'\s]+)["']?\s*$/;
const ID_INDENT_RE = /^\s+- id:\s*["']?([^"'\s]+)["']?\s*$/;
...
export function checkPatchInsert({ file, text, rules }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ID_TOP_RE.test(t) && /^- id:/.test(raw)) {
      // 缩进 0 的 - id 行 = 顶层覆盖已有 loader 行（无论当前是否在 insert 块内，都终结 insert 块）
      const id = t.match(ID_TOP_RE)[1];
      overrideIds.push({ id, line: i + 1 });
      inInsert = false;
      continue;
    }
    if (inInsert && ID_INDENT_RE.test(raw)) {
      const id = raw.trim().match(ID_TOP_RE)[1];
