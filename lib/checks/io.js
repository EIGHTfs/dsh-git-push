...
export function checkIoRisk({ file, text }) {
  ...
  for (const hit of hits) {
    ...
    // 低风险（启动路径上的一次性同步 I/O，如模块顶层/apply() 里读一次配置）
    //   性质上是**提示**而非缺陷：它不影响并发、不随数据量放大，改动收益通常不抵成本。
    //   此前一律按 warning 计 1 分，导致 173 条 io-risk 里 161 条纯提示持续拉低
    //   「健壮性/性能」两维得分，真实高危（high/medium）被淹没在噪声里。
    //   现降为 info + scoreImpact 0：照常展示（含上下文与原因），但不计入评分维度
    //   ——与 score/index.js 既有约定一致（scoreImpact===0 的纯提醒不拉低质量分）。
    const isActionable = hit.risk === 'high' || hit.risk === 'medium';
    findings.push(makeFinding({
      file,
      line: hit.line,
      rule: 'robustness/io-risk',
      kind: 'io-risk',
      severity: isActionable ? 'warning' : 'info',
      message: `${badge} ${label}风险：${hit.call}（${hit.type}/${hit.kind}，${ctxText}）——${hit.reason}`,
      dimensions: ['性能', '健壮性'],
      exemptHint: HINT_IO,
      scoreImpact: isActionable ? (hit.risk === 'high' ? 2 : 1) : 0,
    }));
  }
  return findings;
}
