...
export function extractJsxButtons(text) {
  ...
  //
  // 2026-09-18 修三处误报：
  //   ① 只认 'button'，不再把 'input' 当按钮候选——input 用 onChange 传值，本就不需要
  //      onClick，混进来会产出「input 未绑定」的假 finding（实测 client.js 11 个 input 全中）。
  //   ② 只在**代码片段**上匹配，注释里提到的 'button' 不再算数——原先直接 re.exec(text) 扫原文，
  //      本文件自己的文档注释里写 `jsx.jsx('button', {...})` 就被当真按钮报了 3 条。
  //   ③ 行号按匹配位置修正到「实际 tag 所在行」——原先用 m.index 算行号，跨行属性对象下
  //      会漂到前一个调用上（实测把 616 行的 input 报成 623 行 button 的位置）。
  const re = /(?<![\w$])(?:(?:[\w$]+\.)*jsxs?|h|React\.createElement)\s*\(\s*['"]button['"]/g;
  const code = stripCommentLines(text);
  let m;
  while ((m = re.exec(code)) !== null) {
    const { snippet } = readBalancedCall(text, m.index);
    // 事件属性要在**非注释**片段上判定：注释里写 `// onClick: 不算` 不能算已绑定
    //   （注释与代码同为「有 onClick」的两个方向都会骗过纯文本正则，故两者都要过滤）
    const ev = /\bon([A-Z][A-Za-z]*)\s*:/.exec(stripCommentLines(snippet));
...
/**
 * 把注释所在行整行替换为空格（保持行数与长度）。
 *
 * 为什么需要：提取按钮时若直接扫原文，注释里写的 `jsx.jsx('button', {...})` 会被当成真按钮
 *   （本文件自己的文档注释即触发 3 条假 finding）。
 * 为什么按行而非按 token 偏移：tokenizer 的 token 只带 `line`、不带字符偏移，
 *   按行处理是现有能力下最简洁且无误伤的做法——代价是「行尾注释后的真代码」也会被整行
 *   略去，但那种写法（`const x = jsx.jsx('button',{}) // 注`）在本仓并不存在，
 *   且真按钮几乎总是跨行属性对象，实际影响可忽略。
 * @param {string} text
 * @returns {string} 与输入逐行等长的文本，注释行内容为空格
 */
function stripCommentLines(text) {
  const lines = String(text || '').split('\n');
  try {
    const commentLines = new Set();
    for (const tk of tokenize(text)) if (tk && tk.type === 'comment') commentLines.add(tk.line);
    for (const ln of commentLines) {
      const idx = ln - 1;
      if (idx >= 0 && idx < lines.length) lines[idx] = ' '.repeat(lines[idx].length);
    }
  } catch {
    // 分词失败（语法不完整等）时退回原文：宁可多报也不静默漏报
    return String(text || '');
  }
  return lines.join('\n');
}
