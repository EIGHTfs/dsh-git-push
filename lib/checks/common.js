...
import { checkMagicNumberSmartAst, checkSmallFileReadAst, checkShortFunctionNameAst, checkPlaceholderCredentialAst, checkCredentialRefAst, shellCdDynamicLines as shellCdDynamicAst, writeIntoGitignoredLines as writeIntoGitignoredAst, scanIoRiskAst } from '../ast/index.js';

/** 质量类检查的豁免提示（文件头=整文件豁免）。单处定义，多处复用，避免重复字面量。 */
export const HINT_QUALITY = 'dsh-skip-quality（文件头=整文件）';
/** 大文件/目录类检查的豁免提示（只能文件头）。 */
export const HINT_SIZE = 'dsh-skip-size（文件头=整文件）';
/** 敏感/凭据类检查的豁免提示（文件头=整文件 / 行尾=本行）。 */
export const HINT_SENSITIVE = 'dsh-skip-sensitive（文件头=整文件）';
...
/**
 * astConfirmKind: "shell-cd-dynamic" 的集合取值——cd 到动态路径（$VAR/$()）且无兜底
 * 的行号（报）。见 lib/ast/shell.js shellCdDynamicLines。
 */
/**
 * astConfirmKind: "io-risk" 的集合取值——AST 确认为「真实 fs 调用」的行号。
 *   正则初筛只看字符（会把注释/字符串里的 `fs.writeFile(...)` 示例、规则描述文本
 *   一并命中）；本集合由 scanIoRiskAst 经 tokenizer 给出，注释与字符串内的调用
 *   不会出现在其中 —— 正则误报在此被 AST 修正。
 * @param {string} text 文件全文
 * @returns {Set<number>} 真实 I/O 调用所在行号集合
 */
export function ioRiskLines(text) {
  return new Set(scanIoRiskAst(String(text || '')).map((h) => h.line));
}

/**
 * astConfirmKind: "io-risk" 的分级详情——行号 → 该行的分级命中（供 finding message 用）。
 *   同一行可能有多个 I/O 调用，取**风险最高**的一条描述（改造优先级视角）。
 * @param {string} text 文件全文
 * @returns {Map<number, object>} 行号 → 命中项
 */
export function ioRiskDetail(text) {
  const out = new Map();
  const rank = { high: 0, medium: 1, low: 2, safe: 3 };
  for (const h of scanIoRiskAst(String(text || ''))) {
    const prev = out.get(h.line);
    if (!prev || (rank[h.risk] ?? 9) < (rank[prev.risk] ?? 9)) out.set(h.line, h);
  }
  return out;
}

export function shellCdDynamicLines(text) {
  return shellCdDynamicAst(String(text || ''));
}

/**
 * astConfirmKind: "write-into-gitignored" 的集合取值——写文件/目录目标命中仓库
 * .gitignore 且可能不存在的行号（报）。需仓库上下文 repoPath（读 .gitignore），
 * 无上下文时返回空集合（不报）。见 lib/ast/shell.js writeIntoGitignoredLines。
 */
export function writeIntoGitignoredLines(text, repoPath) {
  return writeIntoGitignoredAst(String(text || ''), String(repoPath || ''));
}
