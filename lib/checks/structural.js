...
import { classifyFunctionPath } from '../ast/scope.js';
...
export function checkFuncLines({ file, text, rules }) {
  ...
  for (const hit of astHits) {
    // 2026-09-23 作用域豁免（P1）：规则声明 scope_rules 且函数为启动路径（apply/init/main 等）→ exempt
    if (rule.scopeRules?.length && resolveScopeAction(rule.scopeRules, { path: classifyFunctionPath(hit.name), functionName: hit.name }) === 'exempt') continue;
...
export function checkComplexity({ file, text, rules }) {
  ...
  for (const hit of checkComplexityAst(text, { warn: threshold, block: threshold * 2 })) {
    // 2026-09-23 作用域豁免（P1）：启动路径函数豁免圈复杂度
    if (rule.scopeRules?.length && resolveScopeAction(rule.scopeRules, { path: classifyFunctionPath(hit.name), functionName: hit.name }) === 'exempt') continue;
    findings.push(makeFinding({
