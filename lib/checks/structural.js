...
export function checkFuncLines({ file, text, rules }) {
  ...
  const fnScope = rule.scopeRules?.length ? analyzeFunctionalScope(text) : new Map(); // 2026-09-23 P4：闭包双重作用域（public 不豁免）
  for (const hit of astHits) {
    // 2026-09-23 作用域豁免（P1）：规则声明 scope_rules 且函数为启动路径（apply/init/main 等）→ exempt；P4：public（被 return/挂 this/exports 暴露）按模块级不豁免
...
export function checkComplexity({ file, text, rules }) {
  ...
  const fnScope2 = rule.scopeRules?.length ? analyzeFunctionalScope(text) : new Map(); // 2026-09-23 P4
  for (const hit of checkComplexityAst(text, { warn: threshold, block: threshold * 2 })) {
    // 2026-09-23 作用域豁免（P1）：启动路径函数豁免圈复杂度；P4：public 不豁免
