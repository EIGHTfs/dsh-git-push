...
export function mapWorkspaceDirs({ workspaceRoot = '', cwd = process.cwd(), maxChildren = 40 } = {}) {
  ...
  try {
    ...
    const top = String(g.stdout || '').trim();
...
export function parseEnvInjection(text = '') {
  ...
  for (const line of String(text).split('\n')) {
    ...
    else if (key.startsWith('工作区根目录')) out.workspaceRoot = val;
    else if (key.startsWith('工作区子目录')) out.children = val.split(' / ').map((x) => x.trim()).filter(Boolean);
    else if (key === '工具') {
