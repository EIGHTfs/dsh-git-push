...
export const FUNCTION_USAGE_HINT = [
  ...
  '· git_commit_push —— 一键提交并推送（先审计→再 commit→再 push；审计同步拦截，通过后 commit+push 走后台，立即返回 async:true + taskId，可继续干别的）。参数：repo=仓库绝对路径、message=提交信息（必填）、audit、dryRun、push、force、ignorePatterns；调用前逐条核对开发者特殊要求，达标才传 requirementsConfirmed=true。用 git_push_status <taskId> 查推送结果。',
  '· git_push_status —— 查询后台提交推送任务（taskId）；返回 status：pending/running/done/error，done 后附 result/error。异步提交后用它在干别的事之间顺带查结果。',
