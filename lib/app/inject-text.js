...
export const FUNCTION_USAGE_HINT = [
  ...
  '· git_commit_push —— 一键提交并推送（先审计→再 commit→再 push；审计同步拦截，通过后 commit+push 走宿主官方后台 job，立即返回 async:true + jobId，结果完成会自动返回、无需特意查询，可继续干别的）。参数：repo=仓库绝对路径、message=提交信息（必填）、audit、dryRun、push、force、ignorePatterns；调用前逐条核对开发者特殊要求，达标才传 requirementsConfirmed=true。如需主动查结果：job_output <jobId>（job_list/job_kill 同理）。',
  ...
  '· 通用 git 透传（1.9.0 起）：`git-sluice <任意 git 参数>` 或 `git-sluice git <args>` —— 参数与 git 完全一致，凭据自动注入（SSH 私钥 / config.json token），无需传 token 参数；例：git-sluice pull / git-sluice clone owner/repo / git-sluice fetch origin。',
  '【凭据由插件托管，不要到处找凭据】GitHub token 与 SSH 公钥存放在插件 config.json（git-push/config.json 的 githubToken、sshPub 键，0600）；SSH 私钥仍为文件（git-push/id_rsa），由插件的推送/校验流程自动读取与选择通道（默认 SSH，token 401 回退 SSH）。',
...
export function formatAuditBlock(r = {}) {
  ...
  if (quality.emptyResult) {
    L.push(`评分：未评分（${quality.emptyReason || '未扫描到任何文件'}）`);
  } else {
    L.push(`评分 ${quality.score ?? '?'}/${quality.level ?? '?'}（满分 100）`);
  }
  ...
  const ds = r.docsScore;
  if (ds && Array.isArray(ds.items)) {
    const total = ds.items.filter((i) => i.hit).reduce((a, i) => a + (Number(i.score) || 0), 0);
    L.push(`文档加分：命中 ${ds.hits?.length ?? 0}/${ds.items.length}（+${total.toFixed(1)}/10）`);
    if (Array.isArray(ds.review) && ds.review.length) L.push(`建议人工复核：${ds.review.join('；')}`);
  }
