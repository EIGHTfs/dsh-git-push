...
const USAGE = '用法: /git-audit [路径] [--full]';
const EXAMPLE = '示例: /git-audit /path/to/repo --full';

/**
 * 解析 /git-audit 后的 rawInput。
 * 路径可带空格（未加引号时，非 flag 的连续 token 拼成路径）。
 * @param {string} raw
 * @returns {{path:string, scope:'diff'|'full', error?:string}}
 */
export function parseGitAuditInput(raw = '') {
  ...
  for (const t of tokens) {
    ...
    if (t.startsWith('--')) return { path: '', scope, error: `未知参数 ${t}。${USAGE}` };
    pathParts.push(t);
  }
  return { path: pathParts.join(' '), scope };
}
