...
const AUDIT_USAGE = '用法: /git-audit [路径] [--full]';
const AUDIT_EXAMPLE = '示例: /git-audit /path/to/repo --full';

/** 兼容旧名（外部/测试可能仍在用）。 */
export function parseGitAuditInput(raw = '') {
  const parsed = parseCommandInput(raw, ['--full'], AUDIT_USAGE);
  if (parsed.error) return { path: '', scope: 'diff', error: parsed.error };
  return { path: parsed.path, scope: parsed.opts.full ? 'full' : 'diff' };
}
...
export async function runGitAuditCommand({ rawInput = '', invocation = null, env = {}, cfg = {}, log = null } = {}) {
  ...
  if (!repo) {
    return rejectRepo(AUDIT_USAGE, AUDIT_EXAMPLE, `不是 git 仓库: ${target}。拒绝全量扫非仓库目录（会把进程打爆）。`);
  }
