...
const GIT_CORE_TOOLS = [
  ...
  { name: 'code_audit',
    description: '审计指定 git 仓库（L0 静态检查 + 10 维度质量评分）：语法/敏感信息/凭据/大文件/文档措辞 + 代码质量。'
      ...
      + 'history=true 走**历史提交审计**：遍历起始~结束提交（since/until，皆缺省=全部历史、含两端）的代码快照逐提交全量审计，'
      + '每提交一份审计清单落盘到 outDir（缺省=git 根 audit-history/）；后台 job 串行执行、立即返回 jobId（无宿主 jobs 时同步降级）。'
    ...
    parameters: {
      ...
      history: { type: 'boolean', description: 'true=历史提交审计（逐提交快照全量 + 落盘报告，后台 job 串行）' },
      since: { type: 'string', description: 'history 起始提交（sha/ref；缺省=最早提交）' },
      until: { type: 'string', description: 'history 结束提交（sha/ref；缺省=HEAD）' },
      outDir: { type: 'string', description: 'history 报告保存目录（缺省=git 根 audit-history/）' },
    } },
];
...
const EXTRA_TOOLS = [
  ...
  { name: 'functions_index',
    description: '函数索引与函数文档（复用 scripts/func-index.js + functions-doc.mjs）：'
      + 'op=analyze 扫描目录全部函数生成 functions-index.json（name/kind/行/signature/comment 槽位，ESM export 支持）；'
      + 'op=apply 读 JSON（人工补 comment 后）生成 docs/函数/*.md，源文件删除归档 _archived（人工注释不丢）；skipEmpty=跳过无注释函数。',
    parameters: {
      op: { type: 'string', description: "analyze=生成函数索引 JSON | apply=生成函数文档 md（需先 analyze + 人工补 comment）" },
      target: { type: 'string', description: 'analyze 扫描目录（缺省 cwd）' },
      skipEmpty: { type: 'boolean', description: 'apply 时跳过无注释函数（缺省 false=全部列出待补）' },
    } },
  ...
  { name: 'git_cred_env',
    description: '输出插件保管凭据的「环境变量前缀」供 AI 执行任意外部 git 命令时粘贴使用（凭据传递，AI 不直接读明文）：'
      + 'SSH 通道 GIT_SSH_COMMAND 指向插件私钥路径（有私钥时）；HTTPS 通道 GIT_ASKPASS 指向生成的 askpass 脚本（从 config.json 读 token 回显，有 token 时）。'
      + '返回 provided/hasSshKey/hasToken/ssh/https 各通道 envPrefix+example——全程只含路径与命令串，不含 token/私钥明文。',
    parameters: {} },
