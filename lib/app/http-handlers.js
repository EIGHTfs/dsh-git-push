...
import {
  ...
  cloneLog, cloneLogs, abortCloneJob,
...
import {
  ...
  previewClone,
  githubFetch, parseGithubOwnerRepo, detectRepoVisibility, describeRepo, liveRemoteHead, liveRemoteHeadAsync, runGit, readSshPub,
  writeAccountStatusFromResult, accountStatusFile, readAccountStatus, refreshAccountStatus, setVisibility,
} from '../git/index.js';
import { readRepoIndexMap, indexEntryForRepo, maintainRepoIndex, mergeCloudReposIntoIndex, updateRepoRemoteStateInIndex, updateRepoIndex } from '../git/repo-index.js';
...
// 2026-09-19 调优：原 6s。此时探测已从「同步串行」改为「异步并发 + SSH 连接复用」，
//   实测 6 仓冷启动 6.0s / 热启动更短（原串行 28.8s），单仓首连约 4.2s。
//   6s 预算会在并发刚铺开时就把后续仓库判超时，恰好复现「除首个外全显示未探测」的现象，
//   故放宽到 15s——它是**上界**而非预期耗时：正常情况全部探完即返回，不会等满。
const LIST_LIVE_BUDGET_MS = 15_000;
// 2026-09-19：远端探测并发度。SSH 握手已由 transport 的连接复用（ControlMaster）降为一次，
//   并发主要用于掩盖单次 ls-remote 的往返延迟；实测 4 仓并发 5.1s（串行复用 9.2s）。
//   取 4 与 SSH 默认 MaxSessions 兼容，且不超过 GitHub 的连接容忍度。
...
export function listRuleSlots(order, disabledSlots = [], hitStats = null) {
  ...
  for (const slot of discovered) {
    ...
    if (r.ok && r.data?.metadata) {
      ...
      // 2026-09-18：清单驱动槽位（private）也要计入。audit-rules-private.yml 用
      //   `private_files:` 清单而非 `rules:` 声明（rules 恒为 []），只数 rules.length 会
      //   把它显示成「0 条规则」——而它实际有 13 条文件名匹配清单且在正常工作
      //   （实测公仓 → 3 个私密文件全报 blocker）。这类槽位按清单条目数计。
      //   归入「拦截级」：该槽位默认语义就是拦截（公仓命中即 blocker 阻断提交）。
...
export async function handleHttp(req = {}, env = {}, cfg = defaultConfig()) {
  ...
  switch (path) {
    ...
    case '/api/git-push/repos-local': {
      ...
      // 2026-09-20：重建统一走 updateRepoIndex（mode='rebuild'，读-改-写合并不全量覆盖），
      //   返回 indexUpdated 供 UI 本地仓库列表据此重新读取。
      let rebuildIndexUpdated = false;
      if (rebuild) {
        // 离线重建：author=离线账号 owner → 索引存 author 一致的仓库（含本地新增）；offline=true 不联网
      ...
      if (indexMap) {
        ...
        const p = path;
        // 2026-09-16 修复：整列表 live 探测设**总预算**（原整表共享 6s）——逐仓 SSH 真源虽准，
        //   但多仓串行（每仓最多 8s）会让「打开本地面板」卡住数分钟（实测 73s）。
        // 2026-09-19 调优：探测已改为「异步并发 + SSH 连接复用」，总耗时不再随仓数线性增长
        //   （实测 6 仓 6.0s）。原先的「全局 SSH 熔断」（一仓失败就放弃全部剩余）随之移除——
        //   并发下所有仓同时开跑，熔断已无意义；改为**逐仓独立判定**（各自的探测失败只影响
        //   自己，标 liveSkipped），单仓超时由 liveRemoteHeadAsync 的 8s 内建超时兜底。
        ...
        list = Object.keys(indexMap).map((name) => {
          ...
          if (exists) {
            try {
              ...
              // 2026-09-19 优化：远端探测**移到下面并发段**执行（原为同步串行，多仓叠加必超预算）。
              //   此处只收集本地信息，并记录待探测项。
              out.__needLive = Date.now() < listDeadline;
        ...
        // 2026-09-19 并发探测：原实现把 liveRemoteHead 放在上面的同步 .map 里串行执行——
        //   单仓握手 4~6s，4 仓即 16~24s，必然撞穿 listDeadline 预算并触发熔断，
        //   于是列表里除第一个仓库外全部显示「未探测远端」（实测就是这个现象）。
        //   改为：① 先同步收集本地信息（快）；② 对标记 __needLive 的仓库**并发**探测（限流
        //   PROBE_CONCURRENCY，配合 transport 的 SSH 连接复用，握手只付一次）；③ 合并结果。
        const toProbe = list.filter((r) => r.__needLive && r.path && r.branch);
        if (toProbe.length) {
          const results = new Map(); // name → { live, elapsed }
          let cursor = 0;
          const probeStart = Date.now();
          const workers = Array.from({ length: Math.min(PROBE_CONCURRENCY, toProbe.length) }, async () => {
            while (cursor < toProbe.length) {
              const item = toProbe[cursor]; cursor += 1;
              // 单仓仍受总预算约束：超时就跳过（标 liveSkipped），不再拖垮整表
              if (Date.now() > probeStart + LIST_LIVE_BUDGET_MS) { results.set(item.name, { live: { ok: false, sha: '' }, elapsed: -1 }); continue; }
              const t0 = Date.now();
              let live = { ok: false, sha: '' };
              // 必须用**异步版**：同步版 runGit 阻塞事件循环，Promise worker 会退化成串行
              //   （实测 6 仓 13s；改用异步后 6.0s）。
              try { live = await liveRemoteHeadAsync({ repoPath: item.path, branch: item.branch || '' }); } catch { /* 忽略 */ }
              results.set(item.name, { live, elapsed: Date.now() - t0 });
            }
          });
          ...
          for (const r of list) {
            ...
            if (!hit) { if (!r.hasRemote) { /* 无远端：保持空态 */ } continue; }
            const { live } = hit;
            if (live.ok && live.sha) {
              ...
              if (rc.ok) {
                ...
                r.ahead = null; r.behind = null; r.remoteHead = ''; // 本地无该 sha（缓存过期/未 fetch）
              }
          ...
          // 未做任何探测（熔断/超预算/无候选）→ 有远端的仓库统一标 liveSkipped（不用过期引用冒充）
          for (const r of list) {
            delete r.__needLive;
    ...
    case '/api/git-push/repos-local-refresh': {
      ...
      for (const t of targets) {
        ...
        try {
          ...
          // 追加写回索引（只更新该条目，不重建/不重扫；2026-09-20 统一走 updateRepoIndex）
    ...
    case '/api/git-push/repos-cloud': {
      ...
      if (r && r.ok && Array.isArray(r.repos)) {
        ...
        // 2026-09-20：统一走 updateRepoIndex（mode='merge-cloud'，返回 indexUpdated）
        const acct = readAccountStatus({ workspaceRoot: env.workspaceRoot });
        const cloudOwner = (acct && acct.username) || String(r.repos[0]?.fullName || '').split('/')[0] || 'EIGHTfs';
        const merged = updateRepoIndex({ workspaceRoot: env.workspaceRoot, owner: cloudOwner, cloudRepos: r.repos, mode: 'merge-cloud' });
        if (merged.ok) indexUpdated = merged.updated || 0;
      }
      return { status: 200, body: { ...r, indexUpdated } };
    }
    case '/api/git-push/repo-visibility': {
      // 云端仓库私有/公开切换（1.5.4 设置侧边栏「仓库可见性」配套）。
      //   POST body: { owner, repo, visibility, confirm:true }——confirm 由
      //   writeConfirmOps 前置校验（破坏性写），改 public 有敏感信息暴露风险。
      //   GET 只读：?owner=&repo= 云端查询当前状态（「云端获取之后可切换」的前置展示，
      //   列表态走 repos-cloud；单查用 GET 免 Origin/confirm）。
    ...
    case '/api/git-push/repo-push': {
      ...
      if (r.ok) {
        // 2026-09-19 修复：推送成功后**精确回写该仓库的远端状态**。
        //   此前只调 maintainRepoIndex（全量重建），而它**不写** remoteHead/remoteHeadAt/
        //   remoteStateAt（那是 updateRepoRemoteStateInIndex 的职责），visibility 又依赖
        //   token 有效性 → 推送完索引里这些字段长期停在上一轮扫描的旧值，
        //   表现为「刚推完却显示旧的远端 HEAD / 可见性仍是未知」。
        //   现在推送成功后直接写该条目：远端 HEAD = 刚推上去的 commit，
        //   ahead/behind 归零（刚推完必然同步），并补查一次可见性真值。
        const repoName = String(path).split('/').filter(Boolean).pop() || '';
        // 2026-09-19 修正：两个写索引动作必须**串行**——updateRepoRemoteStateInIndex 是
        //   「读-改-写单条目」，maintainRepoIndex 是「全量重建后整文件覆盖」；并发跑会让
        //   后完成者盖掉先完成者（实测把刚写好的条目冲掉）。改为顺序执行，且全量重建
        //   放在最后（它是权威快照，精确回写的增量应在其后落定）。
        // 另一处修正：**SSH 通道的 push 结果里没有 commitSha**（只有 API 通道有，见
        //   transport.js 两处 return），此前 `r.push?.commitSha || ''` 在 SSH 推送后取到空串，
        //   会把索引里已有的 remoteHead 抹成空。现改为：拿不到 commitSha 时回退「推送后
        //   本地 HEAD」（推送成功即远端 HEAD == 本地 HEAD），仍拿不到才不写该字段。
        (async () => {
          // ① 先做全量重建（登记同步：新仓库/改名；2026-09-20 统一走 updateRepoIndex，
          //    读-改-写合并不全量覆盖，返回 indexUpdated 供 UI 重读）
          ...
          const headSha = String(r.push?.commitSha || '').slice(0, 40) || localHead;
          updateRepoIndex({
            workspaceRoot: env.workspaceRoot,
            repoName,
            remoteState: {
      ...
      // 2026-09-20：推送成功触发 UI 本地仓库列表重读（indexUpdated=true——后端异步写索引，
      //   UI 收到后即调 /repos-local 重读；即使读到异步更新前的旧值，后续 refresh 端点仍可对齐）
      return { status: r.ok ? 200 : 400, body: { ...r, error, repoIndex: r.ok ? 'updating' : undefined, indexUpdated: r.ok === true } };
    }
    case '/api/git-push/repo-commit': {
      // 2026-09-21：UI 手动提交（只 commit 不 push）——复用 git_commit_push 的本地提交审计
      //   commitWithAudit（审计门禁同步拦截：blocker → blocked 不提交），返回**精简**结果：
      //   blocker 列表 / warning 计数与样例、commitSha、summary——前端直接展示「拦截/警告」。
      //   requirementsConfirmed=true：UI 为人工操作（非 AI 调用），开发者要求门禁视为已满足。
      const path = String(body.path || '').trim();
      ...
      const r = await commitWithAudit({
        ...
        message,
        push: false,
        audit: true, // UI 提交复用提交推送的审计门禁——必须审计（不随配置关闭）
      ...
      if (!r.ok) {
        ...
        return {
          status: 200, // 业务结果（拦截/失败）由 body.blocked/ok 表达，HTTP 200 便于前端统一处理
          body: {
            ok: false,
            blocked: !!r.blocked,
            error: r.error || '提交失败',
    ...
    case '/api/git-push/repo-clone': {
      ...
      // 互斥：已有 clone 在跑时**拒绝**而不是覆盖。
      //   2026-09-18 实测教训：原先 startCloneJob 直接覆盖 current，两次 clone 可并存，
      //   第二次清理目录时第一次仍在写 -> rmdir ENOTEMPTY（「创建目录失败」），
      //   且第一次无人中止，变成孤儿继续下载（任务已 done、字节数仍在涨）。
      ...
      // 2026-09-19：**改为后台 job 运行**——此前这里 await cloneViaApi，整个 HTTP 请求
      //   被克隆全程阻塞（前端只能靠 30 分钟超时硬撑，中途刷新页面即失去任务）。
      //   现在：预检 + 互斥占位仍同步做（失败要立刻如实回错），真正下载丢后台跑，
      //   端点立即返回 202 {async:true}；前端拿 jobId 后由既有 1s 轮询
      //   /clone-progress 驱动，直到 state 变 done 取终态。
      //   终态由 finishCloneJob 落进 lastDone，故**页面刷新后仍能取到**
      //   （进程重启才会丢，这是 clone-jobs.js 既定的内存态语义）。
      const jobId = 'clone-' + Date.now().toString(36);
      (async () => {
        let r;
        try {
          r = await cloneViaApi({
          ...
          // cloneViaApi 自身抛错也必须落终态，否则 current 不释放、
          //   后续所有 clone 都会被互斥判为 busy（永久卡死）
        ...
        // 后台协程**绝不允许**逃逸未捕获异常：那会同时绕过 finishCloneJob，
        //   使 current 永不释放 → 之后每次 clone 都被互斥判 busy（永久卡死）。
    ...
    case '/api/git-push/clone-logs': {
      // 排查用：返回 clone 的结构化日志（最近 200 条）
      const limit = Number.isFinite(body.limit) ? body.limit : 200;
      return { status: 200, body: { ok: true, logs: cloneLogs(limit) } };
    }
    case '/api/git-push/clone-abort': {
      // 手动中止当前 clone（前端「取消」按钮 / 排查时用）
    ...
    case '/api/git-push/account-check': {
      ...
      //   推送/提交同步刷新，设置侧边栏打开时读取。2026-09-20：返回 statusUpdated
      //   供 UI 账号信息据此重新读取（refreshAccount 离线读回）。
