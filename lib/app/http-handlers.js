...
import {
  ...
  githubFetch, parseGithubOwnerRepo, describeRepo,
} from '../git/index.js';
...
export async function handleHttp(req = {}, env = {}, cfg = defaultConfig()) {
  ...
  switch (path) {
    ...
    case '/api/git-push/repos-local': {
      // 账号卡片「本地」面板（2026-09-15 重构：只读 dsh-repo-index.json，不再实时扫目录）：
      //   - ?rebuild=1：扫描 = 重建索引（buildRepoIndex，只存与登录账号一致的条目）后返回索引内容
      ...
      // 登录账号：token 有效时取 /user.login；无 token → 降级不按账号过滤（只读索引原文）
      let owner = '';
      if (token) {
        try {
      ...
      if (rebuild) {
        // 扫描 = 重建索引：maintainRepoIndex（owner 传登录账号 → 索引只存与登录账号一致条目）
        const r = await maintainRepoIndex({ workspaceRoot: env.workspaceRoot, token, owner: owner || 'EIGHTfs', extraRepos: env.extraRepos, extraReposFile: env.extraReposFile });
