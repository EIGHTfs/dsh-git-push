...
import { githubFetch, parseGithubOwnerRepo } from '../git/index.js';
import { getDefaultScanRoot } from './scan-root.js';
import { getTask, listTasks, submitTask } from '../backend/task-queue.js';
...
export async function handleHttp(req = {}, env = {}, cfg = defaultConfig()) {
  ...
  // 2026-09-14 后台任务查询：GET /api/git-push/task/<id>（单查）/ /api/git-push/tasks（列全部）
  if (path.startsWith('/api/git-push/task/')) {
    const t = getTask(path.slice('/api/git-push/task/'.length));
    if (!t) return { status: 404, body: { ok: false, error: '未找到任务' } };
    const done = t.status === 'done' || t.status === 'error';
    return { status: 200, body: { ok: true, taskId: t.id, title: t.title || '', status: t.status, pending: t.status === 'pending', running: t.status === 'running', done, ...(done ? { result: t.result, error: t.error || undefined } : {}), createdAt: t.createdAt, finishedAt: t.finishedAt } };
  }
  if (path === '/api/git-push/tasks') {
    return { status: 200, body: { ok: true, count: listTasks().length, tasks: listTasks().map((t) => ({ taskId: t.id, title: t.title || '', status: t.status, createdAt: t.createdAt, finishedAt: t.finishedAt, done: t.status === 'done' || t.status === 'error' })) } };
  }
  switch (path) {
    ...
    case '/api/git-push/repo-push': {
      ...
      if (r.ok) {
        ...
        }).catch(() => {}); // fire-and-forget，永不让推送响应失败
      }
      return { status: r.ok ? 200 : 400, body: { ...r, repoIndex: r.ok ? 'updating' : undefined } };
    }
