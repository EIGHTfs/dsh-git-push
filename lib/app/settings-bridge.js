...
export function applySettingsToCfg(cfg, patch) {
  ...
  // 2026-09-18：clone 体积守卫与并发数（同上——UI 能存、config.json 落盘，缺这行则重启丢失）
  if (typeof patch.maxCloneFileMB === 'number' && Number.isFinite(patch.maxCloneFileMB) && patch.maxCloneFileMB >= 0) cfg.maxCloneFileMB = patch.maxCloneFileMB;
  if (typeof patch.cloneConcurrency === 'number' && Number.isFinite(patch.cloneConcurrency) && patch.cloneConcurrency >= 1) cfg.cloneConcurrency = Math.min(16, Math.floor(patch.cloneConcurrency));
  if (typeof patch.pushMethod === 'string' && ['ssh', 'api', 'auto'].includes(patch.pushMethod)) cfg.pushMethod = patch.pushMethod;
  // 2026-09-17：推送门禁开关（settings-set 白名单接受、config.json 落盘、重启回读）
  if (typeof patch.pushGate === 'boolean') cfg.pushGate = patch.pushGate;
  // 2026-09-16：commitMessage/defaultScanRoot 设置已移除（不再映射 cfg）——默认扫描路径复用
  //   本地仓库列表选择路径，提交信息留空由调用方生成。
  return out;
}
