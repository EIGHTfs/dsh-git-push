  ...
  try {
    ctx.inject(['settings'], (settingsCtx) => {
      ...
      scope.watch(async (next) => {
        ...
        // 设置页改动 → 同步运行期字段（即时生效）
        // 2026-09-13：注入总开关（原 v1 的 injectFullSkill / injectRepoIndexFull 已废弃移除）；
        //   切换总开关时清环境注入缓存，下次读取按新开关重算（开→探测注入，关→空段）
        if (typeof next.injectSystemPrompt === 'boolean') {
          cfg.injectSystemPrompt = next.injectSystemPrompt;
          envInjectCache = null;
        }
