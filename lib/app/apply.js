...
const ORDER_ENV = 980;          // 运行环境（工作区根 + 目录清单）
...
export async function apply(ctx, config = {}) {
  ...
  const sectionCount = registerContext(ctx, {
    sections: [
      ...
      { name: 'dsh-git-push-usage', order: ORDER_USAGE, text: () => (cfg.injectSystemPrompt ? FUNCTION_USAGE_HINT : '') },
      { name: 'dsh-git-push-env', order: ORDER_ENV, text: envInjectText },
