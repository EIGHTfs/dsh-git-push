 ...
 *
 * 只读数据层 + 查看器页面渲染。整合自 git-commits-viewer（已下线），
 * 逐条避开旧实现的缺陷：
...
/** 页脚 note 各段拼装（root / depth / extraRepos / extraReposFile）。 */
function buildNoteParts(dict, { workspaceRoot, depth, extraRepos, extraReposFile }) {
  return [
...
/** 注入 window.__VIEWER_I18N 的 JSON 文本（< 转义避免提前闭合 script）。 */
function buildViewerI18nJson(lang) {
  return JSON.stringify({ locale: lang, locales: VIEWER_LOCALES }).replace(/</g, '\\u003c');
}
...
export function renderViewerPage({ workspaceRoot = '', depth = 3, extraRepos = [], extraReposFile = '', version = '', locale } = {}) {
  ...
  const noteParts = buildNoteParts(dict, { workspaceRoot, depth, extraRepos, extraReposFile });
  const i18nJson = buildViewerI18nJson(lang);
  return `<!DOCTYPE html>
