 ...
 * 权重沿用 10 维度表（合计 100）。
 *
 * dsh-skip-quality：本文件是**配置数据表**（维度权重表 DEFAULT_WEIGHTS、对数衰减系数表
 *   DIMENSION_K）——表内数字是「维度名 → 参数」映射值，键名已表意，拆成 WEIGHT_X = 15
 *   这类常量反而割裂表格、降低可读性；权重本身由侧边栏配置项可调（非散落魔数），
 *   故整文件豁免质量类魔数告警。逻辑类魔数（超时/上限）在其他文件按需提取命名常量。
 */
