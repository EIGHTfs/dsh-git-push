...
export function countByDimension(findings = []) {
  ...
  // 2026-09-23 公平性修复（大项目小问题线性累加不公平——小问题不会同时出现）：
  //   warning/info 类**每文件每规则每维度最多计 1 次**——一个文件 20 处短变量 = 该文件 1 个
  //   「命名可读性」问题（修一处即覆盖全消），不随同规则条数线性累加；不同规则各计
  //   （同文件 20 个不同规则问题 = 20），与大项目文件数解耦。
  //   blocker（安全/凭据等硬问题）保持每次计 2——同文件多个凭据 = 多个致命风险，不合并。
  const seenRule = new Map(); // `${rule}|${dim}` → Set(file)；仅 warning/info 参与去重
  for (const f of findings) {
    ...
    const sev = f.severity === 'blocker' ? 2 : 1;
    const dedup = f.severity !== 'blocker';
    const key = f.file || '__nofile__';
    const rulePrefix = `${f.rule || '?'}|`;
    for (const dim of f.dimensions || []) {
      if (!(dim in counts)) continue;
      if (dedup) {
        const sKey = rulePrefix + dim;
        if (!seenRule.has(sKey)) seenRule.set(sKey, new Set());
        const fileSet = seenRule.get(sKey);
