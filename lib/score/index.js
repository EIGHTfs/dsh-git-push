...
export function countByDimension(findings = []) {
  ...
  //   warning/info 类**每文件每维度最多计 1 次**——一个文件有 20 处短变量 = 该文件 1 个
  //   「命名可读性」问题（修一处即覆盖全消），不随问题条数线性累加；与大项目文件数解耦，
  //   大项目不再因孤立小问题多被裸计数压低评分。
  //   blocker（安全/凭据等硬问题）保持每次计 2——同文件多个凭据 = 多个致命风险，不合并。
  const seenFile = new Map(); // dim → Set(file)；仅 warning/info 参与去重
  for (const f of findings) {
    ...
    for (const dim of f.dimensions || []) {
      ...
      if (dedup) {
        if (!seenFile.has(dim)) seenFile.set(dim, new Set());
        const fileSet = seenFile.get(dim);
