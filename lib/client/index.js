...
export const DEFAULT_CONFIG = {
  ...
  injectSystemPrompt: true, // 注入总开关：系统提示词（用法/提醒）+ 上下文（环境信息；默认开）
  auditScanScope: 'diff',   // diff | full
  maxScanFiles: 3000,        // 全量审计文件数上限（0=不限）；超限按变动优先截断，弱机防卡死
  maxCloneFileMB: 10,        // clone 单文件体积上限(MB)；超过则跳过并如实告知（0=不限制）
  cloneConcurrency: 6,       // clone 并发下载数；1=串行最慢，过高会撞 GitHub 风控
...
export const SETTINGS_SCHEMA = [
  ...
  { key: 'injectSystemPrompt', type: 'boolean', default: true, label: '注入系统提示词/上下文', hint: '系统提示词：插件功能用法/README 提醒/要求清单（每步）；上下文：工作区目录 + 工具安装路径 + skill 总入口（每个 agent 首次 step 注入一次；工具按 lib/tool-probes.json 模板探测，路径生成到 <配置目录>/tools.json）。关=完全不注入。修改即时保存' },
  ...
  { key: 'maxScanFiles', type: 'number', default: 3000, label: '全量扫描文件上限', hint: '全量审计最多扫多少个文件（0=不限）；超限时按「变动文件优先」截断，避免弱机扫几万文件卡死' },
    { key: 'maxCloneFileMB', type: 'number', default: 10, label: 'clone 单文件上限(MB)', hint: '克隆时跳过超过该大小的文件并在结果中列出（0=不限制）。仓库里的二进制/媒体大文件常达几十 MB，逐个下载会把请求拖到超时' },
    { key: 'cloneConcurrency', type: 'number', default: 6, label: 'clone 并发数', hint: '同时下载的文件数。1=串行（最慢但最温和）；过高会触发 GitHub 风控。受限网络建议 3~6' },
