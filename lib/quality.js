...
/** 内置默认评分权重（yaml scoring.dimensions_weight 降级兜底；v1.59.0 定稿 10 维度合并版，合计 100） */
...
export function scoreQuality(counts = {}, opts = {}) {
  const weights = { ...(opts.weights || DEFAULT_WEIGHTS), ...(opts.qualityWeights || {}) };
  const levels = opts.levels || {};
  // 各维度 0-10 分：问题越多越低
  const dims = {};
  const clamp = (v) => Math.max(0, Math.min(10, v));
  // 可读性：每 1 个大函数 -2 分（v1.60.0：counts.readability 已是分级加权扣分点——warning 0.5/blocker 2，此处直接扣不再乘）
  dims['可读性'] = clamp(10 - (counts.readability || 0));
  // 可维护性：静态难判，给中间值 8（若有大函数/重复则降；加权扣分点同样直接扣）
  dims['可维护性'] = clamp(8 - (counts.readability || 0) * 0.5);
  ...
  // 附加维度：静态难判，给中间值（与可观测性/可部署性/文档/DX 同策略；新增覆盖内容
  // 已并入主维度——API设计/代码异味→可维护性、错误处理/边界/并发→健壮性、日志→可观测性、
  // 配置管理→可部署性、国际化→文档，权重与分值都走主维度）
  dims['可观测性'] = 7;
  dims['可部署性'] = 7;
  dims['文档'] = 7;
  dims['开发者体验'] = 7;
