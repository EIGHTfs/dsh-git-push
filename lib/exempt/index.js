 ...
 * 场景对照（12 场景，见 docs/WORKBOARD-v2.md §3.6）：
...
export const EXEMPT_MARKERS = {
  'dsh-skip-sensitive': {
    ...
    // regex 宽声明：security/* 规则带 pattern 字段编译成 regex kind，
  ...
  'dsh-skip-quality': {
    ...
    // magic-number-smart：配置数据表（如 lib/score/index.js 的权重表/衰减系数表）
    //   数字是「键名 → 参数」映射值，键名已表意，拆成 WEIGHT_X = 15 反而割裂表格。
    //   该 kind 此前不在任何豁免清单里，导致文件头标记**形同虚设**——
    //   lib/score/index.js 从 v1.3.1 起就写着豁免理由却一直被报 7 条魔数；
    //   而规则自身的 exemptHint 又建议用 dsh-skip-quality 豁免，提示与实际行为不符。
    blocked: ['func-lines', 'empty-catch', 'sync-fs', 'magic-number-smart'],
    lineLevel: false,
    hint: '文件头=整文件免质量评分（func-lines/empty-catch/sync-fs/magic-number-smart）',
  },
  'dsh-skip-residue': {
    dimensions: ['可读性', '可维护性'],
