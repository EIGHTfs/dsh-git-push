...
export function ensureSensitiveIgnored(repoPath, { skipWrite = false } = {}) {
  ...
  if (hits.length && !skipWrite) {
    ...
    for (const h of hits) {
      const line = h.path;
      // .samples 目录豁免：该目录内敏感文件照常报告（hits 保留），但不写 .gitignore、不解除跟踪
      if (isSampleExemptDir(repoPath, line)) { sampleExempted++; continue; }
  ...
  return { ok: true, added, hits, tracked, unstaged, sampleExempted, skipped: skipWrite ? 'private-repo-exempt' : undefined };
}
