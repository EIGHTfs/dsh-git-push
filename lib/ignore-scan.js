...
// v1.60.0：豁免体系分维度扩展——每种拦截类型独立豁免标记（继承 sensitive 的「位置语义」：
// 文件头 = 整个文件豁免；对应位置注释 = 豁免单个位置）。
// 通用工厂 makeExemptors(marker)：返回 { hasLine, hasHeader } 两个检测函数，
// 新增拦截类型豁免只需一行注册，不再逐类型手写 head/line 两函数。

/** 通用豁免检测工厂：marker 为豁免标记字符串（如 'dsh-skip-size'），大小写不敏感 */
function makeExemptors(marker) {
  const re = new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
  return {
    /** 行内是否带该豁免标记（同行出现即豁免该位置） */
    hasLine: (line) => re.test(String(line || '')),
    /** 文件头（前 3 行）是否声明整文件豁免 */
    hasHeader: (text) => re.test(String(text || '').split('\n').slice(0, 3).join('\n')),
  };
}

/** 敏感内容豁免（secret/凭据/隐私措辞）：dsh-skip-sensitive（v1.47.0 起，行尾=单行 / 文件头=整文件） */
...
/** 文件大小豁免：dsh-skip-size（只能写文件头——文件大小是整文件属性，位置级无意义） */
export const SIZE_EXEMPT_MARKER = 'dsh-skip-size';
export const sizeExempt = makeExemptors(SIZE_EXEMPT_MARKER);

/** 函数长度豁免：dsh-skip-func-length（函数定义行尾注释=豁免该函数；文件头=整文件函数豁免） */
export const FUNC_LENGTH_EXEMPT_MARKER = 'dsh-skip-func-length';
export const funcLengthExempt = makeExemptors(FUNC_LENGTH_EXEMPT_MARKER);

/** 语法解析豁免（syntax/json/yaml）：dsh-skip-syntax（只能文件头——解析是文件级） */
export const SYNTAX_EXEMPT_MARKER = 'dsh-skip-syntax';
export const syntaxExempt = makeExemptors(SYNTAX_EXEMPT_MARKER);

/** 质量评分豁免（func-lines/silent-catch/sync-in-async/file-score/no-tests）：dsh-skip-quality（文件头=整文件质量维度豁免） */
export const QUALITY_EXEMPT_MARKER = 'dsh-skip-quality';
export const qualityExempt = makeExemptors(QUALITY_EXEMPT_MARKER);

/** 残留类豁免（debugger/todo/console）：dsh-skip-residue（行尾=单行 / 文件头=整文件） */
export const RESIDUE_EXEMPT_MARKER = 'dsh-skip-residue';
export const residueExempt = makeExemptors(RESIDUE_EXEMPT_MARKER);

/** 规则引擎 styleRules 全豁免：dsh-skip-style（文件头=整文件跳过全部 styleRules） */
export const STYLE_EXEMPT_MARKER = 'dsh-skip-style';
export const styleExempt = makeExemptors(STYLE_EXEMPT_MARKER);
 ...
 * 专为样本/测试目录设计（故意构造坏样本时不被门禁挡住推送）。与 v2 lib/exempt 对齐。
 */
...
/**
 * 扫描敏感字段并报告命中文件（2026-09-12 用户指令：扫描到敏感文件不改动 git 忽略配置）。
 * 不再自动追加 .gitignore、不再 git rm --cached 解除跟踪——由仓库方自行决定处理；
 * 基线（node_modules 等 ensureNpmIgnored）与自定义忽略（ensureCustomIgnored）照常。
 * @param {string} repoPath 仓库绝对路径
 * @param {{skipWrite?: boolean}} [opts] 兼容保留（历史调用方传 skipWrite 语义不变：只报告不写）
 * 返回 { ok, added: string[], hits, tracked: string[], unstaged: string[], skipped?: string }。
 *   added/tracked/unstaged 恒空（不再自动写盘/解除跟踪）。
 */

export function ensureSensitiveIgnored(repoPath, { skipWrite = false } = {}) {
  const hits = scanSensitiveFiles(repoPath);
  // 2026-09-12 用户指令「扫描到不动 git 忽略」：敏感文件只报告（hits），
  // 不自动追加 .gitignore、不 git rm --cached 解除跟踪（旧自动写盘逻辑已移除）。
  return { ok: true, added: [], hits, tracked: [], unstaged: [], sampleExempted: 0, skipped: skipWrite ? 'private-repo-exempt' : undefined };
}
