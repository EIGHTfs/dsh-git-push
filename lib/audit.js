 ...
 * 思路：密钥类硬编码已由 SECRET_PATTERNS 拦；本规则补 no-hardcode-config /
 ...
 */
function auditFile(repoPath, file, { exempt = false, commentWordingPatterns, hardcodeFullScan = false, quality = QUALITY_DEFAULTS } = {}) {
  const findings = [];
  ...
  if (!skipSensitive && !file.isBinary && DOC_EXTS.has(ext)) {
    file.addedLines.forEach((line, idx) => {
      ...
      for (const p of CREDENTIAL_REF_PATTERNS) {
        if (p.re.test(line)) {
          findings.push({ rule: 'credential-ref', level: 'warning', file: file.path, line: lineNumber(file, idx), message: `文档含凭据引用或明文（${p.name}）：禁止在文档写凭据引用/明文，凭据统一存插件配置目录（DSH_HOME/git-push/，0600 权限），需要时按需读取` });
          break;
        }
      }
    });
 ...
 */
export function auditRepo(repoPath, { blockOn = 'any', files, exemptRepos = [], commentWordingPatterns, hardcodeFullScan = false, quality = QUALITY_DEFAULTS } = {}) {
  ...
  for (const f of fileList) {
    findings.push(...auditFile(repoPath, f, { exempt, commentWordingPatterns, hardcodeFullScan, quality }));
  }
  const summary = {
  ...
  // v1.39.0：代码质量评分（code-quality-checklist.yaml 维度权重）
