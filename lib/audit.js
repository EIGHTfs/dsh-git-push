 ...
 *
 * 规则插件化（v1.41.0，C 组）：规则数据全部来自规则包（缺省 lib/audit-rules/eightfs.rules.json，
...
import { loadBuiltinRulePack, loadRulePackFromFile, compileRulePack, resolveRulesetChoice, stripJsonComments } from './rule-packs.js';
import { checkFunctionLength, checkSilentCatch, checkSyncInAsync, hasTestFiles, scoreQuality, loadQualityYaml, locateQualityYaml, QUALITY_DEFAULTS } from './quality.js';
...
/** 获取当前生效规则集（未初始化时同步装载内置 EIGHTfs 包） */
export function getAuditRuleset() {
  if (!_ruleset) {
    const r = loadBuiltinRulePack();
    _ruleset = compileRulePack(
      r.ok ? r.pack : { name: '(load-failed)', owner: '(unknown)', version: '0', rules: [] },
      { source: r.ok ? 'builtin' : 'builtin-missing' },
    );
    if (!r.ok) _ruleset.errors = [r.error || '内置规则包装载失败'];
  }
  return _ruleset;
}

/** 按配置同步装载并设置生效规则集（'' / 'builtin' / 'eightfs'=内置包；本地路径=file；url 需异步注入） */
export function initAuditRuleset(cfg) {
  const src = resolveRulesetChoice(cfg);
  const r = src.source === 'file'
    ? loadRulePackFromFile(src.path)
    : src.source === 'url'
      ? { ok: false, error: 'url 规则包需异步装载（loadRulePack 后 applyRulePack 注入）' }
      : loadBuiltinRulePack();
  _ruleset = compileRulePack(
    r.ok ? r.pack : { name: '(load-failed)', owner: '(unknown)', version: '0', rules: [] },
    { source: src.source },
  );
  if (!r.ok) _ruleset.errors = [r.error || '规则包装载失败'];
  return _ruleset;
}

/** 注入异步装载完成的规则包（原始 pack 对象或编译结构），热替换生效规则集 */
export function applyRulePack(pack, { source = 'url' } = {}) {
  _ruleset = pack && Array.isArray(pack.rules) ? compileRulePack(pack, { source }) : pack;
  return _ruleset;
}
 ...
 */
function auditFile(repoPath, file, { exempt = false, commentWordingPatterns, hardcodeFullScan = false, quality = QUALITY_DEFAULTS, ruleset } = {}) {
  ...
  // comment-wording / doc-conversation / 代码质量不豁免（用户指示 2：豁免只是豁免隐私入库）。
  ...
  // 3. 语法/解析（blocker，仅文本代码文件；json 容忍 // 注释行 v1.41.0 C5）
  if (!file.isBinary && !isCredFile) {
  ...
  // v1.41.0 规则来自规则包 secret 桶；json // 注释行豁免隐私入库类规则 C5）
  ...
  // 或凭据明文键值对 → 警告，提示改存插件配置目录内 json 按需读取（v1.40.0 原同级仓废除）。
  if (!skipSensitive && !file.isBinary && DOC_EXTS.has(ext)) {
    file.addedLines.forEach((line, idx) => {
  ...
  // 6. 硬编码本机路径 / 局域网 IP
  // 干什么：拦代码字面量与文档里的死路径/死 IP（no-hardcode-config / host-address-convention）。
  // 触发原话：「gitpush增加审计硬编码功能，测试就用插件本身」
  // 思路：私有库豁免不跳过（换机必炸与可见性无关）；文件头 dsh-skip-sensitive 仍跳过（测试夹具用）。
  // 代码/配置 blocker；文档 warning。相对路径、/tmp、127.0.0.1、占位符、示例词不报。
  // v1.36.1：新增 hardcodeFullScan 开关（设置页「硬编码全量扫」）——默认 false 只扫新增行；
  //   勾选后扫整个文件（含既有历史行），用于换机前排查存量死路径。config 按整行扫。
  if (!fileHeaderExempt && !file.isBinary) {
  ...
  // 7. 代码质量维度（v1.39.0，依据 docs/code-quality-checklist.yaml；仅 .js/.mjs/.cjs 变更文件）
  // 只扫新增/变更的源码文件全文（不要求 diff 行，函数体检测需整体上下文）。
  if (quality?.enabled !== false && (ext === 'js' || ext === 'mjs' || ext === 'cjs') && !file.isBinary && !fileHeaderExempt) {
    let text = '';
 ...
 */
export function auditRepo(repoPath, { blockOn = 'any', files, exemptRepos = [], commentWordingPatterns, hardcodeFullScan = false, quality, ruleset } = {}) {
  const rs = ruleset || getAuditRuleset();
  ...
  for (const f of fileList) {
    findings.push(...auditFile(repoPath, f, { exempt, commentWordingPatterns: mergedWording, hardcodeFullScan, quality: qualityEff, ruleset: rs }));
  }
  const summary = {
  ...
  const blocked = blockOn === 'none' ? false : blockOn === 'any' ? findings.length > 0 : summary.blocker > 0;
