...
import { isBinaryOrLarge, getDiff, hasFileHeaderExempt, hasLineExempt, runGit, sizeExempt, syntaxExempt, qualityExempt, residueExempt, styleExempt, funcLengthExempt } from './core.js';
import { isSampleExemptDir } from './ignore-scan.js';
...
function buildAuditContext(repoPath, file, { exempt = false, commentWordingPatterns, hardcodeFullScan = false, quality = QUALITY_DEFAULTS, ruleset, yamlMode = 'js-yaml' } = {}) {
  ...
  // v1.60.0：二进制/大文件也尝试读文件头（dsh-skip-size 正是大文件豁免用途；读失败按普通文件处理）
  let fileHeaderExempt = false;
  let fileText = '';
  try { fileText = readFileSync(fullPath, 'utf8'); } catch { /* 读不到则按普通文件 */ }
  fileHeaderExempt = hasFileHeaderExempt(fileText);
  const skipSensitive = exempt || fileHeaderExempt;
  // v1.60.0：豁免体系分维度——文件头注释声明对应标记 → 整文件跳过对应拦截类型（继承 sensitive 位置语义）
  const exemptFlags = {
    size: sizeExempt.hasHeader(fileText),            // dsh-skip-size → 跳过二进制/大文件检查
    syntax: syntaxExempt.hasHeader(fileText),        // dsh-skip-syntax → 跳过语法/JSON/YAML 解析
    quality: qualityExempt.hasHeader(fileText),      // dsh-skip-quality → 跳过质量评分（func-lines/silent-catch/sync-in-async）
    residue: residueExempt.hasHeader(fileText),      // dsh-skip-residue → 跳过 debugger/todo/console 残留
    style: styleExempt.hasHeader(fileText),          // dsh-skip-style → 跳过全部 styleRules
    funcLength: funcLengthExempt.hasHeader(fileText), // dsh-skip-func-length → 跳过本文件全部函数长度检查
  };
  ...
  return { repoPath, file, rs, findings, fullPath, ext, isCode, fileHeaderExempt, skipSensitive, exemptFlags, isJsonCommentLine, wordingPatterns, isCredFile, hardcodeFullScan, quality, yamlMode };
}
 ...
 */
/** path-regex：目录结构/命名校验——对文件路径本身跑正则，不依赖 styleCode 与新增行。 */
function checkPathRegexRule(s, rule, push) {
  for (const re of rule.patterns) {
    if (re.test(s.file.path)) { push(rule, undefined, '命中 ' + rule.name); break; }
  }
}

/** regex：新增行对 pattern 扫描（exts 目标文件类型过滤；未声明 exts 保持仅 styleCode）。 */
function checkRegexAddedLines(s, rule, push, styleCode) {
  const { file, ext } = s;
...
/** min-length：新增行声明标识符长度下限（const/let/var 名、函数名）。 */
function checkMinLengthRule(s, rule, push, styleCode) {
  if (!styleCode) return;
  for (const [idx, line] of (s.file.addedLines || []).entries()) {
...
/** max-lines：文件级行数（id 含 file）或函数级最大块跨度。 */
function checkMaxLinesRule(s, rule, push, styleCode) {
  if (rule.id && /file/i.test(rule.id)) {
    // 文件级行数
    let lines = 0;
    try { lines = readFileSync(s.fullPath, 'utf8').split('\n').length; } catch { return; }
  ...
  try {
    const text = readFileSync(s.fullPath, 'utf8');
    const fnLen = maxFunctionSpan(text);
    if (fnLen > rule.threshold) push(rule, undefined, `最长函数体约 ${fnLen} 行 > ${rule.threshold}`);
  } catch { /* 读不到跳过 */ }
}

/** max-complexity：新增行复杂度。 */
function checkMaxComplexityRule(s, rule, push, styleCode) {
  if (!styleCode) return;
  for (const [idx, line] of (s.file.addedLines || []).entries()) {
...
/** max-depth：整文件最大缩进层级（简化 AST，v1.56.0）。 */
function checkMaxDepthRule(s, rule, push, styleCode) {
  if (!styleCode) return;
  try {
    const text = readFileSync(s.fullPath, 'utf8');
...
/** min-occurrences：同语句块重复出现次数。 */
function checkMinOccurrencesRule(s, rule, push, styleCode) {
  if (!styleCode) return;
  try {
    const text = readFileSync(s.fullPath, 'utf8');
...
/** styleRules 各 kind 分派（提取为独立函数控制 checkStyleRules 行数）。 */
function dispatchStyleRule(rule, s, push, styleCode) {
  switch (rule.kind) {
    case 'path-regex': checkPathRegexRule(s, rule, push); break;
    case 'regex': checkRegexAddedLines(s, rule, push, styleCode); break;
    case 'min-length': checkMinLengthRule(s, rule, push, styleCode); break;
    case 'max-lines': checkMaxLinesRule(s, rule, push, styleCode); break;
    case 'max-complexity': checkMaxComplexityRule(s, rule, push, styleCode); break;
    case 'max-depth': checkMaxDepthRule(s, rule, push, styleCode); break;
    case 'min-occurrences': checkMinOccurrencesRule(s, rule, push, styleCode); break;
    case 'repeated-string': checkRepeatedString(s, rule, push); break;
  }
}

function checkStyleRules(s) {
  const { file, findings, rs, isCode, ext } = s;
  ...
  // v1.60.0：dsh-skip-style 文件头 → 跳过全部 styleRules（独立于 sensitive 的更精确豁免）
  if (s.skipSensitive || s.exemptFlags?.style) return;
  ...
  for (const rule of rs.styleRules) {
    ...
    dispatchStyleRule(rule, s, push, styleCode);
  }
}
...
function checkNpmBinaryCredentialFile(s) {
  const { file, findings, fullPath, skipSensitive, isCredFile, exemptFlags } = s;
  ...
  // 1. 二进制/大文件（blocker；v1.60.0：dsh-skip-size 文件头豁免）
  if (!exemptFlags.size && (file.isBinary || isBinaryOrLarge(fullPath))) {
...
function checkSyntaxAndParse(s) {
  const { file, findings, fullPath, ext, isCredFile, exemptFlags } = s;
  if (file.isBinary || isCredFile || exemptFlags.syntax) return; // v1.60.0：dsh-skip-syntax 文件头豁免
...
function scanAddedLineRules(s) {
  const { file, findings, rs, isCode, skipSensitive, isJsonCommentLine, exemptFlags } = s;
  file.addedLines.forEach((line, idx) => {
    ...
    if (!isCode || exemptFlags.residue) return; // v1.60.0：dsh-skip-residue 文件头豁免 debugger/todo
...
function countConsoleLogs(s) {
  const { file, findings, isCode, exemptFlags } = s;
  if (!isCode || exemptFlags.residue) return; // v1.60.0：dsh-skip-residue 豁免
...
function checkCodeQuality(s) {
  const { file, findings, ext, fullPath, quality, fileHeaderExempt, exemptFlags } = s;
  if (quality?.enabled === false || fileHeaderExempt || exemptFlags.quality) return; // v1.60.0：dsh-skip-quality 文件头豁免质量评分
  if ((ext !== 'js' && ext !== 'mjs' && ext !== 'cjs') || file.isBinary) return;
  let text = '';
  try { text = readFileSync(fullPath, 'utf8'); } catch { /* 读不到跳过质量规则 */ }
  if (!text) return;
  // 可读性：单函数行数（>50 warning，>100 blocker）；v1.60.0：dsh-skip-func-length 文件头豁免全部函数长度检查
  if (!exemptFlags.funcLength) {
 ...
 */
/** 汇总 findings 并判定 blocked（blockOn：'none' 不拦 / 'any' 有 finding 即拦 / 默认仅 blocker 拦）。 */
function summarizeFindings(findings, blockOn) {
  return {
    blocker: findings.filter((x) => x.level === 'blocker').length,
    warning: findings.filter((x) => x.level === 'warning').length,
    total: findings.length,
    blocked: summaryBlockerBlocks(blockOn, findings),
  };
}

export function auditRepo(repoPath, { blockOn = 'any', files, exemptRepos = [], commentWordingPatterns, hardcodeFullScan = false, quality, qualityWeights, ruleset, visibility = 'unknown', yamlMode = 'js-yaml' } = {}) {
  const rs = ruleset || getAuditRuleset();
  ...
  if (fileList.length === 0) {
    ...
    const summary = summarizeFindings(priv.findings, blockOn);
    return {
      ok: true, repo: repoPath, findings: priv.findings,
      summary: { blocker: summary.blocker, warning: summary.warning, total: summary.total },
      blocked: summary.blocked, passed: !summary.blocked,
      note: '无 diff 变更；存量私密文件检查命中', ruleset: rulesetInfo(rs),
    };
  }
  const findings = [];
  ...
  const summary = summarizeFindings(findings, blockOn);
  // .samples 目录豁免（2026-09-11）：审计照常出结果（findings/summary 全量），
  // 但该目录内 blocker 不算提交拦截 —— 样本/测试目录故意构造坏样本时不被门禁挡住。
  // 豁免作用于全部 blockOn 档位：blocked 判定用「非豁免 findings」子集（any 档含 warning 同理）。
  const nonExempt = findings.filter((x) => !isSampleExemptDir(repoPath, x.file));
  const sampleExemptCount = findings.length - nonExempt.length;
  const blocked = summaryBlockerBlocks(blockOn, nonExempt);
  const res = { ok: true, repo: repoPath, findings, summary: { blocker: summary.blocker, warning: summary.warning, total: summary.total }, blocked, passed: !blocked, blockOn, ruleset: rulesetInfo(rs) };
  if (sampleExemptCount > 0) res.sampleExemptFindings = sampleExemptCount;
  if (exempt) {
    res.exempted = true;
  ...
  if (qualityEff?.enabled !== false) {
    attachQualityScore(res, { repoPath, findings, summary, fileList, qualityWeights });
  }
  return res;
}
 ...
 */
function attachQualityScore(res, { repoPath, findings, summary, fileList, qualityWeights }) {
  const yamlPath = locateQualityYaml(repoPath);
  const qy = loadQualityYaml(yamlPath);
  if (qualityWeights && typeof qualityWeights === 'object') {
    qy.weights = { ...qy.weights, ...qualityWeights };
  }
  const hasSrcChange = fileList.some((f) => /\.(js|mjs|cjs|ts|tsx|jsx)$/.test(f.path || ''));
  const testing = hasTestFiles(repoPath);
  // v1.60.0：质量扣分分级——func-lines 按级别加权（warning 50-100 行 -0.5 / blocker >100 行 -2），
  // 避免「9 个 50 行函数就把可读性扣到 0」；docs/ 参考文件与 test/ 测试代码不计入质量扣分
  // （测试里刻意用同步 IO/长函数是常态，非交付质量债）。
  const isDeliveryCode = (f) => {
    const p = String(f.file || f.path || '');
    return !p.startsWith('docs/') && !p.startsWith('test/');
  };
  const qualityFindings = findings.filter((x) => ['func-lines', 'silent-catch', 'sync-in-async'].includes(x.rule));
  const readabilityScore = qualityFindings
    .filter((x) => x.rule === 'func-lines' && isDeliveryCode(x))
    .reduce((acc, x) => acc + (x.level === 'blocker' ? 2 : 0.5), 0);
  const counts = {
    readability: readabilityScore,
    robustness: qualityFindings.filter((x) => x.rule === 'silent-catch' && isDeliveryCode(x)).length,
    performance: qualityFindings.filter((x) => x.rule === 'sync-in-async' && isDeliveryCode(x)).length,
    testing,
  };
