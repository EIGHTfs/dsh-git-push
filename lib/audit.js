 ...
 */
/**
 * v1.42.0：措辞改写规则表——cleanCommentWording 内嵌 scrub 抽出为模块级数据（行为零变化）。
 * 每个 re 的 replace 回调收到 (match, capture…)；bump 计数器（改写次数），返回值即替换文本。
 */
...
/** v1.42.0：对单段注释执行全部改写规则（含自定义 pattern 补删），返回改写后文本 */
function scrubCommentWording(comment, extraPatterns, bump) {
  let c = comment;
  for (const { re, fn } of WORDING_REWRITES) {
    c = c.replace(re, (...a) => { bump(); return fn(...a); });
  }
  // 7) 自定义措辞（规则注入的新增 pattern，对内置改写未覆盖的再删一次）
  for (const re of extraPatterns) {
    if (re instanceof RegExp) {
      const withColon = new RegExp(re.source + '[\uFF1A:]?', re.flags);
      c = c.replace(withColon, () => { bump(); return ''; });
    }
  }
  return c;
}
...
export function cleanCommentWording(text, extraPatterns = []) {
  ...
  const scrub = (comment) => {
    // v1.42.0：改写规则抽为模块级 WORDING_REWRITES + scrubCommentWording（原内嵌规则原样搬移）
    return scrubCommentWording(comment, extraPatterns, () => { count++; });
  };
...
function buildAuditContext(repoPath, file, { exempt = false, commentWordingPatterns, hardcodeFullScan = false, quality = QUALITY_DEFAULTS, ruleset } = {}) {
  ...
  return { repoPath, file, rs, findings, fullPath, ext, isCode, fileHeaderExempt, skipSensitive, isJsonCommentLine, wordingPatterns, isCredFile, hardcodeFullScan, quality };
}
...
function auditFile(repoPath, file, { exempt = false, commentWordingPatterns, hardcodeFullScan = false, quality = QUALITY_DEFAULTS, ruleset } = {}) {
  ...
  const s = buildAuditContext(repoPath, file, { exempt, commentWordingPatterns, hardcodeFullScan, quality, ruleset });
