...
export function auditRepo(repoPath, { blockOn = 'any', files, exemptRepos = [], commentWordingPatterns, hardcodeFullScan = false, quality, qualityWeights, ruleset, visibility = 'unknown', yamlMode = 'js-yaml' } = {}) {
  ...
  const res = { ok: true, repo: repoPath, findings, summary: { blocker: summary.blocker, warning: summary.warning, total: summary.total }, blocked: summary.blocked, passed: !summary.blocked, blockOn, ruleset: rulesetInfo(rs) };
  if (exempt) {
    res.exempted = true;
