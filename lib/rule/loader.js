...
/**
 * 规则槽位（默认加载顺序）。
 * 0.2.0：只列已建槽位（nodejs 代码规则 / docs 链接与文档规则）；
 * 规划中的槽位（npm/html/frontend/comment/dsh/private/structure/version）建好后追加一行即生效。
 */
export const RULE_SLOTS = ['nodejs', 'docs'];

/** 规划中但未建的槽位（供 discoverRuleSlots 对照，缺失不报错）。 */
export const PLANNED_SLOTS = ['npm', 'html', 'frontend', 'comment', 'dsh', 'private', 'structure', 'version'];
