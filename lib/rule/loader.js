// dsh-skip-sensitive: FORCE_LOAD_FALLBACK 含私密文件清单声明（凭据路径数据定义，非文档引用旧位置）
/**
...
import { normalizeScopeRules } from './scope.js';
...
export function loadYamlRuleFile(slot, dir = RULE_YAML_DIR) {
  ...
  try {
    ...
    return { ok: true, data: parsed || { rules: [] }, file };
  } catch (e) {
 ...
 *      例外：环境变量 DSH_GIT_PUSH_RULE_SLOTS 显式列出该槽位 → 强制加载（显式指定即开）。
...
export function loadRuleFiles(order = null, opts = {}) {
  ...
  // UI 禁用槽位（2026-09-12）：设置页单击禁用 → 装载最前过滤；
  // nodejs/private 属 FORCE_LOAD_SLOTS 安全红线，UI 禁用不生效（后端强制加载）。
  const uiDisabled = new Set(Array.isArray(opts.disabledSlots) ? opts.disabledSlots.filter(Boolean) : []);
  // 2026-09-13 修复 explicit 误伤：显式强制名单**只认环境变量 DSH_GIT_PUSH_RULE_SLOTS**
  // （用户点名要强制开启的槽位），不再把调用方传的 order/settings.auditRuleOrder 当强制名单——
  // 否则 auditRuleOrder 全量 14 个会被当 explicit，yml 顶层 disabled:true 全部失效（i18n 默认关闭被误加载）。
  ...
  for (const slot of slots) {
    // UI 单击禁用优先：该槽位任何规则都不加载（安全红线槽位除外）
    if (uiDisabled.has(slot) && !FORCE_LOAD_SLOTS.has(slot)) continue;
    ...
    if (ruleData?.metadata && !merged.metadata.name) merged.metadata = ruleData.metadata;
    // 规则级 disabled：合并前过滤 disabled:true 条目（安全红线规则 isForceLoadRule 强制保留）
    // 2026-09-13：逐条打 __slot 归属标记——按规则包聚合审计命中数（侧边栏规则包行的
    //   「拦截/警告/通过」显示该规则包的实际命中数）。__slot 用下划线前缀避免与 yml 字段冲突。
    merged.rules.push(...(Array.isArray(ruleData?.rules)
      ? ruleData.rules.filter((x) => x?.disabled !== true || isForceLoadRule(x.id))
        .map((x) => (x && typeof x === 'object' ? { ...x, __slot: slot, scopeRules: normalizeScopeRules(x) } : x))
      : []));
    Object.assign(merged.severity_map, ruleData?.severity_map || {});
    Object.assign(merged.thresholds, ruleData?.thresholds || {});
    merged.ignore.push(...(Array.isArray(ruleData?.ignore) ? ruleData.ignore : []));
    // 私密拦截清单跨文件追加（强制槽位 private.yml 永远最后加载 → 天然追加）
    if (Array.isArray(ruleData?.private_files)) merged.private_files.push(...ruleData.private_files);
  }
  return { ok: errors.length === 0, merged, order: slots, files, errors };
}

/**
 * 设置/取消槽位的 yml 顶层 `disabled`（2026-09-13 约定：启用/禁用以 yml 解析，不单独存变量）。
 *
 * 文本级操作（保留文件内注释与其它内容，不用 js-yaml dump 避免丢注释）：
 *   - 启用：删除顶层 `disabled:` 行
 *   - 禁用：已存在 → 值改为 true；不存在 → 在 metadata 前插入 `disabled: true`
 * nodejs/private 强制加载槽位禁止禁用（安全红线，由调用方拒绝，此处同样防御）。
 * @param {string} slot 槽位名
 * @param {boolean} disabled 目标状态
 * @param {object} [opts] { dir }
 * @returns {{ok: boolean, disabled: boolean, error?: string}}
 */
export function setSlotDisabled(slot, disabled, opts = {}) {
  const dir = opts.dir || RULE_YAML_DIR;
  if (FORCE_LOAD_SLOTS.has(slot)) {
    return { ok: false, error: `${slot} 属安全红线强制槽位，不可通过 UI 禁用` };
  }
  const file = join(dir, `audit-rules-${slot}.yml`);
  if (!existsSync(file)) return { ok: false, error: `规则文件缺失: ${file}` };
  const text = readFileSync(file, 'utf8');
  // 顶层 disabled（无缩进）行：^disabled: [true|false]，多行匹配
  const re = /^disabled:\s*(true|false)\s*$/m;
  let out;
  if (disabled) {
    if (re.test(text)) out = text.replace(re, 'disabled: true');
    else {
      // 在第一个非注释、非空行（通常是 metadata:）之前插入
      const lines = text.split('\n');
      let idx = lines.findIndex((l) => l.trim() && !l.trim().startsWith('#'));
      if (idx < 0) idx = lines.length;
      lines.splice(idx, 0, 'disabled: true');
      out = lines.join('\n');
    }
  } else {
    // 2026-09-13 修复污染：删除整行（含行尾换行），不能只删 `disabled: true` 内容——
    //   否则残留的 \n 会留下一个空行（folder.yml/i18n.yml 曾被误加空行）。
    out = re.test(text) ? text.replace(/^disabled:\s*(true|false)\s*$\n?/m, '') : text;
    // 清理可能留下的多空行粘连（保留单个空行语义）
    out = out.replace(/\n\s*\n\s*\n/g, '\n\n');
  }
  try {
