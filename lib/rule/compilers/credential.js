...
registerCompiler(
  ...
  (r, ctx) => {
    ...
    return ruleOut({
      ...
      dimensions: pickDimensions(r, [DIMENSIONS.安全性], ctx?.errors),
    });
  },
);
...
registerCompiler(
  ...
  (r, ctx) => {
    ...
    return ruleOut({
      ...
      // 2026-09-14 bugfix：补 level（severityLevel：error→blocker）。原缺省默认 warning，
      //   导致 checkCredentialFiles 的 capSeverity('error', 'blocker') 永远不升 blocker，
      //   与文档契约「yml error → 引擎级 blocker 拦截」不一致，.env 等凭据文件提交不拦截。
