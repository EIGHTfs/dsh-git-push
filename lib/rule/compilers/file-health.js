...
/* ───────────────────────── 默认参数（规则条目可用 snake_case 字段覆盖） ───────────────────────── */

/** 三维权重（合计 1.0）：行数 / 大小 / 单行最大长度。 */
const DEFAULT_WEIGHT_LINES = 0.4;
const DEFAULT_WEIGHT_SIZE = 0.35;
const DEFAULT_WEIGHT_LINE_LENGTH = 0.25;
/** 满分基准（score = max(MIN_SCORE, BASE_SCORE - penalty)）。 */
const BASE_SCORE = 10;
/** 下限分：扣分再多也不低于此值（避免 0/负数影响加权）。 */
const MIN_SCORE = 0.1;
/** 行数等级边界（4 个分隔点 → 0~4 级）。 */
const DEFAULT_LINE_LEVELS = [200, 400, 800, 1500];
/** 文件大小等级边界（KB）。 */
const DEFAULT_SIZE_LEVELS = [30, 100, 300, 1000];
/** 单行最大长度等级边界（字符）。 */
const DEFAULT_LINE_LENGTH_LEVELS = [120, 200, 300, 500];
/** 等级扣分表（level 0~4）。 */
const DEFAULT_PENALTIES = [0, 1.0, 2.5, 4.5, 7.0];
/** 严重度分界：score < WARN_SCORE 记 warning，< BLOCK_SCORE 记 blocker。 */
const WARN_SCORE = 9;
const BLOCK_SCORE = 5;
...
registerCompiler(
  ...
  (r) => ruleOut({
    ...
    extra: {
      // 权重（合计 1.0；规则条目可覆盖）
      weightLines: r.weight_lines !== undefined ? Number(r.weight_lines) : DEFAULT_WEIGHT_LINES,
      weightSize: r.weight_size !== undefined ? Number(r.weight_size) : DEFAULT_WEIGHT_SIZE,
      weightLineLength: r.weight_line_length !== undefined ? Number(r.weight_line_length) : DEFAULT_WEIGHT_LINE_LENGTH,
      baseScore: r.base_score !== undefined ? Number(r.base_score) : BASE_SCORE,
      // 等级边界（可覆盖：lines_levels/size_levels/line_length_levels 各 4 个阈值分隔点）
      lineLevels: Array.isArray(r.lines_levels) ? r.lines_levels.map(Number) : [...DEFAULT_LINE_LEVELS],
      sizeLevels: Array.isArray(r.size_levels) ? r.size_levels.map(Number) : [...DEFAULT_SIZE_LEVELS],
      lengthLevels: Array.isArray(r.line_length_levels) ? r.line_length_levels.map(Number) : [...DEFAULT_LINE_LENGTH_LEVELS],
      // 扣分表（level 0~4）
      penalties: Array.isArray(r.level_penalties) ? r.level_penalties.map(Number) : [...DEFAULT_PENALTIES],
      // 豁免（路径包含命中即按规则处理）
      ...
      warnScore: r.warn_score !== undefined ? Number(r.warn_score) : WARN_SCORE,
      blockScore: r.block_score !== undefined ? Number(r.block_score) : BLOCK_SCORE,
    },
  }),
);
