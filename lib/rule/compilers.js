 ...
 * 与既有 readability/magic-number（纯 regex 快速版）并存：smart 版按「数字 + 上下文 + 豁免清单」
 * 精确判定，版本号/日期/HTTP 状态码/常见合法常量/状态枚举不报。
 */
registerCompiler(
  'magic-number-smart',
...
/**
 * cordis.patch.yml insert 语义检查（v1.1.1 修复误报）。
...
registerCompiler(
  ...
  (r) => ruleOut({
    ...
    level: severityLevel(r.severity || 'warning'),
    message: r.description || r.name || '',
    dimensions: [DIMENSIONS.健壮性, DIMENSIONS.可部署性],
    extra: {
...
registerCompiler(
  ...
  (r) => ruleOut({
    ...
    level: severityLevel(r.severity || 'warning'),
    message: r.description || r.name || '',
    dimensions: Array.isArray(r.dimensions) ? r.dimensions : [DIMENSIONS.可维护性, DIMENSIONS.可读性],
    extra: {
      // 权重（合计 1.0；规则条目可覆盖）
