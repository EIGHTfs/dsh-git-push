...
export async function cloneViaApi({ target = '', dest = '', token = '', branch = '' } = {}) {
  ...
  const failed = [];
  const addFail = (path, status, reason) => failed.push({ path, status, reason });
  for (const treeEntry of treeRes.json?.tree || []) {
    ...
    if (treeEntry.mode === '120000') {
      ...
      if (linkRes.status !== 200) { addFail(rel, linkRes.status, linkRes.error || `HTTP ${linkRes.status}`); continue; }
      try {
        await mkdir(dirname(out), { recursive: true });
        await symlink(String(linkRes.json?.content ?? linkRes.text ?? ''), out);
        files++;
      } catch (e) { addFail(rel, -1, `symlink 失败: ${e?.message || e}`); }
      continue;
    }
    ...
    if (blob.status !== 200) { addFail(rel, blob.status, blob.error || `HTTP ${blob.status}`); continue; }
    try {
      ...
      files++;
    } catch (e) { addFail(rel, -1, `写盘失败: ${e?.message || e}`); }
  }
  // 转成 git 仓库并设 origin
  for (const args of [
