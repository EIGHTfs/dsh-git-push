...
export async function cloneViaApi({ target = '', dest = '', token = '', branch = '' } = {}) {
  ...
  let files = 0;
  /** 逐文件失败明细（拉取/写盘）：用于如实反映克隆完整性，不再静默跳过 */
  const failed = [];
  for (const treeEntry of treeRes.json?.tree || []) {
    ...
    if (treeEntry.mode === '120000') {
      ...
      if (linkRes.status !== 200) { failed.push({ path: rel, reason: `HTTP ${linkRes.status}${linkRes.error ? ': ' + linkRes.error : ''}` }); continue; }
      try {
        await mkdir(dirname(out), { recursive: true });
        await symlink(String(linkRes.json?.content ?? linkRes.text ?? ''), out);
        files++;
      } catch (e) { failed.push({ path: rel, reason: `symlink 失败: ${e?.message || e}` }); }
      continue;
    }
    try { await mkdir(dirname(out), { recursive: true }); } catch { /* 忽略 */ }
    const blob = await api(`/git/blobs/${treeEntry.sha}`, 'GET');
    // 2026-09-18：此前 blob 失败只 continue、不留痕，于是「中途超时」的克隆会
    //   静默缺文件却仍返回 ok:true（实测复现：blob 全失败仍报成功）。改为计数上报。
    if (blob.status !== 200) { failed.push({ path: rel, reason: `HTTP ${blob.status}${blob.error ? ': ' + blob.error : ''}` }); continue; }
    try {
      ...
      files++;
    } catch (e) { failed.push({ path: rel, reason: `写盘失败: ${e?.message || e}` }); }
  }
  // 转成 git 仓库并设 origin
  for (const args of [
  ...
  if (failed.length > 0) {
    ...
    return {
      ok: false,
      error: `克隆未完成：${failed.length}/${total} 个文件拉取失败（已清理半成品目录，可直接重试）`,
