...
export function writeAccountStatus(status, { workspaceRoot = '' } = {}) {
  try {
    ...
    try { chmodSync(tmp, 0o600); } catch { /* CIFS 可能改不了 mode */ }
    renameSync(tmp, p);
...
export function buildAccountStatus(result = {}) {
  ...
  return {
    ...
    token: { valid: !!ts.valid, login: ts.login || '', checkedAt: ts.checked ? now : '', detail: ts.detail || '' },
    ssh: { valid: !!ss.valid, login: ss.login || '', checkedAt: ss.checked ? now : '', detail: ss.detail || '' },
    checkedAt: now,
  };
}
...
export function writeAccountStatusFromResult(result, { workspaceRoot = '' } = {}) {
  return writeAccountStatus(buildAccountStatus(result), { workspaceRoot });
}
...
export async function refreshAccountStatus({ workspaceRoot = '', token = '' } = {}) {
  try {
    const result = await checkGithubAccount({ workspaceRoot, token });
    const status = buildAccountStatus(result);
