...
import { readFileSync, mkdirSync, existsSync, watchFile, unwatchFile } from 'node:fs';
...
export function resetScanLive({ workspaceRoot = '' } = {}) {
  ...
  try {
    ...
    writeJsonAtomic(p, { version: 0, found: [], startedAt: new Date().toISOString(), done: false });
  } catch { /* 尽力而为 */ }
