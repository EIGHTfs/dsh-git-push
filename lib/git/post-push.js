/**
 * dsh-git-push — 推送成功后增强（D15 拆出的小模块）
 *
...
/**
 * dsh- 前缀项目自动打 tag（2026-09-02 起全部走 api.github.com）：
...
export async function autoTagDSHProject({ repoPath = '', version = '', commitSha = '', owner = '', repo = '', token = '' } = {}) {
  try {
    ...
    const tagRes = await githubFetch(`/repos/${ownerName}/${repoName2}/git/refs`, {
      token, method: 'POST', body: { ref: `refs/tags/${tag}`, sha: commitSha }, timeout: tagTimeoutMs,
    });
    if (tagRes.status === 201) return { ok: true, tag };
    if (tagRes.status === 422) {
      if (/already exists/i.test(tagRes.json?.message || '')) return { ok: false, skipped: 'tag-exists', tag };
      return { ok: false, skipped: '422', tag, error: tagRes.json?.message || tagRes.status };
    }
    return { ok: false, skipped: String(tagRes.status), tag, error: tagRes.error || ('GitHub API ' + tagRes.status) };
  } catch (e) {
