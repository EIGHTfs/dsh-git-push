 ...
 *   2) PLUGIN_ROOT/lib/user-requirements.json（内置 EIGHTfs 清单）
...
/**
 * 把基线忽略项 + 敏感文件 + 自定义忽略追加进 .gitignore（幂等：已存在行不重复写）。
 ...
 *   added 追加总行数 / files 敏感文件路径 / baseline 基线补入数 / custom 自定义补入数 /
 *   tracked 已跟踪被解除的敏感文件 / unstaged 解除跟踪成功数
 */
export function ensureGitignore(repoPath, { customIgnorePatterns = '' } = {}) {
  ...
  for (const h of files) {
    ...
    const ls = runGit(['ls-files', '--error-unmatch', '--', rel], { cwd: repoPath });
    if (ls.ok) {
      tracked.push(rel);
      const rm = runGit(['rm', '--cached', '--ignore-unmatch', '--', rel], { cwd: repoPath });
      if (rm.ok) unstaged.push(rel);
    }
  }
  ...
  return { added, files: files.map((h) => h.path), baseline, custom, tracked, unstaged };
}
 ...
 */
export async function commitAndPush({ repoPath = '', message = '', push = true, dryRun = false, token = '', customIgnorePatterns = '', requirementsConfirmed = false } = {}) {
  const steps = [];
  ...
  if (privateExempt && gi.files.length) steps.push(`sensitive-scan(${gi.files.length}文件, private 豁免不写)`);
  ...
  const pr = await pushViaApi({ repoPath, token });
  if (pr.ok) {
...
async function uploadBlobsAndBuildTree({ repoPath, entries, remoteBlobShas, api }) {
  ...
  for (const e of entries) {
    ...
    if (!raw.ok) return fail(`读 blob 失败: ${e.path}`);
