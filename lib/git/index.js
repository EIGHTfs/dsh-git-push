...
/** 把敏感文件追加进 .gitignore（幂等：已存在行不重复写）。返回追加行数。 */
export function ensureGitignore(repoPath) {
  ...
  for (const rel of files) {
    const pat = `/${rel}`;
    if (lines.some((l) => l.trim() === rel || l.trim() === pat)) continue;
    additions.push(`/${rel}`);
    added++;
  }
  if (added) {
 ...
 * @param {object} [opts] { depth=3, extraRepos=[], maxRepos=200 }
 * @returns {Array<object>} 仓库信息列表
 */
export function scanRepos(root = '.', { depth = 3, extraRepos = [], maxRepos = 200 } = {}) {
  const out = [];
  const seen = new Set();
