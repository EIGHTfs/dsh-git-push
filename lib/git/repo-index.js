...
import { writeTextAtomic, readJson, writeJsonAtomic, updateJsonAtomic } from './atomic-json.js';
  ...
  try {
    ...
    for (const r of Array.isArray(parsed.repos) ? parsed.repos : []) {
      ...
      map[r.name] = {
        ...
        // 2026-09-16：透传远端状态补查字段（updateRepoRemoteStateInIndex 写入）——列表
        //   liveSkipped 补查后的 SSH 真源结果：云端 HEAD / 领先落后 / 同步态
        remoteHead: r.remoteHead || '',
        ahead: (r.ahead === null || r.ahead === undefined) ? null : Number(r.ahead),
        behind: (r.behind === null || r.behind === undefined) ? null : Number(r.behind),
        remoteHeadAt: r.remoteHeadAt || '',
        synced: !!r.synced,
        remoteStateAt: r.remoteStateAt || '',
      };
    }
    return map;
  } catch { return null; }
...
export async function collectProjectSkills(projectPath) {
  const skills = [];
  const seen = new Set();
  ...
  if (await exists(skillsDir)) {
    try {
      // 逐文件读 skill 文档：改异步以免在循环内阻塞（skill 数随项目增长）
...
async function buildRepoIndexLocalOnly({ workspaceRoot, repos, localOnlyExtra }) {
  ...
  try {
    for (const entry of await readdir(workspaceRoot)) {
      ...
      // 单个条目的 stat 失败（权限/竞态删除）跳过，不影响整体扫描
      try { if (!(await stat(p)).isDirectory()) continue; } catch { continue; }
...
export async function buildRepoIndex({ workspaceRoot, depth = 20, extraRepos = [], extraReposFile = '', token = '', manualVisibility = {}, localOnlyExtra = [], owner = 'EIGHTfs', offline = false, maxRepos = 200 } = {}) {
  ...
  // collectProjectSkills 已改 async（逐文件读 skill 文档不再阻塞），
  //   故用 Promise.all 并发收集；后续 .sort 保持原有按名排序。
  const repoList = (await Promise.all(ownedRepos.map(async (r) => {
    const skills = await collectProjectSkills(r.path);
  ...
  const localOnly = await buildRepoIndexLocalOnly({ workspaceRoot, repos, localOnlyExtra });
...
export function syncRepoIndex({ content, workspaceRoot = '', owner = 'EIGHTfs', syncTarget = '' } = {}) {
  ...
  // 2026-09-16 不变量「索引只存本地仓库」：写入前**剔除云端-only 残留条目**（cloudOnly=true 或
  //   path 为空的旧数据）——此前 merge 曾把本地无副本的云仓库登记进索引，这些条目既违反新语义，
  //   又让前端列表出现空 path 行（React 同 key 警告）。写侧统一过滤，读到的旧索引入口即自愈。
  let out = content;
  try {
    const parsed = JSON.parse(content);
    if (parsed && Array.isArray(parsed.repos)) {
      const kept = parsed.repos.filter((r) => r && !r.cloudOnly && String(r.path || '').trim());
      if (kept.length !== parsed.repos.length) {
        parsed.repos = kept;
        out = JSON.stringify(parsed, null, 2) + '\n';
      }
    }
  } catch { /* 非 JSON 内容（或调用方传纯文本）→ 原样写 */ }
  // 2026-09-16 统一：复用 atomic-json 的原子写（.tmp + rename + chmod），原手写 tmpPath 逻辑移除
  const w = writeTextAtomic(target, out);
  return w.ok ? { ok: true, written: [target] } : { ok: false, written: [] };
}

/**
 * 2026-09-20：把「全量重建结果」与「现有索引」**合并**（不全量覆盖）。
 *
 * 背景（用户实测）：maintainRepoIndex 全量重建后用新内容**整文件覆盖**索引，
 *   而每次重建只含当次扫描范围（owner 过滤 / depth / maxRepos 受限）——于是
 *   dsh-repo-index.json 常被覆盖成「只剩最新一次扫描的子集」，丢历史条目。
 *   合并语义：新结果按 name 覆盖命中条目（保留现有条目的独有字段，如
 *   remoteHead/ahead 等远端状态），现有未命中条目保留，新条目追加——
 *   任何一次更新都不丢既有数据。
 * @param {object|null} existing 现有索引对象（null=无/损坏）
 * @param {string} freshContent buildRepoIndex 输出的 JSON 字符串
 * @returns {{index: object, updated: number}}
 */
function mergeFreshIntoExisting(existing, freshContent) {
  let fresh = null;
  try { fresh = JSON.parse(freshContent); } catch { fresh = null; }
  if (!fresh || !Array.isArray(fresh.repos)) {
    // 新结果不可用（损坏/空）→ 保留现有索引原样
    return { index: existing || { name: 'dsh-repo-index', version: 1, generatedAt: new Date().toISOString(), repos: [], localOnly: [] }, updated: 0 };
  }
  const index = existing && Array.isArray(existing.repos) ? existing : {
    name: fresh.name || 'dsh-repo-index',
    version: 1,
    generatedAt: new Date().toISOString(),
    owner: fresh.owner || 'EIGHTfs',
    note: 'auto-generated by dsh-git-push: DO NOT EDIT MANUALLY',
    repos: [],
    localOnly: [],
  };
  const freshMap = new Map(fresh.repos.map((r) => [String(r.name || ''), r]));
  let updated = 0;
  const seen = new Set();
  const merged = (index.repos || []).map((prev) => {
    const fr = freshMap.get(String(prev.name || ''));
    if (!fr) return prev; // 本次扫描没扫到的旧条目：保留
    seen.add(String(prev.name || ''));
    updated += 1;
    // fresh 覆盖核心字段（path/repoUrl/cloneCmd/skills…），prev 独有字段（远端状态等）保留；
    // visibility 只在 fresh 有真值时才覆盖（fresh 在离线重建时可能回退为既有标注/空）
    const next = { ...prev, ...fr };
    if (!String(fr.visibility || '').trim()) next.visibility = prev.visibility;
    return next;
  });
  for (const fr of fresh.repos) {
    if (!seen.has(String(fr.name || ''))) { merged.push(fr); updated += 1; }
  }
  return {
    index: {
      ...index,
      repos: merged.sort((a, b) => String(a.name).localeCompare(String(b.name))),
      localOnly: [...new Set([...(index.localOnly || []), ...(fresh.localOnly || [])])].sort(),
      generatedAt: new Date().toISOString(),
      owner: fresh.owner || index.owner,
    },
    updated,
  };
}
...
/**
 * **统一更新入口（2026-09-20 收口）**：所有 dsh-repo-index.json 更新都走本函数。
 *
 * 三种模式：
 *   · mode='rebuild'（默认）：全量重建（buildRepoIndex）→ 与现有索引**合并**写回
 *     （不全量覆盖，任何更新不丢既有条目）——替代原 maintainRepoIndex 的整文件覆盖；
 *   · mode='merge-cloud'：云端扫描结果合并（转发 mergeCloudReposIntoIndex）；
 *   · mode='single'：单条目远端状态补查（转发 updateRepoRemoteStateInIndex）。
 *
 * 返回 `indexUpdated` 标志：调用方（HTTP 端点 / 工具 / CLI）透传给 UI，
 *   UI 本地仓库列表据此重新读取 dsh-repo-index（/repos-local），保持列表最新。
 * @param {object} opts { workspaceRoot, token, owner, depth, extraRepos, extraReposFile,
 *   syncTarget, offline, maxRepos, mode, cloudRepos, repoName, remoteState }
 * @returns {Promise<{ok:boolean, indexUpdated:boolean, target?:string, updated?:number, error?:string}>}
 */
export async function updateRepoIndex({ workspaceRoot = '', token = '', owner = 'EIGHTfs', depth = 20, extraRepos = [], extraReposFile = '', syncTarget = '', offline = false, maxRepos = 200, mode = 'rebuild', cloudRepos = [], repoName = '', remoteState = {} } = {}) {
  try {
    if (mode === 'merge-cloud') {
      const r = mergeCloudReposIntoIndex({ workspaceRoot, owner, cloudRepos, syncTarget });
      return { ...r, indexUpdated: r.ok === true };
    }
    if (mode === 'single') {
      const r = updateRepoRemoteStateInIndex({ workspaceRoot, repoName, remoteState, syncTarget });
      return { ...r, indexUpdated: r.ok === true };
    }
    // rebuild（默认）：全量重建 → 与现有索引合并（不全量覆盖）
    const target = syncTarget || locateRepoIndex(workspaceRoot) || join(credentialsDir({ workspaceRoot: workspaceRoot || '' }), INDEX_FILE);
    let existing = null;
    try { if (existsSync(target)) existing = JSON.parse(readFileSync(target, 'utf8')); } catch { existing = null; }
    const manualVisibility = parseManualVisibility(existing ? JSON.stringify(existing) : '');
    ...
    const { index, updated } = mergeFreshIntoExisting(existing, fresh);
    const sync = syncRepoIndex({ content: JSON.stringify(index, null, 2) + '\n', workspaceRoot, owner, syncTarget });
    return { ok: sync.ok, target: sync.written[0] || '', indexUpdated: sync.ok === true, updated };
  } catch (e) {
...
/**
 * 推送成功后全量重建索引（v1 语义；2026-09-20 起内部走 updateRepoIndex 合并，不再整文件覆盖）。
 * token 缺失时可见性回退既有标注/未知；任何失败静默返回 {ok:false, error}，不阻塞推送结果。
 * @deprecated 新代码请直接调 updateRepoIndex（统一入口）；本函数保留签名兼容旧调用。
 */
export async function maintainRepoIndex({ workspaceRoot = '', token = '', owner = 'EIGHTfs', depth = 20, extraRepos = [], extraReposFile = '', syncTarget = '', offline = false, maxRepos = 200 } = {}) {
  const r = await updateRepoIndex({ workspaceRoot, token, owner, depth, extraRepos, extraReposFile, syncTarget, offline, maxRepos, mode: 'rebuild' });
  return r;
}
...
export function mergeCloudReposIntoIndex({ workspaceRoot = '', owner = 'EIGHTfs', cloudRepos = [], syncTarget = '' } = {}) {
  try {
    ...
    for (const cloudRepo of Array.isArray(cloudRepos) ? cloudRepos : []) {
      ...
      // 2026-09-16 语义收缩：**dsh-repo-index 只存本地仓库**——云端扫描只**更新**索引里已有
      //   条目的云端字段（defaultBranch/pushedAt/visibility 等），**不新增云端-only 条目**
      //   （本地没有副本的云仓库不进索引；前端「云端有、本地无」由 repos-cloud 的 localExists 判断）。
      if (!prev) continue;
      ...
      const next = {
        ...prev,                                                     // 保留本地条目全部既有字段（path/skills/cloneCmd…）
        name,
        repoUrl,
        visibility,
      ...
      if (prev.visibility !== next.visibility || prev.pushedAt !== next.pushedAt) updated++;
...
/**
 * 把「远端状态补查结果」写回索引条目（2026-09-16：本地扫描完自动补查 liveSkipped 仓库）。
 *
 * 本地列表（repos-local）对超预算/熔断的仓库标 liveSkipped（远端状态未知）；本函数把
 *   liveRemoteHead 补查到的 remoteHead/ahead/behind/remoteHeadAt/synced 写回索引，
 *   下次列表读取直接有值（不再每次补查）。
 * 2026-09-19 扩展：remoteState 支持 visibility —— 推送成功后回写可见性，
 *   此前只有「全量重建索引（maintainRepoIndex）」才会刷新 visibility，而推送路径
 *   调的就是全量重建，它**不写** remoteHead/remoteHeadAt/remoteStateAt，
 *   于是推送后索引里这些远端状态字段长期停留在上一轮扫描的旧值。
 * @param {object} opts { workspaceRoot, repoName, remoteState }
 *   remoteState = { remoteHead, ahead, behind, remoteHeadAt, synced, visibility? }
 *   visibility 传空/未传 = 不改动既有值（避免把已知可见性抹成「未知」）
 * @returns {{ok:boolean, target?:string, error?}}
 */
export function updateRepoRemoteStateInIndex({ workspaceRoot = '', repoName = '', remoteState = {}, syncTarget = '' } = {}) {
  try {
    const target = syncTarget || locateRepoIndex(workspaceRoot) || join(credentialsDir({ workspaceRoot: workspaceRoot || '' }), INDEX_FILE);
    let index = null;
    try {
      if (existsSync(target)) index = JSON.parse(readFileSync(target, 'utf8'));
    } catch { index = null; }
    if (!index || !Array.isArray(index.repos) || !repoName) return { ok: false, error: '索引缺失/仓库名缺失' };
    const hit = index.repos.find((r) => r.name === repoName);
    if (!hit) return { ok: false, error: `索引无 ${repoName} 条目（先云端/本地扫描登记）` };
    const { remoteHead = '', ahead = null, behind = null, remoteHeadAt = '', synced = false, visibility = '' } = remoteState;
    hit.remoteHead = remoteHead;
    hit.ahead = ahead;
    hit.behind = behind;
    hit.remoteHeadAt = remoteHeadAt;
    hit.synced = synced;
    // 可见性只在拿到真值时覆盖：'' 表示「本次没查到」而非「未知」，
    //   直接写会把云端扫描/手工标注得到的好值抹掉。
    if (visibility) hit.visibility = visibility;
    hit.remoteStateAt = new Date().toISOString();
    index.generatedAt = new Date().toISOString();
    const written = syncRepoIndex({ content: JSON.stringify(index, null, 2) + '\n', workspaceRoot, owner: index.owner || 'EIGHTfs', syncTarget });
