 ...
 *
 * 写入目标：工作区 dsh-git-push-User/<owner>/dsh-repo-index.json（既有文件原位更新；
 *   与 dsh-repo-index skill 权威位置一致）。失败全部静默降级，不阻塞扫描/推送。
 */
...
/** 找工作区下的索引文件：<root>/dsh-git-push-User/<owner>/dsh-repo-index.json（owner 目录动态）。 */
export function locateRepoIndex(workspaceRoot) {
  if (!workspaceRoot) return null;
  const userDir = join(workspaceRoot, 'dsh-git-push-User');
  if (!existsSync(userDir)) return null;
  try {
    for (const owner of readdirSync(userDir)) {
      const p = join(userDir, owner, INDEX_FILE);
...
/** 写入索引：既有文件原位更新；不存在写 工作区/dsh-git-push-User/<owner>/dsh-repo-index.json。 */
export function syncRepoIndex({ content, workspaceRoot = '', owner = 'EIGHTfs', syncTarget = '' } = {}) {
  const target = syncTarget || locateRepoIndex(workspaceRoot) || join(workspaceRoot || '.', 'dsh-git-push-User', owner, INDEX_FILE);
  try {
