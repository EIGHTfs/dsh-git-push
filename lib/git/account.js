...
export async function checkGithubAccount({ workspaceRoot = '', token = '', checkSsh = true } = {}) {
  ...
  if (tok) {
    ...
    const raw = String(userResp.json?.message || userResp.error || '');
    // 2026-09-17：区分「凭据无效」与「本次没测成」——网络超时/断网时凭据本身可能完全正常，
    //   不能一律报「已失效」（否则用户以为凭据坏了、按钮没用）。timeout 标志供上层文案分支。
    const timedOut = /abort|timeout|timed out|ETIMEDOUT|ENOTFOUND|ECONNRESET|EAI_AGAIN/i.test(raw)
      || Number(userResp.status) === 0;
    tokenStatus = {
      valid: ok,
      checked: true,
      login: ok ? userResp.json.login : '',
      timeout: !ok && timedOut,
      detail: ok ? '' : (raw || `HTTP ${userResp.status}`),
    };
...
export function formatGithubAccountBlock(r = {}) {
  ...
  const s = r.sshStatus || {};
  // 2026-09-17：取消单一「已登录 GitHub：X」汇总行——Token 与 SSH 可以是**不同 GitHub 用户**
  //   且各自有效，写唯一用户名会隐藏另一个账号。改为逐条列出各自用户名/掩码/时间与失效原因。
  const fmtAt = (iso) => {
    const tt = Date.parse(String(iso || ''));
    if (!Number.isFinite(tt)) return '';
    const d = new Date(tt); const p = (n) => String(n).padStart(2, '0');
    return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}`;
  };
  const line = (label, present, item, masked) => {
    if (!present) return `⏳ ${label}：未配置`;
    const when = fmtAt(item.checkedAt);
    const who = item.login || masked || '已配置';
    if (item.valid) return `✅ ${label}：${who}${when ? `｜校验于 ${when}` : ''}`;
    if (item.timeout) return `⏳ ${label}：${who}｜未测成（网络超时，可重试）`;
    if (when) return `❌ ${label}：${who}｜失效于 ${when}${item.detail ? `（${item.detail}）` : ''}`;
  ...
  if (!r.cookieSet) {
    ...
    L.push('✅ 凭据可用');
    if (cred.hasToken || cred.hasSshPub) L.push(`- 公钥仓库数：${r.publicRepos ?? '?'}｜套餐：${r.plan || '?'}`);
    L.push(`- ${line('Token', cred.hasToken, t, cred.tokenMasked)}`);
    L.push(`- ${line('SSH 公钥', cred.hasSshPub, s, cred.sshFingerprint)}`);
  } else {
    L.push(`⚠️ 凭据不可用：${r.detail || '凭据缺失或未校验'}`);
    L.push(`- ${line('Token', cred.hasToken, t, cred.tokenMasked)}`);
    L.push(`- ${line('SSH 公钥', cred.hasSshPub, s, cred.sshFingerprint)}`);
  }
  return L.join('\n');
