...
export function testSshAuth({ workspaceRoot = '' } = {}) {
  ...
  for (const { keyPath } of keys) {
    ...
    const m = out.match(/Hi\s+([A-Za-z0-9._-]+)!/);
    if (m) return { valid: true, checked: true, login: m[1], detail: out.trim() };
    // 权限拒绝 / 超时 / 网络失败 → 无效
    if (!m) {
...
export async function checkGithubAccount({ workspaceRoot = '', token = '', checkSsh = true } = {}) {
  ...
  if (tok) {
    ...
    const ok = Boolean(res.status === 200 && res.json?.login);
    tokenStatus = { valid: ok, checked: true, login: ok ? res.json.login : '', detail: ok ? '' : (res.json?.message || res.error || `HTTP ${res.status}`) };
    if (ok) userProfile = res.json;
  }
