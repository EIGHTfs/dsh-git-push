...
export function liveRemoteHead({ repoPath = '', branch = '' } = {}) {
  ...
  for (const { keyPath, kind } of resolveSshKeys()) {
    ...
    tried += 1;
    const sshCmd = `ssh -i "${keyPath}" -p 443 -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile="${known}" -o BatchMode=yes -o ConnectTimeout=5`;
