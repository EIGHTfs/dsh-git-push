...
export async function dispatchPush({ repoPath = '', branch = '', token = '', force = false, pushMethod = 'ssh' } = {}) {
  ...
  if (isNonFastForward(sshRes.reason)) {
    ...
    const remoteHead = runGit(['rev-parse', `refs/remotes/origin/${cur}`], { cwd: repoPath }).stdout;
    return {
      ok: false, pushed: false, method: 'ssh', diverged: true,
      localHead: localHead || '', remoteHead: remoteHead || '',
      reason: `远端已分叉（non-fast-forward）：本地 ${(localHead || '?').slice(0, 7)} 与远端 ${(remoteHead || '?').slice(0, 7)} 各有对方没有的提交。`
        + '已阻止回落 API（回落会在远端重建提交、加剧分叉）：请确认后以 force 强推本地历史，或先 fetch/pull 整合远端。',
    };
  }
  const apiRes = await tryApi();
  if (apiRes.ok) {
