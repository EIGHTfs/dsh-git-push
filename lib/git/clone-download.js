 ...
 *
 * 端点选择（实测结论，2026-09-18 / 2026-09-19 修订）：
 *   api.github.com/repos/<o>/<r>/contents/<path>?ref=<branch>  → 稳定，并发 8/8 通；
 *       但忽略 Range（HTTP 200，无 Content-Range，整包重发）
 *   raw.githubusercontent.com/<o>/<r>/<branch>/<path> → 支持 Range（HTTP 206 + Content-Range）；
 *       但本机网络下**并发极不稳**——实测并发 8 只有 1 条通，其余直接连不上
 *
 * 由此确定**双通道策略**（首个成功即用，全失败才报错）：
 *   常态（have === 0）：先 api（稳），失败退 raw
 *   续传（have > 0）  ：先 raw（唯一支持 Range），失败退 api（整包重下）
 *   2026-09-19 之前只用 raw，导致 gallery（99 个文件）克隆时 95 个因并发不通报 404。
 *
...
import { mkdir, rename, unlink, chmod, symlink, rm, readdir, rmdir, access } from 'node:fs/promises';
...
export async function downloadBlobs(o) {
  const {
    ...
    concurrency = DEFAULT_CONCURRENCY, onProgress = null, signal = null,
  } = o;
  ...
  const one = async (entry) => {
    // 中止检查点：清理同目录残留前会 abort，worker 必须在此退出，
    //   否则「边删边写」会让 rmdir 报 ENOTEMPTY，且下载变成无人回收的孤儿。
    ...
    try {
      const r = await fetchToFile({ owner, repo, branch, relPath: rel, token, out, partPath, size, signal, onBytes: (n) => { transferred += n; emit(); } });
  ...
  const worker = async () => {
    for (;;) {
      if (signal?.aborted) return;   // 中止后立即退出，不再领取新任务
      const i = cursor++;
  ...
  // 已中止时跳过：此时上层正在清理同一目录，再删一次只会互相踩。
  if (!signal?.aborted) {
    try { await removeDirForce(partsDir); } catch { /* 忽略 */ }
  }
 ...
 */
/**
 * 强制删除目录，专治 CIFS 上的 ENOTEMPTY。
 *
 * 为什么不能直接用 rm(recursive)：
 *   实测（2026-09-18）在 CIFS 挂载（vers=3.02, nounix, actimeo=1）上，
 *   目录内文件正被并发写入时 rm 会失败并抛
 *     ENOTEMPTY: directory not empty, rmdir '.../.dsh-parts'
 *   —— Node 的 rm 先 readdir 收集条目、再逐个删、最后 rmdir；
 *   而 readdir 与服务端之间没有原子性，边删边写就会出现
 *   「readdir 看到的已删完、rmdir 时服务端又有新条目」。
 *   故此处：先重试若干次（给写者时间退出），仍失败则逐个 unlink（force 语义，
 *   文件可能已被别人删掉，ENOENT 必须忽略），最后再 rmdir 并忽略失败。
 * @param {string} dir 目标目录
 * @param {number} attempts 重试次数
 * @returns {Promise<boolean>} 是否已确认删除（目录不存在也算 true）
 */
export async function removeDirForce(dir, attempts = 3) {
  ...
  // 兜底：逐个删条目后再 rmdir。ENOENT 一概忽略（并发下文件可能已消失）
  try {
    const entries = await readdir(dir);
    for (const name of entries) {
      const p = join(dir, name);
      try { await rm(p, { recursive: true, force: true }); }
      catch { try { await unlink(p); } catch { /* 已不存在 */ } }
    }
  } catch { /* readdir 失败：目录可能已不在 */ }
  try { await rmdir(dir); return true; } catch { /* 已被删或仍有并发写入 */ }
  try { await access(dir); return false; } catch { return true; }   // 不存在即视为成功
}

async function fetchToFile({ owner, repo, branch, relPath, token, out, partPath, size, onBytes, signal = null }) {
  if (signal?.aborted) return { ok: false, status: 0, reason: '已中止' };
  // 复用**上次已下完的最终文件**：clone 失败后目录会被保留（见 clone.js 的 kept 语义），
  //   那些已 rename 到位、长度与远端一致的文件无需重下。
  //   为什么必须在这里判：续传原本只认 .part 分片，而失败清理会删掉 .dsh-parts，
  //   于是最终文件虽被保留却仍被重下——实测 gallery（153.8MB）第二次 clone
  //   把 13 个已下好的文件全部重下一遍，保留形同虚设。
  //   判据与「.part 已完整」一致：长度相等即认为完整（rename 是原子的，
  //   最终路径上不存在半截文件，故长度相等可安全视为完整）。
  if (size > 0) {
    let haveFinal = 0;
    try { if (existsSync(out)) haveFinal = statSync(out).size; } catch { haveFinal = 0; }
    if (haveFinal === size) return { ok: true, reused: true };
  }
  let have = 0;
  ...
  // 双通道：先 api.github.com，失败再退 raw。原因见文件头「端点选择」——
  //   实测（2026-09-19）本机网络下 raw 并发 8 只有 1 条通，api 并发 8 全通；
  //   而 raw 是唯一支持 Range 续传的端点。故常态走 api（稳定），
  //   需要续传（have > 0）或 api 失败时退 raw（能断点续，接受其并发不稳）。
  const useRawFirst = have > 0;
  const apiUrl = `https://api.github.com/repos/${owner}/${repo}/contents/${encoded}?ref=${encodedBranch}`;
  const rawUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${encodedBranch}/${encoded}`;
  const mkHeaders = (raw, withRange) => {
    const h = { 'User-Agent': 'dsh-git-push' };
    if (token) h.Authorization = `Bearer ${token}`;
    // api 的 contents 端点要 Accept 才回原始字节（否则回 JSON/base64）
    if (!raw) h.Accept = 'application/vnd.github.raw';
    if (withRange && have > 0) h.Range = `bytes=${have}-`;
    return h;
  };
  const urls = useRawFirst ? [[rawUrl, true], [apiUrl, false]] : [[apiUrl, false], [rawUrl, true]];

  // 依次尝试候选通道，首个成功即用；全失败则回报最后一条的成因
  let resp = null;
  let lastErr = { status: 0, reason: '无可用通道' };
  for (const [candidate, withRange] of urls) {
    let r;
    try {
      r = await fetch(candidate, { headers: mkHeaders(withRange, withRange), ...(signal ? { signal } : {}) });
    } catch (e) {
      lastErr = { status: 0, reason: `请求失败: ${e?.message || e}` };
      continue;
    }
    if (r.status === 200 || r.status === 206) { resp = r; break; }
    let detail = '';
    try { detail = (await r.text()).slice(0, 120); } catch { /* 忽略 */ }
    lastErr = { status: r.status, reason: `HTTP ${r.status}${detail ? ': ' + detail : ''}` };
  }
  if (!resp) return { ok: false, status: lastErr.status, reason: lastErr.reason };
