...
import { getSkipSet } from '../skip-dirs.js';
...
/** 扫描仓库内敏感文件（相对路径列表）。 */
/** 递归扫描跳过的目录 = 统一跳过目录（node_modules/.git + yml 黑名单关键词，2026-09-16 共用 lib/skip-dirs.js）。 */
const SENSITIVE_SKIP_DIRS = getSkipSet();
...
function sensitiveLineMatch(line) {
  ...
  for (const k of SENSITIVE_KEYS) {
    ...
    if (match) {
      ...
      if (!qm) continue;
      let fieldValue = qm[2].trim();
      if (/\+|\$\{/.test(fieldValue)) continue; // 拼接/模板插值 = 表达式
      if (/[\u4e00-\u9fff]/.test(fieldValue)) continue; // 值含中文 = UI 文案/说明，不是凭据（如 account: '账号检查'）
      if (fieldValue.length >= k.valueMin && !SENSITIVE_PLACEHOLDER_VALUE.test(fieldValue)) return k.name;
    }
  }
  return null;
}
 ...
 */
export async function scanSensitiveFiles(repoPath) {
  const hits = [];
  // 递归扫描：改异步以免在大仓库上逐文件阻塞事件循环
  //   （readdir 带 withFileTypes，目录项类型一次带回；内容读取与 stat 同样异步化）
  const walk = async (dir, rel) => {
    ...
    for (const it of items) {
      ...
      if (it.isDirectory()) {
        ...
        await walk(join(dir, it.name), relPath);
      } else if (it.isFile()) {
        // 文件名黑名单：名字本身就是凭据载体
        ...
        // 内容级：文本文件内敏感键值检测
        try {
          const st = await stat(join(dir, it.name));
        ...
        let text;
        try { text = await readFile(join(dir, it.name), 'utf8'); } catch { continue; }
