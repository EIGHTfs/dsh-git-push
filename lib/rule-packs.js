...
/** 规则槽位（插件自带 YAML 的固定身份；private 为强制加载槽位，不受 auditRuleOrder 排序影响） */
export const RULE_SLOTS = ['nodejs', 'frontend', 'comment', 'dsh', 'npm', 'template', 'private'];
/** 默认加载顺序（侧边栏可改；后覆盖前） */
export const DEFAULT_RULE_ORDER = ['nodejs', 'frontend', 'comment', 'dsh', 'npm']; // template 是空模板：不加载（用户自定义入口，侧边栏启用才入顺序）；dsh=v1.50.0 dsh 插件专属审计；npm=v1.51.0 npm 发布审计
...
function compileRuleBuckets(out, merged, errors) {
  ...
  for (const r of merged?.rules || []) {
    ...
    if (patterns.length) {
      ...
      if (res.length) {
        out.styleRules.push({
          id: r.id, name: label, severity, level, kind: 'regex', patterns: res, message, fixable: r.fixable === true,
