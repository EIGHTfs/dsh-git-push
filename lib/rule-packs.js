...
/** 规则槽位（插件自带四份 YAML 的固定身份） */
export const RULE_SLOTS = ['nodejs', 'frontend', 'comment', 'template'];
/** 默认加载顺序（侧边栏可改；后覆盖前） */
export const DEFAULT_RULE_ORDER = ['nodejs', 'frontend', 'comment']; // template 是空模板：不加载（用户自定义入口，侧边栏启用才入顺序）
/** YAML 规则目录（插件内置） */
...
export function loadYamlRuleFiles(order) {
  ...
  const errors = [];
  const files = [];
  const merged = { metadata: {}, rules: [], severity_map: {}, thresholds: {}, ignore: [], output: {}, dynamic_detection: null, fullScan: null, quality: null, commentScoring: null };
  for (const slot of seq) {
    ...
    // severity_map / thresholds / output：后覆盖前
    ...
    for (const rule of d.rules || []) {
      ...
      // comment.yml 形态：规则内嵌 blacklist/whitelist/scoring → 提升为合并级 commentScoring（后覆盖前）
      if (Array.isArray(rule.blacklist) && rule.blacklist.length) {
        const cur = merged.commentScoring || { blacklist: [], whitelist: [], features: [], threshold: 60, thresholdHighlySuspicious: 80, maxScore: 100, suggestions: [] };
        ...
        for (const b of rule.blacklist || []) {
          if (!b || !b.pattern) continue;
          const i = cur.blacklist.findIndex((x) => x.pattern === b.pattern);
          if (i >= 0) cur.blacklist[i] = { ...cur.blacklist[i], ...b };
          else cur.blacklist.push(b);
        }
        for (const w of rule.whitelist || []) {
          if (!w || !w.pattern) continue;
          const i = cur.whitelist.findIndex((x) => x.pattern === w.pattern);
          if (i >= 0) cur.whitelist[i] = { ...cur.whitelist[i], ...w };
          else cur.whitelist.push(w);
        }
        for (const f of rule.additional_features || []) {
          if (!f || !f.name) continue;
          const i = cur.features.findIndex((x) => x.name === f.name);
          if (i >= 0) cur.features[i] = { ...cur.features[i], ...f };
          else cur.features.push(f);
        }
    ...
    if (dC && typeof dC === 'object') {
      ...
      for (const b of dC.blacklist || []) {
        if (!b || !b.pattern) continue;
        const i = cur.blacklist.findIndex((x) => x.pattern === b.pattern);
        if (i >= 0) cur.blacklist[i] = { ...cur.blacklist[i], ...b };
        else cur.blacklist.push(b);
      }
      for (const w of dC.whitelist || []) {
        if (!w || !w.pattern) continue;
        const i = cur.whitelist.findIndex((x) => x.pattern === w.pattern);
        if (i >= 0) cur.whitelist[i] = { ...cur.whitelist[i], ...w };
        else cur.whitelist.push(w);
      }
      for (const f of dC.features || []) {
        if (!f || !f.name) continue;
        const i = cur.features.findIndex((x) => x.name === f.name);
        if (i >= 0) cur.features[i] = { ...cur.features[i], ...f };
        else cur.features.push(f);
  ...
  if (merged.rules.length === 0) errors.push('合并后无任何规则');
...
export function validateYamlRuleSet(merged) {
  ...
  const ids = new Set();
  for (const [i, r] of (merged.rules || []).entries()) {
    const label = r?.id || `#${i}`;
...
function compileRuleBuckets(out, merged, errors) {
  ...
  for (const r of merged?.rules || []) {
    ...
    // 数值型（函数行数等 → codeQuality；其余风格数值 → styleRules）
    if (r?.id === 'func-lines' || (r?.max_lines && /function/i.test(r?.name || ''))) {
    ...
    if (r?.min_length !== undefined || r?.max_lines !== undefined || r?.max_complexity !== undefined || r?.min_occurrences !== undefined) {
      out.styleRules.push({
        id: r.id, name: label, severity, level, kind: r.min_length !== undefined ? 'min-length' : r.max_lines !== undefined ? 'max-lines' : r.max_complexity !== undefined ? 'max-complexity' : 'min-occurrences',
        threshold: r.min_length ?? r.max_lines ?? r.max_complexity ?? r.min_occurrences,
    ...
    if (patterns.length) {
      ...
      if (res.length) {
        out.styleRules.push({ id: r.id, name: label, severity, level, kind: 'regex', patterns: res, message, fixable: r.fixable === true });
      }
      continue;
    }
...
export function getCompiledRulePack(choice = {}) {
  ...
  const c = Array.isArray(choice) ? { order: choice }
    : typeof choice === 'string'
      ...
      : (choice || {});
  const order = Array.isArray(c.order) && c.order.length ? c.order.filter((s) => RULE_SLOTS.includes(s)) : [...DEFAULT_RULE_ORDER];
...
export function resolveRulesetChoice(cfg) {
  ...
  if (!s || s === 'builtin' || s === 'eightfs' || s === 'default' || s === 'yaml') return { source: 'yaml', order: [...DEFAULT_RULE_ORDER] };
  const order = s.split(',').map((x) => x.trim()).filter(Boolean);
  return { source: 'yaml', order: order.filter((x) => RULE_SLOTS.includes(x)) };
}
...
export async function loadRulePack(choice = {}) {
  ...
  const order = Array.isArray(c.order) && c.order.length ? c.order : [...DEFAULT_RULE_ORDER];
  const loaded = loadYamlRuleFiles(order);
