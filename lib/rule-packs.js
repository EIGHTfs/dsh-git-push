...
function compileRuleBuckets(out, merged, errors) {
  ...
  for (const r of merged?.rules || []) {
    ...
    if (r?.min_length !== undefined || r?.max_lines !== undefined || r?.max_complexity !== undefined || r?.min_occurrences !== undefined || r?.max_depth !== undefined) {
      out.styleRules.push({
        id: r.id, name: label, severity, level,
        kind: r.min_length !== undefined ? 'min-length' : r.max_lines !== undefined ? 'max-lines' : r.max_complexity !== undefined ? 'max-complexity' : r.max_depth !== undefined ? 'max-depth' : 'min-occurrences',
