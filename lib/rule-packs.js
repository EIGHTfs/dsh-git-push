...
import { FULLSCAN_DEFAULTS } from './full-scan.js'; // v1.43.0：fullScan 段编译兜底缺省
 ...
 */
/**
 * v1.43.0：校验 fullScan 段（附属能力：全仓 AI 对话残留注释扫描）。
 * 阈值必须正数；keywords/protectWords 必须数组且每项含 name/pattern（正则可编译）。
 */
function validateFullScanSection(section, errors) {
  if (section === undefined) return;
  if (!section || typeof section !== 'object') { errors.push('fullScan 必须是对象'); return; }
  if (section.threshold !== undefined && (!Number.isFinite(section.threshold) || section.threshold <= 0)) errors.push('fullScan.threshold 必须是正数');
  for (const [key, arrKey] of [['keywords', 'fullScan.keywords'], ['protectWords', 'fullScan.protectWords']]) {
    if (section[key] !== undefined && !Array.isArray(section[key])) { errors.push(`${arrKey} 必须是数组`); continue; }
    for (const item of (Array.isArray(section[key]) ? section[key] : [])) {
      if (!item || typeof item !== 'object' || !item.pattern) { errors.push(`${arrKey} 每项需含 name/pattern/score`); continue; }
      try { new RegExp(item.pattern, item.flags || ''); } catch (e) { errors.push(`${arrKey} 正则编译失败: ${String(e?.message || e).slice(0, 80)}`); }
    }
  }
}
...
export function validateRulePack(obj) {
  ...
  validateFullScanSection(obj.fullScan, errors); // v1.43.0：fullScan 附属能力段校验
 ...
 */
/** v1.42.0/v1.43.0：编译 quality 段（函数行数阈值 + 文件级评分阈值；缺省=内置基准） */
...
export function compileRulePack(pack, { source = 'builtin' } = {}) {
  ...
  const out = {
    ...
    wordingPatterns: [],
    docConvPatterns: [],
    quality: compileQualitySection(pack?.quality), // v1.42.0/v1.43.0：函数行数 + 文件级评分阈值（规则包可覆盖）
    codeQuality: {},
    // v1.43.0：fullScan 附属能力段（全仓 AI 对话残留注释扫描；缺省 null → full-scan.js 内置缺省）
    fullScan: compileFullScanSection(pack?.fullScan),
    errors,
  };
...
/**
 * v1.43.0：编译规则包 fullScan 段（附属能力：全仓 AI 对话残留注释扫描）。
 * 缺省/非法项逐条降级为内置缺省（黑名单加分/白名单减分/阈值），引擎见 lib/full-scan.js。
 * @returns {{threshold:number, keywords:Array<{name,re,score}>, protectWords:Array<{name,re,score}>, signals:object}|null}
 */
function compileFullScanSection(section) {
  if (!section || typeof section !== 'object') return null; // null → 引擎用内置缺省
  const safe = (arr, dfltScore) => (Array.isArray(arr) ? arr : []).map((k) => {
    let re = /(?!)/; // 永不匹配兜底
    try { re = new RegExp(k.pattern, k.flags || ''); } catch { /* 校验期已报错，这里降级 */ }
    return { name: String(k.name || '(unnamed)'), re, score: Number(k.score) > 0 ? Number(k.score) : dfltScore };
  });
  return {
    threshold: Number(section.threshold) > 0 ? Number(section.threshold) : 60,
    keywords: safe(section.keywords, 40),
    protectWords: safe(section.protectWords, 30),
    signals: { ...FULLSCAN_DEFAULTS.signals, ...(section.signals || {}) },
  };
}
