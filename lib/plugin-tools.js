...
/**
 * git_scan — 扫描 DSH workspace 下所有 git 仓库。
 * deps：{ workspaceRoot, depth, extraRepos, extraReposFile }
 */
export function toolGitScan(deps) {
...
/**
 * git_commit_push — 一键提交并推送（带审计门禁）。
 * deps：{ commitWithAudit, env }
 */
export function toolCommitPush(deps) {
  const { commitWithAudit, env } = deps;
  return defineTool({
...
/**
 * code_audit — 对指定 git 仓库执行代码审计。
 * deps：{ auditRepoPath }
 */
export function toolCodeAudit(deps) {
  const { auditRepoPath } = deps;
  return defineTool({
    name: 'code_audit',
...
/**
 * audit_full_scan — 全仓 AI 对话残留注释扫描（附属能力）。
 * deps：{ fullScan }
 */
export function toolAuditFullScan(deps) {
  const { fullScan } = deps;
  return defineTool({
    name: 'audit_full_scan',
    description: '全仓 AI 对话残留注释扫描（附属能力，v1.43.0）：黑名单关键词加分（用户指示/原话/客户要求/request 等）、白名单保护词减分（ID/密码/token 等技术词），总分 ≥ 阈值记「警告」并标⚠。规则来自规则包 fullScan 段（缺省内置 EIGHTfs 包），ruleset 可选临时换包（builtin | 本地规则包绝对路径 | http(s)://）。纯只读——只报告不删码不拦截；返回 markdown 表格列出全部命中位置（文件:行号/分数/命中/文本），按分数降序。适合老仓库一次性清洗；日常提交门禁只对本仓新增行注释温和提醒。',
    parameters: {
      repo: { type: 'string', description: '仓库绝对路径' },
      ruleset: { type: 'string', description: '可选：本次扫描用规则包（builtin | 本地规则包绝对路径 | http(s):// 在线包），缺省用配置 auditRuleset' },
    },
    output: { schema: { type: 'string' }, render: textRender },
    execute: async ({ repo, ruleset }) => {
      if (!repo) return JSON.stringify({ ok: false, error: '缺少 repo' }, null, 2);
      const result = await fullScan(repo, { ruleset: ruleset || undefined });
      return JSON.stringify(result, null, 2);
    },
  });
}
...
/**
 * git_gen_readme — 按模板生成 README。
 * deps：{ workspaceRoot }
 */
export function toolGenReadme(deps) {
...
/**
 * git_remote_create — 按项目文件夹创建远程仓库。
 * deps：{ workspaceRoot }
 */
export function toolRemoteCreate(deps) {
  const { workspaceRoot } = deps;
  return defineTool({
...
/**
 * git_set_visibility — 切换 GitHub 仓库公开/私有状态。
 * deps：{ workspaceRoot }
 */
export function toolSetVisibility(deps) {
  const { workspaceRoot } = deps;
  return defineTool({
...
/**
 * git_clone — 从 GitHub 远端 clone 仓库。
 * deps：{ workspaceRoot }
 */
export function toolClone(deps) {
  const { workspaceRoot } = deps;
  return defineTool({
    name: 'git_clone',
...
/**
 * git_rebuild_history — 重建 git 仓库历史。
 * deps：{ workspaceRoot }
 */
export function toolRebuildHistory(deps) {
  const { workspaceRoot } = deps;
  return defineTool({
    name: 'git_rebuild_history',
    description: '重建 git 仓库历史。squash-bugfixes=补丁并入主版本；drop-versions=删版本区间；fresh=当前文件树作为唯一提交（不改 package.json 版本号）。破坏性操作前打 backup tag。force=true 才覆盖远端（覆盖前须经明确授权，默认 false 只改本地）。dryRun=true 只预览。',
    parameters: {
      repo: { type: 'string', description: 'git 仓库绝对路径' },
      ...
      force: { type: 'boolean', description: 'true=覆盖远端当前分支（覆盖前须经明确授权）；默认 false 只改本地' },
    },
    output: { schema: { type: 'string' }, render: textRender },
    execute: async (params) => {
...
/**
 * git_push_rules — 管理 comment-wording 审计规则。
 * deps：{ env, workspaceRoot }
 */
export function toolPushRules(deps) {
  const { env, workspaceRoot } = deps;
  return defineTool({
    name: 'git_push_rules',
...
/**
 * push_permit_status — 查询「AI 回复推送许可」开关状态。
 * deps：{ workspaceRoot, permitCommitMessage }
 */
export function toolPermitStatus(deps) {
  const { workspaceRoot, permitCommitMessage } = deps;
  return defineTool({
...
/**
 * push_permit_config — 设置「AI 回复推送许可」开关。
 * deps：{ workspaceRoot }
 */
export function toolPermitConfig(deps) {
  const { workspaceRoot } = deps;
  return defineTool({
...
export function registerAgentTools(ctx, env, { commitWithAudit, auditRepoPath, fullScan }) {
  const { log, workspaceRoot, depth, extraRepos, extraReposFile, permitCommitMessage } = env;
  const deps = { env, workspaceRoot, depth, extraRepos, extraReposFile, permitCommitMessage, commitWithAudit, auditRepoPath, fullScan };

  ctx.inject(['tools'], (tctx) => {
    const tools = tctx.get('tools');
    if (!tools) return;

    tools.register(toolGitScan(deps));
    tools.register(toolCommitPush(deps));
    tools.register(toolCodeAudit(deps));
    tools.register(toolAuditFullScan(deps));
    tools.register(toolGenReadme(deps));
    tools.register(toolRemoteCreate(deps));
    tools.register(toolSetVisibility(deps));
    tools.register(toolClone(deps));
    tools.register(toolRebuildHistory(deps));
    tools.register(toolPushRules(deps));
    tools.register(toolPermitStatus(deps));
    tools.register(toolPermitConfig(deps));
