...
import { initAuditRuleset, applyRulePack } from './audit.js';
import { loadRulePack } from './rule-packs.js';
...
export async function resolvePluginEnv(ctx, config = {}) {
  ...
  return {
    ...
    exemptRepos, pushScopeCfg, permitCommitMessage,
    // 运行期可变字段（原 let 闭包语义 → env 属性；设置页/HTTP/工具改写，读方实时取值）
    customIgnorePatterns, hardcodeFullScan,
    injectFullSkill: config.injectFullSkill === true,
