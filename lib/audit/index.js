 ...
 */
import { collectTextFiles, readText, isGitRepo } from './collector.js';
...
/** 变动审计：git diff HEAD（仅 git 项目；非 git 项目退化为全量扫描）。 */
export function auditChanged(repoPath, opts = {}) {
  if (!isGitRepo(repoPath)) return auditFull(repoPath, opts);
  // v2 简化：变动范围先以 full 托底（git diff 解析放在 1.1.2 测试后的增强轮次）
