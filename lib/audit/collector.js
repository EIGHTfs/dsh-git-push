...
export function collectTextFiles(dir, { depth = 10, gitIgnoreRoot = null, includeIgnored = false, testExemptRoot = null } = {}) {
  ...
  if (!includeIgnored && gitIgnoreRoot) {
    ...
    if (existsSync(auditIgnorePath) && out.length) {
      try {
        const rels = out.map((f) => relative(gitIgnoreRoot, f.full).replace(/\\/g, '/'));
        const r = spawnSync(
        ...
        if (r.status === 0 || r.status === 1) {
          ...
          if (ignored.size) {
            for (let i = out.length - 1; i >= 0; i -= 1) {
              const rel = relative(gitIgnoreRoot, out[i].full).replace(/\\/g, '/');
              if (ignored.has(rel)) out.splice(i, 1);
            }
          }
        }
      } catch { /* 判定失败保留原结果 */ }
