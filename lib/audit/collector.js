...
function isGitWorkTree(dir) {
  ...
  let ok = false;
  try {
    const r = spawnSync('git', ['-c', 'core.quotepath=false', '-C', dir, 'rev-parse', '--is-inside-work-tree'], {
...
function tryLoadGitIgnoreSet(root) {
  ...
  let ignoreSet = null;
  try {
    const probe = spawnSync('git', ['-c', 'core.quotepath=false', '-C', root, 'rev-parse', '--is-inside-work-tree'], {
      encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], timeout: GIT_PROBE_TIMEOUT_MS,
    });
    ...
    if (allDirs.length) {
      const r = spawnSync('git', ['-c', 'core.quotepath=false', '-C', root, ...ignoreExtraArgs, 'check-ignore', '--stdin', '--no-index'], {
      ...
      if (r.status === 0 || r.status === 1) {
        ...
        for (const dirRel of allDirs) {
          const probeResult = spawnSync('git', ['-c', 'core.quotepath=false', '-C', root, ...ignoreExtraArgs, 'check-ignore', '-q', '--', dirRel], {
...
export async function collectTextFiles(dir, { depth = 10, gitIgnoreRoot = null, includeIgnored = false, testExemptRoot = null } = {}) {
  ...
  // 2026-09-22 修复（theme 全量审计漏忽略）：文件级判定门控从「存在 .auditignore」放宽为
  ...
  if (!includeIgnored && gitIgnoreRoot && out.length) {
    const relOf = (full) => relative(gitIgnoreRoot, full).replace(/\\/g, '/');
    if (isGitWorkTree(gitIgnoreRoot)) {
      // git 路径：批量喂 check-ignore（一次 spawn 判完，最省进程；.auditignore 有则叠加）
      const auditIgnorePath = join(gitIgnoreRoot, '.auditignore');
      const extra = existsSync(auditIgnorePath) ? ['-c', `core.excludesFile=${auditIgnorePath}`] : [];
      try {
        const rels = out.map((f) => relOf(f.full));
        const r = spawnSync(
          'git', ['-c', 'core.quotepath=false', '-C', gitIgnoreRoot, ...extra, 'check-ignore', '--stdin', '--no-index'],
          { input: rels.join('\n'), encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout: IGNORE_BATCH_TIMEOUT_MS },
        );
        if (r.status === 0 || r.status === 1) {
    ...
    } else if (existsSync(join(gitIgnoreRoot, '.auditignore'))) {
