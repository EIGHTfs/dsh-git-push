 ...
 *      并把清单写入插件配置目录 credentialsDir()/tools-index.md（仓库存一份，跨机可查；v1.40.0 原同级仓废除）
 *
...
import { credentialsDir } from './core.js';
...
export function formatToolsEnvInjection(tools) {
  ...
  const parts = ['【dsh-git-push 环境注入：工具安装路径】本机已探测工具（清单已同步到插件配置目录 git-push/tools-index.md）：'];
  if (found.length) {
...
/**
 * 把工具清单写入插件配置目录 credentialsDir()/tools-index.md（幂等；目录缺失自动创建）。
 * v1.40.0：原同级仓 dsh-git-push-User 废除；userDir 保留为旧参数别名（自定义目录）。
...
export function ensureToolsIndexFile({ workspaceRoot = '', userDir = '', tools = [] } = {}) {
  const dir = userDir || credentialsDir({ workspaceRoot });
  if (!dir) return { ok: false, file: '', error: '无法确定插件配置目录' };
