...
export function shellCdDynamicLines(text) {
  ...
  for (const ln of candidates) {
    ...
    if (/\|\|/.test(line.slice(m.index))) continue; // ④ 有兜底
    if (/&&/.test(line.slice(m.index))) continue; // ⑤ && 链（cd 失败即断链）
    out.add(ln);
  }
  return out;
}
...
function extractWriteTarget(line) {
  const m = /\b(writeFile|writeFileSync|appendFile|appendFileSync|createWriteStream|mkdir|mkdirSync)\s*\(\s*['"]([^'"]+)['"]/.exec(line);
  if (m && m[2]) return m[2];
