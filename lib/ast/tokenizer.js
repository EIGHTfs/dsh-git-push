  ...
  for (const raw of lines) {
    ...
    while (i < n) {
      ...
      if (ch === '/' && next !== '/' && next !== '*') {
        ...
        const prevKwRe = /^(?:return|typeof|case|in|of|new|delete|void|do|else|yield|await|instanceof)$/;
        const afterKeyword = prevTok && prevTok.type === 'ident' && prevKwRe.test(prevTok.value);
        if (!prevIsValue || afterKeyword) {
          let j = i + 1;
          let re = '/';
