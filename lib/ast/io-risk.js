...
import {
  ...
  SYNC_FS_FNS, ASYNC_FS_FNS, KIND_BY_FN, REQUEST_PATTERNS, LEVELS, raise, META_FS_FNS,
} from './io-risk-const.js';
import { buildCallGraph, isInRequestPath } from './callgraph.js';
...
function judgeIoTokend(ctx) {
  const { tokens, lines, idx, asyncFns, loops, within, enclosingFnText, enclosingFnName, promiseAllRanges, callGraph } = ctx;
  ...
  // 2026-09-23 P3：请求路径判定升级——函数体内 HTTP 特征（detectRequestContext）
  //   或「被请求路径函数调用」（调用链 isInRequestPath）——修「请求处理调用的公共函数漏报」
  const inRequest = detectRequestContext(lines, enclosingFnText(line))
    || (callGraph && enclosingFnName(line) ? isInRequestPath(enclosingFnName(line), callGraph) : false);
  const inStartup = !inAsync && !inLoop && !inRequest;
  const type = isSync ? 'sync' : 'async';
  const kind = META_FS_FNS.has(t.value) ? 'meta' : (KIND_BY_FN[t.value] || 'read'); // 2026-09-23：元数据/查询/幂等目录操作独立档
...
function gradeRisk({ isSync, inAsync, inLoop, inSmallFixedLoop, inRequest, inParallelAll, kind }) {
  ...
  // 2026-09-23（io-risk 优化，对照诊断「97% 误报」）：请求路径上的**元数据/查询/幂等目录操作**
  //   （existsSync/statSync/readdirSync/mkdir(recursive)/rmdir…）单文件微秒级、不搬运数据内容，
  //   不构成请求路径阻塞 → 降 low（不再按「请求路径 I/O」一律 medium）。
  //   注意：循环内的 meta 已被上方循环分级接管（循环内逐次 stat 累积按循环规则判），不受本档豁免。
  else if (inRequest && kind === 'meta') { risk = 'low'; reason = '请求处理路径上的元数据/查询/幂等目录操作（单文件微秒级，无数据内容搬运）'; }
  else if (inRequest) { risk = 'medium'; reason = '请求处理路径上的 I/O'; }
  else if (isSync) { risk = 'low'; reason = '启动路径上的同步 I/O（一次性初始化）'; }

  // 写/删加权一档（数据不可逆）——**仅当该次 I/O 会重复执行时**。
  ...
  //   2026-09-23：rename 同卷改名 = 元数据级（非数据内容搬运，DELETE 移 .trash 等）→ 不再参与加权。
  if (repeated && (kind === 'write' || kind === 'delete')) {
...
/** 从 openIdx（应为 `(`）起配对最近的闭合括号，返回闭括号 token 下标（无配对返回 -1）。 */
...
function findPromiseAllRanges(tokens) {
  ...
  for (let i = 0; i < tokens.length - 3; i++) {
    ...
    const isPromiseAll = a.type === 'ident' && a.value === 'Promise'
      && b.type === 'punct' && b.value === '.'
      && c.type === 'ident' && c.value === 'all'
      && d.type === 'punct' && d.value === '(';
    if (!isPromiseAll) continue;
    const close = findMatchingClose(tokens, i + 3);
    if (close > i + 3) ranges.push([i + 3, close]);
  }
  return ranges;
}
...
export function scanIoRiskAst(text = '') {
  ...
  const callGraph = buildCallGraph(src); // 2026-09-23 P3：单文件调用链——被请求路径函数调用的函数也判请求路径（修漏报）
  ...
  const fnNameAt = (line) => {
    let best = null;
    for (const [sf, ef] of allFns) if (line >= sf && line <= ef) { best = [sf, ef]; break; }
    if (!best) return '';
    const head = src.split('\n')[best[0] - 1] || '';
    const m = head.match(/(?:function\s+)?([A-Za-z_$][\w$]*)\s*(?:=\s*)?(?:\(|=>)/);
    return m ? m[1] : '';
  };
  const ctx = { tokens, lines, asyncFns, loops, within, enclosingFnText, enclosingFnName: fnNameAt, promiseAllRanges, callGraph };
