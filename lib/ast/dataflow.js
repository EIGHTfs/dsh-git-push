...
function scanClearThenAccess(tokens, from, to) {
  ...
  for (let i = from; i <= to; i++) {
    ...
    if (applyArgWriteback(tokens, i, nxt, obj, clearMap)) continue;
    if (applyAssignClearOrWriteback(tokens, i, to, t, nxt, obj, clearMap)) continue;
    const afterClear = applyClearMethods(t, member, memberIdx, callOpen, tokens, obj, clearMap, i);
    if (afterClear !== i) { i = afterClear; continue; }
    i = applyAccessAfterClear(t, nxt, member, memberIdx, callOpen, tokens, i, obj, clearMap, out);
  }
  return out;
}
