 ...
 * handler（handleViewer/handleCommit/...），每个 handler 经 deps 显式接收运行期共享依赖
...
/** /git-push/viewer 提交历史查看器页面（只读 GET）。 */
function handleViewer(req, res, deps) {
  ...
  const page = renderViewerPage({
    workspaceRoot: env.workspaceRoot, depth: env.depth, extraRepos: env.extraRepos,
    extraReposFile: env.extraReposFile, version: __VERSION__,
  });
  res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  res.end(page);
}

/** GET /api/git-push/repos — 扫描仓库（只读查看器）。 */
function handleRepos(req, res, url, deps) {
  ...
  return respond(res, { ok: true, count: repos.length, root: qRoot || env.workspaceRoot, repos });
}

/** GET /api/git-push/commits — 单仓库提交历史（只读查看器）。 */
function handleCommits(req, res, url, deps) {
  const { respond, env } = deps;
  const repoParam = url.searchParams.get('repo');
  if (!repoParam) return respond(res, { ok: false, error: { code: 'PARAM', message: '缺少 repo（仓库路径或名称）' } }, 400);
  const repoPath = resolveViewerRepo(repoParam, { root: env.workspaceRoot, depth: env.depth, extraRepos: env.extraRepos, extraReposFile: env.extraReposFile });
...
/** GET /api/git-push/diff — 单文件 diff（只读查看器）。 */
function handleDiff(req, res, url, deps) {
  const { respond, env } = deps;
  const repoParam = url.searchParams.get('repo');
...
export function registerHttpApi(ctx, env, { auditRepoPath, commitWithAudit, fullScan }) {
  ...
  ctx.inject(['webServer'], (wctx) => {
    ...
    webServer.register({
      ...
      handler: (req, res) => handleViewer(req, res, deps),
    });
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          if (p === '/api/git-push/repos' && m === 'GET') return handleRepos(req, res, url, deps);
          if (p === '/api/git-push/commits' && m === 'GET') return handleCommits(req, res, url, deps);
          if (p === '/api/git-push/diff' && m === 'GET') return handleDiff(req, res, url, deps);
