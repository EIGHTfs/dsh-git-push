...
export function extractComments(content, ext) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isC) {
      if (inBlock) {
        const end = t.indexOf('*/');
        out.push({ line: i + 1, text: t.replace(/^\*+\s?/, '').replace(/\*\/\s*$/, '').trim(), prevBlankOrComment, nextIsCode });
...
export function listTextFiles(repoPath, root = repoPath) {
  const out = [];
  let entries;
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      out.push(...listTextFiles(join(repoPath, e.name), root));
    } else if (e.isFile()) {
      ...
      out.push({ path: relative(root, p).split(sep).join('/'), ext, full: p });
    }
  }
  return out;
}
