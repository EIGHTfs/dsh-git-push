window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const en = {
      ...
      savedYamlMode: '✅ YAML check mode saved: next audit parses with the new mode (js-yaml real parse / heuristic loose line check)',
      accountTopHint: 'Account status (Token / SSH key check, auto-runs on opening settings)',
      sshEmail: 'SSH email (for key generation)',
      sshEmailHint: 'e.g. your-name@example.com. Generates ssh-rsa 4096 keypair; private key stays local, copy the public key to GitHub → Settings → SSH and GPG keys → New SSH key.',
      genKey: 'Generate key',
    ...
    function GitPushRuleCards(props) {
      ...
      // 不再硬编码——新增规则 yml 重启后自动出现在此卡。缺省回退内置映射（防老配置/host 未注入）。
      ...
      inject() {
        return {
          ...
          saveQualityWeight: (dim, weight) => {
            ...
            this.publish();
            void this.scope.set('qualityWeights', next).then(() => this.flashSaved('savedQualityWeight')).catch(() => { this.failed = true; this.publish(); });
          },
          editSshEmail: (text) => {
