...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return jsx.jsxs('div', {
        ...
        children: [
          ...
          jsx.jsxs('div', {
            ...
            children: [
              // 2026-09-21：commit 按钮与 push 并排——本地提交（复用提交推送的本地提交审计，
              ...
              jsx.jsx('button', {
                ...
                children: busy === 'push:' + r.path ? '推送中…' : 'push',
              }),
          ...
          jsx.jsxs('div', {
            ...
            children: [
              ...
              jsx.jsx('button', {
                ...
                className: 'dshgp_keybtn',
                disabled: visBusy,
                // 2026-09-20：二次确认改为**独立弹窗**（Controller.visConfirm + 全局确认面板），
    ...
    function dshgp_RepoCloudPane(props) {
      const s = props.state;
      const rows = (s.cloudRepos || []).map((r) => jsx.jsx(dshgp_RepoCloudRow, { key: r.fullName, repo: r, state: s, onClone: props.cloneFlow, requestVisSwitch: props.requestVisSwitch })); // 2026-09-23 bugfix：传参名写错（visSwitch→requestVisSwitch），父 props 无 visSwitch，按钮点击 = undefined() 没反应
      return jsx.jsxs('div', {
        className: 'dshgp_repanepane',
        children: [
          jsx.jsxs('div', {
          ...
          s.visConfirm ? dshgp_VisConfirmDialog({
            p: s.visConfirm,
            busy: s.repoBusy === 'vis:' + s.visConfirm.fullName,
    ...
    class dshgp_Controller {
      ...
      /** 2026-09-21：本地仓库手动 commit（只提交不推送）——复用提交推送的本地提交审计。
      ...
      async commitLocalRepo(path) {
        ...
        try {
          ...
          const d = (res && res.ok !== undefined) ? res : (res && res.data ? res.data : res);
          if (d && d.ok) {
            ok = true;
            const w = d.warningCount || 0;
            msg = '✅ 已提交' + (d.commitSha ? ' ' + d.commitSha : '') + (w ? '（⚠️ ' + w + ' 条 warning，见审计）' : '');
          } else if (d && d.blocked) {
            const list = (d.blockers && d.blockers.length ? d.blockers.slice(0, 2).join('；') : (d.error || '审计拦截'));
            msg = '🚫 审计拦截：' + list;
          } else {
            msg = '⚠️ ' + (d && d.error ? d.error : '提交失败');
          }
          this.localMsg = msg;
        } catch (e) {
      ...
      inject() {
        return {
          ...
          commitLocalRepo: (path) => { void this.commitLocalRepo(path); }, // 2026-09-21：本地 commit（复用提交审计）
