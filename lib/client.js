window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const zh = {
      ...
      sectionGuide: 'Token 与 SSH 公钥的填写、检测、保存都在插件配置里进行，本页只读展示登录状态。',
      sectionGoConfig: '去插件配置填写 →',
      sectionGoConfigHint: '侧边栏 → 设置 → 插件 → 插件配置 → Git 提交推送',
    ...
    const en = {
      ...
      sectionGoConfig: 'Go to plugin config →',
      sectionGoConfigHint: 'Sidebar → Settings → Plugins → Plugin config → Git push',
    ...
    const cssText = [
      ...
      '.dshgp_section_go{font:inherit;font-size:13px;border-radius:8px;padding:8px 12px;cursor:pointer;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);border:.5px solid var(--dsw-alias-border-l4);align-self:flex-start}',
      '.dshgp_section_go:hover{background:var(--dsw-alias-bg-layer-3)}',
    ...
    function GitPushRuleCards(props) {
      const state = props.state;
      if (!state.available) return null;
      // 规则槽位：可排序四身份（nodejs/frontend/comment/template）+ 强制槽位 private（v1.48.0 私密拦截，不可排序）
      const SLOT_NAMES = { nodejs: 'Node.js 规则', frontend: '前端 HTML 规则', comment: '关键词规则', template: 'template 模板（空）', private: '私密文件拦截（强制）' };
      const order = Array.isArray(state.ruleOrder) && state.ruleOrder.length ? state.ruleOrder : ['nodejs', 'frontend', 'comment'];
                ...
                jsx.jsxs('div', {
                  ...
                  children: [
                    ...
                    jsx.jsxs('div', {
                      ...
                      children: [
                        jsx.jsx('span', { className: 'dshgp_section_go', children: t('sectionGoConfig') }),
