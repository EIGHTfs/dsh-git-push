window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    function GitPushRuleCards(props) {
      ...
      const SLOT_NAMES = { nodejs: 'Node.js 规则', frontend: '前端 HTML 规则', comment: '关键词规则', dsh: 'dsh 插件审计（v1.50.0）', npm: 'npm 发布审计（v1.51.0）', template: 'template 模板（空）', private: '私密文件拦截（强制）' };
      // v1.50.0：dsh 槽位加入默认顺序；v1.51.0：npm 槽位加入。存量用户若之前排序过（ruleOrder 不含新槽位）自动补在末尾，保证新规则默认生效
      let order = Array.isArray(state.ruleOrder) && state.ruleOrder.length ? [...state.ruleOrder] : ['nodejs', 'frontend', 'comment', 'dsh', 'npm'];
      for (const add of ['dsh', 'npm']) {
        if (order.length && !order.includes(add)) {
