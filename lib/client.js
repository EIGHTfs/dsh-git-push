...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const dshgp_css = [
      ...
      '.dshgp_visconfirm{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:5px 8px;border-radius:8px;background:color-mix(in srgb,var(--dsw-alias-label-warning) 9%,transparent);border:.5px solid color-mix(in srgb,var(--dsw-alias-label-warning) 30%,transparent)}',
    ...
    function dshgp_AccountTab(props) {
      // 2026-09-16：账号信息/本地仓库列表的读取时机已收敛到三种（不再每次切 tab 重读）：
    ...
    // 2026-09-14 移植 gamebanana-mods-downloader 的 path-picker 小模块：一行接入 📂 按钮 // 当前打开的输入框（null = 走 onPick 回调）
          ...
          jsx.jsxs('div', {
            ...
            children: [
              ...
              jsx.jsx('button', {
                type: 'button',
                className: 'dshgp_keybtn',
                // 2026-09-16：扫描中禁用，不可重复点击（后台独立进程扫描链路）
    ...
    /** 云端面板单行：仓库名 + 私有/公开/分支/更新时间 + clone 按钮 + 可见性切换（1.5.4，二次确认）。 */
    function dshgp_RepoCloudRow(props) {
      const r = props.repo;
      const s = props.state;
      const busy = s.repoBusy === 'clone:' + r.fullName;
      const visBusy = s.repoBusy === 'vis:' + r.fullName;
      // 二次确认态：confirmVis = 待切换目标（'public'|'private'），空 = 未确认
      const [confirmVis, setConfirmVis] = react.useState('');
      const [visMsg, setVisMsg] = react.useState('');
      const target = r.private ? 'public' : 'private';
      const targetLabel = target === 'public' ? '公开' : '私有';
      const curLabel = r.private ? '私有' : '公开';
      // 2026-09-16：云端仓库标记本地是否已有（后端 repos-cloud 附 localExists/localPath）
      const localTag = r.localExists
        ? jsx.jsx('span', { className: 'dshgp_pill dshgp_pill_on', children: '本地已有' })
        : null;
      const confirmBar = confirmVis
        ? jsx.jsxs('div', { className: 'dshgp_visconfirm', children: [
            jsx.jsx('span', { className: 'dshgp_reprowmeta', children: ['确认将 ' + r.fullName + ' 从「' + curLabel + '」改为「' + (confirmVis === 'public' ? '公开' : '私有') + '」？改公开有敏感信息暴露风险。'] }),
            jsx.jsx('button', {
              type: 'button', className: 'dshgp_keybtn',
              disabled: visBusy,
              onClick: async () => {
                const res = await props.visSwitch(r.fullName, confirmVis);
                setVisMsg(res && res.ok ? '✅ 已切换为' + (res.visibility === 'private' ? '私有' : '公开') : '❌ ' + ((res && res.error) || '失败'));
                setConfirmVis('');
              },
              children: visBusy ? '切换中…' : '确认切换',
            }),
            jsx.jsx('button', {
              type: 'button', className: 'dshgp_keybtn',
              disabled: visBusy,
              onClick: () => { setConfirmVis(''); setVisMsg(''); },
              children: '取消',
            }),
          ] })
        : null;
        ...
        children: [
          jsx.jsxs('div', {
            ...
            children: [
              ...
              localTag,
              visMsg ? jsx.jsx('span', { className: visMsg.startsWith('✅') ? 'dshgp_fb dshgp_fbok' : 'dshgp_fb dshgp_fbfail', children: visMsg }) : null,
            ],
          }),
          jsx.jsxs('div', {
            className: 'dshgp_pushcol',
            children: [
              ...
              jsx.jsx('button', {
                ...
                className: 'dshgp_keybtn',
                disabled: visBusy,
                onClick: () => { setConfirmVis(target); setVisMsg(''); },
    ...
    function dshgp_RepoCloudPane(props) {
      const s = props.state;
      const rows = (s.cloudRepos || []).map((r) => jsx.jsx(dshgp_RepoCloudRow, { key: r.fullName, repo: r, state: s, onClone: props.cloneFlow, visSwitch: props.visSwitch }));
      return jsx.jsxs('div', {
        className: 'dshgp_repanepane',
        children: [
          jsx.jsxs('div', {
    ...
    /** 注入总开关块（2026-09-13 新增，2026-09-20 语义扩展）：控制 systemPrompt 段 + 上下文注入启停。 */
    ...
    /** 推送与默认值区块（2026-09-16 收敛：只保留推送通道；默认扫描路径复用本地仓库列表选择路径）。 */
      ...
      return jsx.jsxs('div', {
        ...
        children: [
          ...
          jsx.jsxs('div', { className: 'dshgp_field', children: [
            ...
            // 2026-09-16：下拉标签带括号注释（选项 value 仍是 ssh/api/auto，不改值——
          ...
          // 2026-09-17：推送门禁（类似会话指挥家写操作拦截）——开启后 AI 推送必须
    ...
    class dshgp_Controller {
      constructor(scope) {
        ...
        // 注入总开关（默认开；2026-09-13 新增，2026-09-20 扩展为系统提示词+上下文）
        ...
        // 2026-09-16：凭据有效性快照（token/ssh 各自 valid/login/checkedAt）——来自 account-status.json
        ...
        this.localScanning = false; // 2026-09-16：独立进程扫描进行中（按钮禁用，不可重复扫描）
        this.localMsg = '';
        this.cloudRepos = [];
      ...
      project() {
        ...
        return {
          ...
          // 2026-09-16：凭据有效性快照（account-check → account-status.json 的 token/ssh 分项），
      ...
      async scanLocalRepos(path, rebuild, { silent = false } = {}) {
        ...
        this.localLoading = false;
        this.publish();
        // 2026-09-16：读取/扫描收尾后**自动补查未知远端状态**——列表里 liveSkipped（本地列表预算/
      ...
      /** 手动「扫描」= 独立进程后台离线扫描 + 只读新增 diff（2026-09-16）。
      ...
      async loadCloudRepos({ silent = false } = {}) {
        ...
        try {
          ...
          if (cloudRes && cloudRes.ok) {
            ...
            // 2026-09-16：云端获取后**重新读索引刷新本地列表**——后端已把云端仓库（含
      ...
      /**
       * 云端仓库私有/公开切换（1.5.4）：POST repo-visibility（confirm 前置后端校验）。
       * 成功后就地更新列表该项（云端真源返回值），并给行内反馈。
      ...
      async switchVisibility(fullName, target) {
        ...
        if (res && res.ok) {
          ...
          for (const item of arr) {
            if (String(item.fullName || '') === fullName) item.private = res.visibility === 'private';
          }
          this.cloudRepos = arr;
       ...
       * 2026-09-20：关 = host 侧 systemPrompt 段全部返回空串 + 上下文注入不注入；
      ...
      inject() {
        return {
          ...
          visSwitch: (fullName, target) => this.switchVisibility(fullName, target), // 云端私有/公开切换（1.5.4）
    ...
    function apply(ctx) {
      ...
      // 2026-09-16（修复「重启后不预读」）：**每次进设置页都预读**账号信息 + 本地仓库列表
      ...
      //   没选过则空 → 后端自动识别 DSH 家根。
      let savedPath = '';
      try { savedPath = (typeof window !== 'undefined' && window.localStorage && window.localStorage.getItem('dshgp-scan-path')) || ''; } catch { /* 忽略 */ }
