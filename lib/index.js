 ...
 *   plugin-audit.js         审计服务：auditRepoPath / fullScan / isUserRepoPath
 ...
 *   plugin-http.js          HTTP API：/api/git-push/*（含 runGitVersion）
...
/** 启动 git 全局配置：core.filemode false（CIFS 可执行位噪声）+ safe.directory=*（均失败不阻断启动）。 */
function setupGitGlobals(log) {
  // v1.18.4：core.filemode false（CIFS 可执行位噪声）
  try {
...
/** 凭据目录收敛：目录随实例走，不入 git（v1.40.0 B1/B5）。 */
function ensureCredentialsDir(workspaceRoot, log) {
  try {
...
export async function apply(ctx, config = {}) {
  ...
  // v1.18.4：git 全局配置（filemode false / safe.directory=*）+ v1.40.0 凭据目录收敛
  setupGitGlobals(log);
  ensureCredentialsDir(workspaceRoot, log);

  // 推送许可：session/event turn/end 防抖监听 + 定时器 effect 清理（拆分前位于 commitWithAudit 定义之后）
  permit.registerSessionListener(ctx);

  // HTTP API：/api/git-push/*（webServer 服务存在时）
  registerHttpApi(ctx, env, { auditRepoPath: auditor.auditRepoPath, commitWithAudit: commitFlow.commitWithAudit, fullScan: auditor.fullScan });
  ...
  registerAgentTools(ctx, env, { commitWithAudit: commitFlow.commitWithAudit, auditRepoPath: auditor.auditRepoPath, fullScan: auditor.fullScan });
}
