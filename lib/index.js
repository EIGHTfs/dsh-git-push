...
import { scanRepos, readExtraReposFile, commitAndPush, getDiff, scanSensitiveFiles, ensureSensitiveIgnored, genReadme, rebuildHistory, previewRebuildHistory, listVersionCommits, resolveGitToken, ensureRemoteRepo, loadUserRequirements, cloneViaApi, detectRepoVisibility, setRepoVisibility } from './core.js';
...
export async function apply(ctx, config = {}) {
  ...
  ctx.inject(['tools'], (tctx) => {
    ...
    tools.register(defineTool({
      ...
      description: '对指定 git 仓库一键提交并推送：先审计（默认开，L0 静态检查语法/敏感信息/凭据/大文件/文档对话类措辞，发现严重问题拦截），再扫描敏感字段(cookie/device/username/password/token)自动加 .gitignore，再 git add -A → commit（message 必填）→ push origin <当前分支>。push 前自动 fetch 并检查 ahead/behind，远端领先时不推。repo 传仓库绝对路径（可用 git_scan 查）。audit=false 可关闭审计；llmAudit=true 追加 LLM 深度审查（需配置 llmAuditProvider/Model）。dryRun=true 只模拟不写入。调用时若插件 User/<GitHub用户名>/ 存在开发者特殊要求清单，需先逐条核对达标并传 requirementsConfirmed=true，否则拦截。',
      parameters: {
        ...
        requirementsConfirmed: { type: 'boolean', description: '已核对 User/<用户名>/ 开发者特殊要求（v1.10.0）：存在要求清单时必须 true，false 会被拦截' },
      },
      output: { schema: { type: 'string' }, render: textRender },
    ...
    tools.register(defineTool({
      ...
      description: '按项目文件夹创建远程仓库：对本地 git 仓库（repo 传仓库绝对路径，如 /vol02/.../workspace/xxx）取目录名做仓库名，检查 GitHub 是否已存在同名仓库（owner=EIGHTfs），不存在则用 GitHub token 自动创建（visibility=private/public，默认 private），并设置 origin（SSH 443 端口）。dryRun=true 只探测预演不写 remote 不调创建 API。token 自动探测：项目内 .git-push-token / workspaceRoot data/sensitive/github-token。',
      parameters: {
    ...
    tools.register(defineTool({
      ...
      description: '切换 GitHub 仓库公开/私有状态：对本地 git 仓库（repo 传仓库绝对路径）调 GitHub API PATCH /repos/{owner}/{repo} 的 private 字段，支持 public ↔ private 双向切换。改 public 有敏感信息暴露风险（公开后任何人可看仓库内容，先确认无凭据/隐私），改 private 安全。token 自动探测（优先插件 User/<用户名>/github-token）。成功后可用 git_scan 或 code_audit 确认。',
      parameters: {
        repo: { type: 'string', description: '本地 git 仓库绝对路径（项目文件夹）' },
    ...
    // v1.13.0 远端 clone（默认走 api.github.com，github.com 直连被阻断时仍可拉取）
    tools.register(defineTool({
      name: 'git_clone',
      description: '从 GitHub 远端 clone 仓库到本地（默认走 api.github.com tarball 通道，github.com 直连被网络阻断时仍可用；自动探测远端默认分支 master/main）。target 传 owner/repo 或完整 URL（https://github.com/o/r.git / git@github.com:o/r.git / ssh://git@ssh.github.com:443/o/r.git）。dest 传目标目录绝对路径（缺省放 workspaceRoot），已存在非空目录会拒绝防覆盖。branch 可选指定分支。在 /tmp 中转建仓后整拷回目标，兼容 CIFS 卷。',
      parameters: {
    ...
    log.info('工具已注册: git_scan / git_commit_push / code_audit / git_gen_readme / git_rebuild_history');
  });
}
