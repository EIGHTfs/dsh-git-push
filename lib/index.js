 ...
 *   可见性来自 GitHub API（token）。权威源 = 同级仓 dsh-git-push-User/<owner>/dsh-repo-index.json。
...
import { scanRepos, readExtraReposFile, commitAndPush, getDiff, scanSensitiveFiles, ensureSensitiveIgnored, genReadme, rebuildHistory, previewRebuildHistory, listVersionCommits, resolveGitToken, resolveValidGitToken, ensureRemoteRepo, loadUserRequirements, cloneViaApi, detectRepoVisibility, setRepoVisibility, ensureUserRepoSibling, USER_REPO_NAME, ensureGlobalFilemodeFalse, ensureGlobalSafeDirectoryStar, persistGithubToken, persistSshPub, githubTokenStatus, formatRemoteHeadsTable, checkGithubAccount, formatGithubAccountBlock, collectRepoSkillDocs, formatRepoSkillInjection, collectRepoSkillDirs, formatRepoSkillDirsInjection, runGit, resolveUserDir, hasFileHeaderExempt, collectFunctionManual, generateSshKey, ensureAuxSshRemote, buildReadmeCheckHint } from './core.js';
...
export async function apply(ctx, config = {}) {
  ...
  async function maintainRepoIndex() {
    ...
    try {
      ...
      const { dir: userDir, user: owner } = resolveUserDir({ workspaceRoot, extraRepos, extraReposFile });
      if (!userDir) return { ok: false, error: '同级仓 dsh-git-push-User 未就绪，无法写索引' };
      // 手工可见性基线：从现有 JSON（或旧 md）解析，GitHub API 查不到时回退
      let existing = '';
      const defaultTarget = join(userDir, owner, 'dsh-repo-index.json');
      const legacyMd = join(userDir, owner, 'dsh-repo-index.md');
      const flatJson = join(userDir, 'dsh-repo-index.json');
      for (const p of [repoIndexSyncTarget || defaultTarget, flatJson, legacyMd]) {
      ...
      // 【改为】配置缺失时用 resolveGitToken 自动探测（User 仓 github-token 等），保证能查到真实可见性。
  ...
  /** 识别 user 仓（dsh-git-push-User 或任意 -User 结尾目录）：审计特殊照顾——自动确保私有。 */
  function isUserRepoPath(repoPath) {
    try {
      const base = (repoPath || '').replace(/[\\/]+$/, '');
      const name = base.split(/[\\/]/).pop() || '';
      return name === USER_REPO_NAME || /-User$/i.test(name);
    } catch { return false; }
  }
  ...
  async function auditRepoPath(repoPath, { forceLlm = false } = {}) {
    ...
    const result = auditRepo(repoPath, { blockOn: effectiveBlockOn, exemptRepos: effectiveExemptRepos, commentWordingPatterns: commentWordingRules, hardcodeFullScan });
        ...
        try {
          ...
          if (p === '/api/git-push/status' && m === 'GET') {
            return respond(res, {
              ...
              repoIndex: { enabled: repoIndexEnabled, tokenPath: repoIndexTokenPath || '(未配置)', syncTarget: repoIndexSyncTarget || '(自动: dsh-git-push-User/<owner>/dsh-repo-index.json)', injectFull: injectRepoIndexFull },
              git: runGitVersion(),
            });
          }
  ...
  ctx.inject(['tools'], (tctx) => {
    ...
    tools.register(defineTool({
      name: 'code_audit',
      description: '对指定 git 仓库执行代码审计（默认 L0 静态检查：语法/JSON/YAML/敏感信息/凭据入库/二进制大文件/debugger 残留 + 代码质量维度：函数行数/静默catch/async同步阻塞/测试覆盖 + 0-100 评分与 A-D 等级，标准见 docs/code-quality-checklist.yaml）。llm=true 时追加 LLM 深度审查（需配置 llmAuditProvider/Model）。repo 传仓库绝对路径。返回问题清单（blocker 拦截级 / warning 提醒级）、quality 评分与是否通过。',
      parameters: {
        repo: { type: 'string', description: 'git 仓库绝对路径' },
