...
export async function apply(ctx, config = {}) {
  ...
  // ══════════════════════════════════════════════════════════════════════
  // 【上下文注入入口】agent/pre-step 钩子（本插件的全部上下文注入都在这里）
  //   查找线索：搜「上下文注入」「pre-step」「collectRepoSkillDocs / collectRepoSkillDirs」
  //   注入内容（按顺序追加为一条 user 消息，仅首次注入，避免每步重复烧 token）：
  ...
  //            即 v1.23.x 时期行为，2026-09-07 按恢复为可选开关）
  //        · 默认（false）→ 只注入 skill 目录 + 文件清单（collectRepoSkillDirs + formatRepoSkillDirsInjection），
  //          正文由 AI 按需读取，省 token
  //     2) 环境注入（v1.26.0）：工作目录映射 + 工具安装路径（envInjectionEnabled 可关）
  ...
  const injectFullSkill = config.injectFullSkill === true; // 设置页开关：注入全部 skill 正文
