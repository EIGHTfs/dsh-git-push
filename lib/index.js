...
export async function apply(ctx, config = {}) {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    const entry = { githubToken: '', tokenConfigured: tok.configured };
    ...
    scope.watch(async (next) => {
      ...
      if (!saved.ok) {
        log.warn(`GitHub token 写入失败: ${saved.error}`);
        return;
      }
      log.info(`GitHub token 已写入 ${saved.source}`);
      try {
        await scope.replace({ tokenConfigured: true });
      } catch (e) {
        log.warn(`清掉 settings 里的 token 明文失败: ${e?.message || e}`);
      }
    });
  });
    ...
    tools.register(defineTool({
      ...
      execute: async (params) => {
        ...
        if (Array.isArray(heads) && heads.length) {
          result.remoteHeadsText = heads.map((h, i) => `${i + 1}. ${h.sha}  ${h.title}  ${h.time}`).join('\n');
        }
