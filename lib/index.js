...
export async function callTool(toolName, args = {}, env = {}, cfg = defaultConfig(), log = null) {
  switch (toolName) {
    ...
    case 'code_audit': {
      ...
      return { ok: true, scope: res.scope, summary, quality, block, findings: res.findings };
    }
    case 'git_gen_readme': {
 ...
 */
export function listRuleSlots(order, disabledSlots = []) {
  ...
  for (const slot of discovered) {
    ...
    if (r.ok && r.data?.metadata) {
      // 2026-09-12：meta 顺带统计拦截/警告/通过数量（前端规则包行直接显示，免逐个拉 rule-detail）
      const rules = Array.isArray(r.data.rules) ? r.data.rules : [];
      const blocker = rules.filter((x) => x && (x.severity === 'blocker' || x.severity === 'error')).length;
      ...
      meta[slot] = {
        ...
        stats: { blocker, warning, pass: Math.max(0, rules.length - blocker - warning), total: rules.length },
        // 2026-09-13：disabled 以 yml 顶层为准（用户指示「启用/禁用以 yml 解析，不单独存变量」）。
        //   disabledSlots 参数仅向后兼容（旧配置/旧调用仍生效），新切换走 toggle-rule 写 yml。
        disabled: r.data.disabled === true || disabledSlots.includes(slot),
      };
    } else {
      meta[slot] = { name: slot, description: '', author: '', stats: { blocker: 0, warning: 0, pass: 0, total: 0 }, disabled: disabledSlots.includes(slot) };
    }
  }
...
export async function handleHttp(req = {}, env = {}, cfg = defaultConfig()) {
  ...
  switch (path) {
    ...
    case '/api/git-push/rule-slots':
      return { status: 200, body: { ok: true, slots: listRuleSlots(cfg.auditRuleOrder, Array.isArray(cfg.auditDisabledSlots) ? cfg.auditDisabledSlots : []) } };
    ...
    case '/api/git-push/toggle-rule': {
      // 2026-09-13：启用/禁用规则包 = 直接改 yml 顶层 disabled（用户指示「以 yml 解析，不单独存变量」）。
