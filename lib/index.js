/**
 * dsh-git-push — git 自动提交推送插件 v1.36.0（内置代码审计门禁 + dsh-repo-index 自动维护）
 *
...
import { scanRepos, readExtraReposFile, commitAndPush, getDiff, scanSensitiveFiles, ensureSensitiveIgnored, genReadme, rebuildHistory, previewRebuildHistory, listVersionCommits, resolveGitToken, resolveValidGitToken, ensureRemoteRepo, loadRequirements, cloneViaApi, detectRepoVisibility, setRepoVisibility, ensureGlobalFilemodeFalse, ensureGlobalSafeDirectoryStar, persistGithubToken, persistSshPub, githubTokenStatus, formatRemoteHeadsTable, checkGithubAccount, formatGithubAccountBlock, collectRepoSkillDocs, formatRepoSkillInjection, collectRepoSkillDirs, formatRepoSkillDirsInjection, runGit, hasFileHeaderExempt, collectFunctionManual, generateSshKey, ensureAuxSshRemote, buildReadmeCheckHint, credentialsDir, getDefaultGithubOwner, setDefaultGithubOwner } from './core.js';
import { auditRepo, cleanCommentWording, initAuditRuleset, applyRulePack, getAuditRuleset } from './audit.js';
import { getCompiledRulePack, loadRulePack } from './rule-packs.js';
...
import { buildRepoIndex, syncRepoIndex, detectSkillsDir, parseManualVisibility, formatRepoIndexInjection } from './repo-index.js';
...
export const Config = z.object({
  ...
  // v1.41.0 审计规则包来源：''/'builtin'/'eightfs'=内置 EIGHTfs 包；本地绝对路径=file；http(s)://=在线包
...
export async function apply(ctx, config = {}) {
  ...
  // v1.41.0（C3）：审计规则包来源——'' / 'builtin' / 'eightfs' = 内置 EIGHTfs 包；
  // 本地绝对路径 = file 来源；http(s):// = url 来源（启动后异步装载热替换）
  const auditRulesetChoice = typeof config.auditRuleset === 'string' ? config.auditRuleset.trim() : '';
  const auditRuleset = initAuditRuleset(auditRulesetChoice);
  if (auditRuleset.errors?.length) {
    log.warn(`审计规则包装载告警: ${auditRuleset.errors.join('；')}`);
  }
  log.info(`审计规则包: ${auditRuleset.meta.name}@${auditRuleset.meta.version} (owner=${auditRuleset.meta.owner}, source=${auditRuleset.meta.source}, secret=${auditRuleset.secretPatterns.length} wording=${auditRuleset.wordingPatterns.length} docConv=${auditRuleset.docConvPatterns.length})`);
  if (auditRulesetChoice && /^https?:\/\//i.test(auditRulesetChoice)) {
    // url 来源：启动后异步装载完成即热替换（loadRulePack 内置 8s 超时与 2MB 上限）
    loadRulePack(auditRulesetChoice).then((r) => {
      if (!r.ok) { log.warn(`在线规则包装载失败，继续用内置包: ${r.error}`); return; }
      applyRulePack(r.pack, { source: 'url' });
      log.info(`在线规则包已生效: ${r.pack.name}@${r.pack.version} (owner=${r.pack.owner})`);
    }).catch(() => { /* 保持内置包 */ });
  }
  ...
  let hardcodeFullScan = config.hardcodeFullScan === true;
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    const entry = { githubToken: '', sshPub: '', tokenConfigured: tok.configured, commentWordingEnabled: true, commentWordingCustom: '', commentWordingRulesFile: '', commentWordingRulesUrl: '', envInjectionEnabled: true, envInjectionTools: '', injectFullSkill: false, injectRepoIndexFull: false, customIgnorePatterns: '', hardcodeFullScan: false };
    ...
    scope.watch(async (next) => {
      ...
      if (next && typeof next.injectRepoIndexFull === 'boolean') {
        injectRepoIndexFull = next.injectRepoIndexFull;
        log.info(`injectRepoIndexFull 设置已更新 → ${injectRepoIndexFull}（设置页即时生效）`);
      }
      ...
      if (next && typeof next.hardcodeFullScan === 'boolean') {
        hardcodeFullScan = next.hardcodeFullScan;
        log.info(`hardcodeFullScan 设置已更新 → ${hardcodeFullScan}（硬编码审计${hardcodeFullScan ? '全量扫' : '只扫新增行'}）`);
      }
  ...
  //            缓存过期时同步 tools-index.md 到插件配置目录（v1.40.0 原同级仓废除）。
  //        A3) 提交前 README 检查（v1.32.0）——每次调用 git_commit_push 前核对 README。
  //     B) agent/pre-step 钩子（user 消息，仅首次注入，避免每步重复烧 token）：
  //        1) 两仓 skill 注入（dsh-git-push/skills + 技能仓库 ai-work-archive/skills 的 git-workflow .md，v1.40.0）
  ...
  //             正文由 AI 按需读取，省 token
  //        2) dsh-repo-index JSON（v1.35.0）：默认只注入文件名；injectRepoIndexFull=true 注入正文。md 表格已废弃。
  //        3) 设备/用户 json 脱敏注入（v1.27.0）：只注入账号/站点清单，不注入密码明文
  ...
  let injectRepoIndexFull = config.injectRepoIndexFull === true; // v1.35.0：true=注入 JSON 正文，默认只注入文件名
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    try {
      const idxText = formatRepoIndexInjection({
        workspaceRoot,
        syncTarget: repoIndexSyncTarget,
        full: injectRepoIndexFull,
      });
      if (idxText) {
        blocks.push(idxText);
        log.info(`已注入 dsh-repo-index.json（full=${injectRepoIndexFull}）`);
      }
    } catch (e) {
  ...
  // v1.40.0（B1/B5）：凭据收敛进插件配置目录 credentialsDir()，原同级仓 dsh-git-push-User 启动 clone 废除。
  // token/SSH 密钥经设置页写入（persistGithubToken/persistSshPub）；目录随实例走，不入 git。
  try {
  ...
  async function auditRepoPath(repoPath, { forceLlm = false, ruleset } = {}) {
    ...
    const result = auditRepo(repoPath, { blockOn: effectiveBlockOn, exemptRepos: effectiveExemptRepos, commentWordingPatterns: commentWordingRules, hardcodeFullScan, ruleset: rulesetOverride });
        ...
        try {
          ...
          if (p === '/api/git-push/status' && m === 'GET') {
            return respond(res, {
              ...
              audit: { auditEnabled, blockOn, llmAudit: llmAuditOn, llmAuditProvider: llmAuditProvider || '(未配置)', llmAuditModel: llmAuditModel || '(未配置)', exemptRepos, hardcodeFullScan },
              ...
              repoIndex: { enabled: repoIndexEnabled, tokenPath: repoIndexTokenPath || '(未配置)', syncTarget: repoIndexSyncTarget || `(自动: ${join(credentialsDir({ workspaceRoot }), 'dsh-repo-index.json')})`, injectFull: injectRepoIndexFull },
              git: runGitVersion(),
            });
          }
