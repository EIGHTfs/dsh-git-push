...
export async function handleHttp(req = {}, env = {}, cfg = defaultConfig()) {
  ...
  switch (path) {
    ...
    case '/api/git-push/account-check': {
      ...
      const b = req.body || {};
      if (b.sshPub) persistSshPub(String(b.sshPub), { workspaceRoot: env.workspaceRoot });
      const result = await checkGithubAccount({ workspaceRoot: env.workspaceRoot, token: String(b.githubToken || '') });
