...
export const Config = {
  ...
  extraReposFile: { type: 'string', default: '', description: '额外仓库清单文件（每行一个绝对路径，# 注释；运行时实时读取）' },
  ...
  auditLevel: { type: 'string', default: 'standard', description: '审计强度：quick | standard | deep（默认 standard）' },
  auditRuleset: { type: 'string', default: '', description: '自定规则目录（空=内置规则包；放 audit-rules-<名>.yml 即整体替换）' },
  weightOverrides: { type: 'string', default: '', description: '权重覆盖 JSON（如 {"安全性":100}，空=默认权重表）' },
  ...
  return [
    ...
    { name: 'git_commit_push',
      description: '对指定 git 仓库一键提交并推送：先审计（默认开，L0 静态检查语法/敏感信息/凭据/大文件/文档对话类措辞，'
        ...
        + 'audit=false 可关闭审计；dryRun=true 只模拟不写入。',
      parameters: {
...
export async function apply(ctx, config = {}) {
  ...
  ctx?.log?.info?.(`dsh-git-push v${VERSION} 已接线（工具 ${toolCount} 个 / 注入 ${sectionCount} 段 / HTTP ${httpWired ? '已注册' : '未注册'}；审计默认${cfg.auditEnabled ? '开' : '关'}）`);
  return { ok: true, version: VERSION, tools: toolCount, sections: sectionCount, http: httpWired };
}
...
export async function callTool(toolName, args = {}, env = {}, cfg = defaultConfig()) {
  switch (toolName) {
    ...
    case 'git_commit_push': {
      ...
      if (audit) {
        const res = auditWithScope(repo, { scope: cfg.auditScanScope || 'diff', auditLevel: cfg.auditLevel || 'standard', rulesetDir: cfg.auditRuleset || '' });
    ...
    case 'code_audit': {
      ...
      const auditOpts = { scope: 'diff', rulesetDir: args.ruleset || '', auditLevel: level };
