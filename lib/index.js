/**
 * dsh-git-push — git 自动提交推送插件 v1.26.0（内置代码审计门禁 + dsh-repo-index 自动维护）
 *
 ...
 *
 * 审计（v1.1.0 内置）： *   L0 静态（零 token）：JS 语法 / JSON / YAML / 敏感信息硬编码 / 凭据入库 / npm 包文件入库 / 大文件 / debugger / console
...
export async function apply(ctx, config = {}) {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    const entry = { githubToken: '', sshPub: '', tokenConfigured: tok.configured, commentWordingEnabled: true, commentWordingCustom: '', commentWordingRulesFile: '', commentWordingRulesUrl: '', envInjectionEnabled: true, envInjectionTools: '' };
  ...
  // 「按照 dsh-skill-mandatory.md 写代码」「不通过 skill 文件软强制」
  // 每个 agent 只注入一次，避免每步重复烧 token。
  // v1.26.0 追加「环境注入」：工作目录映射（当前 cwd / 项目实际目录 / 父子目录）
  //   + 工具安装路径（python3/node/git/ffmpeg 等，清单同步 tools-index.md 到同级仓）。
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    const blocks = [];
    // 1) 只注入 skill 目录（路径+文件清单），不注入全文。「其实我只想注入skill目录」
    const dirs = collectRepoSkillDirs({ workspaceRoot });
    const text = formatRepoSkillDirsInjection(dirs);
    if (text) {
      blocks.push(text);
      log.info(`已注入两仓 skill 目录 ${dirs.length} 个`);
    }
    // 2) 环境注入：工作目录映射 + 工具路径（可关；tools-index.md 每次注入时同步到同级仓）
    // envInjectionTools 逗号分隔自定义工具名，非空则只探测这些（覆盖内置清单）
    if (envInjectionOn) {
      try {
        ...
        blocks.push(env.dirsText);
        blocks.push(env.toolsText);
        try {
    ...
    // 3) 设备/用户 json 脱敏注入（v1.27.0；随 envInjection 开关）
    if (envInjectionOn) {
      try {
  ...
  async function commitWithAudit({ repo, message, push, dryRun, audit, llmAudit: llm, showReadme = true, requirementsConfirmed = false }) {
    ...
    if (wantAudit) {
      ...
      const result = await commitAndPush({ repoPath: repo, message, push: push !== false, dryRun: !!dryRun, requirementsConfirmed, workspaceRoot });
    ...
    try {
      ...
      for (const repoPath of targets.paths) {
        const res = await commitWithAudit({
          repo: repoPath, message: permitCommitMessage, push: true, audit: true, showReadme: false,
        });
        results.push({
          repo: repoPath,
          ok: !!res.ok,
