...
export async function commitWithAudit({
  ...
  if (doAudit) {
    // 修复（2026-09-11）：cfg.auditRuleOrder 默认 []，空数组被当有效槽位 → loadRuleFiles([]) 零规则 → 审计 0 findings。
    // 空数组视为「未配置」，回落 undefined 走 loadRuleFiles 默认槽位解析。
    ...
    // .samples 目录豁免：审计照常出结果，但该目录内 blocker 不算提交拦截
    const blocking = res.findings.filter((f) => f.severity === 'blocker' && !isSampleExemptDir(repoPath || '.', f.file)).length;
    if (blocking > 0) {
      return { ok: false, blocked: true, error: `审计拦截：${blocking} 个 blocker`, audit: auditResult };
    }
  }
  const r = await commitAndPush({ repoPath, message, push, dryRun, customIgnorePatterns, requirementsConfirmed, force });
