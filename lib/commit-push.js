 ...
 * @param {object} p { repoPath, audit?, rulesetDir?, slots?, cfg? }
 *   （2026-09-17：删除 auditLevel——审计固定完整流程，不留强度参数）
 */
export async function runAudit({ repoPath = '', audit, rulesetDir, slots, cfg: extCfg } = {}) {
  ...
  const slotOrder = Array.isArray(slots) && slots.length ? slots
    ...
    : undefined;
  const auditRes = await auditWithScope(repoPath || '.', {
    scope: cfg.auditScanScope || 'diff',
    maxScanFiles: cfg.maxScanFiles,
    rulesetDir: rulesetDir || '', // 2026-09-17：自定规则目录已从设置移除，只认显式传参（工具 ruleset 参数）
    slots: slotOrder,
 ...
 * @param {object} p
 *   { repoPath, message, push?, dryRun?, audit?, rulesetDir? }
 *   audit 缺省取插件配置 auditEnabled；rulesetDir 只认显式传参（设置项已移除）。
...
export async function commitWithAudit({
  ...
  pushGate = false,
  pushConfirmed = false,
  paths = '', // 2026-09-21：精确 add 路径（逗号分隔，相对 repoPath）；空 = git add -A
} = {}) {
  const cfg = defaultConfig();
  const a = await runAudit({ repoPath, audit, rulesetDir, slots });
  if (!a.ok) return { ok: false, blocked: true, error: a.error, audit: a.audit };
  // 推送通道缺省取插件配置（默认 ssh：推本地 HEAD、远端 sha 与本地一致）
  const r = await commitAndPush({ repoPath, message, push, dryRun, customIgnorePatterns, requirementsConfirmed, force, pushMethod: pushMethod || cfg.pushMethod || 'ssh', pushGate, pushConfirmed, paths });
