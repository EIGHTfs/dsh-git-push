...
async function commitPushDoPush({ repoPath, branch, push, workspaceRoot, result }) {
...
export async function commitAndPush({ repoPath, message, push = true, dryRun = false, requirementsConfirmed = false, workspaceRoot = '', customIgnorePatterns = '' } = {}) {
  ...
  const pushed = await commitPushDoPush({ repoPath, branch: pre.branch, push, workspaceRoot, result });
