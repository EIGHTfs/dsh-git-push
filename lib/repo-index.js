...
export async function queryGitHubVisibility(repoName, token) {
  ...
  try {
    const res = await fetch(`https://api.github.com/repos/EIGHTfs/${encodeURIComponent(repoName)}`, {
    ...
    if (data.visibility === 'private' || data.private === false && data.visibility === 'private') return '私有';
    return '公开';
  } catch {
    return null;
  }
...
/** 从 remote URL 生成仓库地址 + 恢复命令（EIGHTfs 仓库统一 ssh 模式；私有加 token） */
export function remoteToRepoInfo(remote, visibility) {
  if (!remote) return { repoUrl: '（无 remote）', cloneCmd: '—' };
  // 归一化 github 地址：git@github.com:EIGHTfs/x.git / https://github.com/EIGHTfs/x.git / https://<token>@github.com/EIGHTfs/x.git
  let m = remote.match(/(?:git@|https?:\/\/)(?:[^@/]+@)?github\.com[:/]([\w.-]+)\/([\w.-]+?)(?:\.git)?$/);
  if (m && m[1] === 'EIGHTfs') {
    const name = m[2];
    const repoUrl = `git@github.com:EIGHTfs/${name}.git`;
    const cloneCmd = visibility === '私有'
      ? `git clone https://<token>@github.com/EIGHTfs/${name}.git`
      : `git clone ${repoUrl}`;
    return { repoUrl, cloneCmd };
  }
  return { repoUrl: remote, cloneCmd: `git clone ${remote}` };
}
 ...
 * }
 */
export async function buildRepoIndex({ workspaceRoot, depth = 3, extraRepos = [], tokenPath = '', manualVisibility = {}, localOnlyExtra = [] } = {}) {
  ...
  for (const r of repos) {
    ...
    if (!remote.includes('github.com') && !remote.includes('github.com:')) continue; // 非 GitHub 仓库不查
  ...
  lines.push('');
  lines.push('> 命名模式：除特例外全部是 `git@github.com:EIGHTfs/<项目名>.git`（公开）或 `https://<token>@github.com/EIGHTfs/<项目名>.git`（私有）。');
  lines.push('> 私有仓库 clone 需 GitHub token（本机 token 位于 `~/.dsh/git-rescue/token` 或 `dsh-test-home/git-rescue/token`）。');
  lines.push('');
  ...
  lines.push('# 2) 公开库：');
  lines.push('git clone git@github.com:EIGHTfs/<项目名>.git');
  lines.push('# 3) 私有库（dsh-link-bridge 等）：');
  lines.push('TOKEN=$(cat ~/.dsh/git-rescue/token 2>/dev/null | tr -d \' \\n\')');
  lines.push('git clone https://${TOKEN}@github.com/EIGHTfs/<项目名>.git');
  lines.push('# 4) 恢复后按 dsh-plugin-main-install 三要素装回主环境（测试实例先行 + 接管式重启）');
  lines.push('```');
  lines.push('');
  lines.push('## 四、维护约定');
  lines.push('');
