 ...
 *       到插件配置目录 credentialsDir()/dsh-repo-index.json（v1.40.0：原同级仓 dsh-git-push-User 废除）。
 *
 * 数据来源：
 ...
 * v1.27.0：md 表格索引废弃（删 dsh-repo-index.md），改为 JSON 结构；v1.40.0：同步目标改为插件配置目录。
 */
import { execSync } from 'node:child_process';
import { existsSync, readdirSync, readFileSync, mkdirSync, writeFileSync, statSync } from 'node:fs';
import { join, basename, dirname } from 'node:path';
import { scanRepos, githubFetch, parseGithubOwnerRepo, apiOriginOf, credentialsDir } from './core.js';
...
/** 查 GitHub 仓库可见性；token 缺失/请求失败返回 null（回退手工/未知）。owner 从 remote 探测，默认 EIGHTfs。 */
export async function queryGitHubVisibility(repoName, token, owner = 'EIGHTfs') {
  ...
  try {
    ...
    // v1.36.1：owner 不再硬编码——非 EIGHTfs 的仓库（如 gamebanana-mods-downloader）之前全查错 → 「未知」
 ...
 *   owner              — GitHub 账号变量（默认 EIGHTfs；v1.40.0 起只入 JSON 字段，不再决定文件路径）
 * }
 * @returns {string} JSON.stringify(obj, null, 2) 序列化结果（含 DO NOT EDIT 注释字段）
 */
/** 可见性查询：GitHub API 优先，失败回退手工标注/未知。 */
...
export async function buildRepoIndex({ workspaceRoot, depth = 3, extraRepos = [], extraReposFile = '', tokenPath = '', manualVisibility = {}, localOnlyExtra = [], owner = 'EIGHTfs' } = {}) {
  ...
  const { visibilityMap, failed } = await buildRepoIndexVisibility({ repos, token, manualVisibility, owner });
...
/**
 * v1.27.0：写入 dsh-repo-index.json。v1.40.0：目标改插件配置目录 credentialsDir()/dsh-repo-index.json
 * （原同级仓 dsh-git-push-User/<owner>/ 废除）；userDir 保留为旧参数别名（= 自定义目录）。
 * @param {{ content: string, syncTarget?: string, userDir?: string, owner?: string }} o
 *   content   — buildRepoIndex 序列化出的 JSON 字符串
 *   syncTarget— 可选覆盖目标路径（默认 credentialsDir()/dsh-repo-index.json）
 *   userDir   — 旧参数别名：给自定义目录时按 <dir>/dsh-repo-index.json 落盘
 *   owner     — 旧参数：仅写入 JSON owner 字段（路径不再按 owner 分层）
 * @returns {{ ok: boolean, written: string[] }}
 */
export function syncRepoIndex({ content, userDir = '', owner = 'EIGHTfs', syncTarget = '' } = {}) {
  const target = syncTarget
    || (userDir ? join(userDir, 'dsh-repo-index.json') : '')
    || join(credentialsDir({}), 'dsh-repo-index.json');
...
/**
 * 定位 JSON 索引文件。优先 syncTarget；v1.40.0 默认 = 插件配置目录 credentialsDir()/dsh-repo-index.json。
 * md 表格已废弃，不再作为权威源。
 */
export function resolveRepoIndexFile({ workspaceRoot = '', owner = '', syncTarget = '' } = {}) {
  if (syncTarget) return syncTarget;
  const def = join(credentialsDir({ workspaceRoot }), 'dsh-repo-index.json');
  return def;
}
...
export function formatRepoIndexInjection({ workspaceRoot = '', owner = '', syncTarget = '', full = false } = {}) {
  ...
  if (!file) {
    lines.push('插件配置目录未就绪，索引文件尚未定位。下次 git_commit_push 推送成功后自动生成 JSON。');
    return lines.join('\n');
  }
  lines.push(`文件：${file}`);
  let exists = false;
