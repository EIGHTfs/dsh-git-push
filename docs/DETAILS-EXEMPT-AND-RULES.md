...
## 十六、版本号纪律（2026-09-17 起，硬约束）

> **未经开发者明确许可，禁止升版本号。**

「升版本」指**同时或任一**改动下列三处（由 `scripts/scan-version.mjs` 校验一致性）：

| # | 位置 | 字段 |
|---|---|---|
| 1 | `lib/self/index.js` | `VERSION`（**单一事实源**） |
| 2 | `package.json` | `version` |
| 3 | `README.md` | 版本记录表「当前版本」行 |

### 执行规则

- **默认行为**：功能改动照常落码、照常提交，但版本号**保持不变**（当前停在 `1.3.8`）。
- **升版条件**：仅当开发者**当次明确许可**（如「升到 1.3.8」）才可升；此前任何「功能变更」不再自动触发升版。
- **提交信息**：不得写「版本：X → Y」，除非本次确已获许可并同步三处。
- **一致性校验**：`node scripts/scan-version.mjs` 仍须通过——不升版时三处**都保持旧值**即通过。

### 为什么这样定

历史上「功能变更 → 顺手升版本」被当作默认动作，导致版本号推进快于实际发布节奏，
且**三处同步**一旦漏改就会破坏一致性校验。改为「许可制」后，版本节奏由开发者掌握。

### 违反判定

提交 diff 中出现 `1.3.7` → 更高版本号的改动，且提交信息或对话中无开发者许可记录 → 视为违规。
